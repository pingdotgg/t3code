...
export function derivePendingRequests(activities: ReadonlyArray<OrchestrationThreadActivity>) {
  ...
  for (const activity of activities) {
    ...
    if (activity.kind === "approval.requested") {
      ...
      (activity.kind === "provider.approval.respond.failed" && isRequestResponseStale(activity))
    ) {
      closedApprovals.add(requestId);
      approvals.delete(requestId);
    } else if (
      activity.kind === "user-input.resolved" ||
      (activity.kind === "provider.user-input.respond.failed" && isRequestResponseStale(activity))
    ) {
