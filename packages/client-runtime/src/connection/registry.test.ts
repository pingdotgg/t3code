...
describe("EnvironmentRegistry", () => {
  it.effect(
    "keeps disabled persisted environments disconnected",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("preserves disabled state while updating a registration", () =>
    Effect.gen(function* () {
      ...
      const updatedProfile = new BearerConnectionProfile({
        ...BEARER_PROFILE,
      ...
      const harness = yield* makeHarness(
        ...
        { initiallyDisabled: [BEARER_TARGET.environmentId] },
      );
  ...
  it.effect("retains a live runtime when only registration labels change", () =>
    Effect.gen(function* () {
      const updatedTarget = new BearerConnectionTarget({
        ...BEARER_TARGET,
      ...
      yield* Effect.gen(function* () {
        ...
        expect(
          (yield* SubscriptionRef.get(registry.entries)).get(
            updatedTarget.environmentId,
          ),
        ).toEqual({
          target: updatedTarget,
          profile: Option.some(updatedProfile),
        });
      }).pipe(Effect.provide(harness.layer), Effect.scoped);
    }),
  );

  it.effect("replaces a live runtime when a registration endpoint changes", () =>
    Effect.gen(function* () {
      const updatedProfile = new BearerConnectionProfile({
        ...BEARER_PROFILE,
  ...
  it.effect("serializes registration edits with re-pairing", () =>
    Effect.gen(function* () {
      const editPreparationStarted = yield* Deferred.make<void>();
      const continueEdit = yield* Deferred.make<void>();
      const freshCredential = new BearerConnectionCredential({
      ...
      yield* Effect.gen(function* () {
        const registry = yield* EnvironmentRegistry.EnvironmentRegistry;
        const edit = yield* registry
          .updateRegistration(
            BEARER_TARGET.environmentId,
            Deferred.succeed(editPreparationStarted, undefined).pipe(
              Effect.andThen(Deferred.await(continueEdit)),
              Effect.as(
                new BearerConnectionRegistration({
                  target: BEARER_TARGET,
                  profile: new BearerConnectionProfile({
                    ...BEARER_PROFILE,
                    label: "Edited bearer environment",
                  }),
                  credential: BEARER_CREDENTIAL,
                }),
              ),
        ...
        yield* Deferred.await(editPreparationStarted);

        const rePair = yield* registry
          .register(
            new BearerConnectionRegistration({
              target: BEARER_TARGET,
        ...
        expect(
          (yield* Ref.get(harness.storedCredentials)).get(
            BEARER_TARGET.connectionId,
          ),
        ).toEqual(BEARER_CREDENTIAL);

        yield* Deferred.succeed(continueEdit, undefined);
  ...
  it.effect(
    "restores a persisted environment when its shadowing platform environment disappears",
    () =>
      Effect.gen(function* () {
        const shadowedTarget = new RelayConnectionTarget({
          environmentId: TARGET.environmentId,
