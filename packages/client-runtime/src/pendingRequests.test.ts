...
describe.each(["approval", "user-input"])("%s request completion", (requestKind) => {
  ...
  it.each([`${requestKind}.resolved`, `provider.${requestKind}.respond.failed`])(
    ...
    (kind) => {
      const closed = makeActivity({
        ...
        kind,
        createdAt: "2026-02-23T00:00:01.000Z",
        payload: { requestId: "request-1", reason: "request-not-found", detail: "Callback ended." },
      });
  ...
  it.each(["provider-error", "future-reason", null])(
    "keeps a failed reply retryable when reason is %s, regardless of error text",
    (reason) => {
