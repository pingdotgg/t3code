...
export function subscribeWithInputEffect<TTag extends EnvironmentSubscriptionRpcTag>(
  tag: TTag,
  input: Effect.Effect<EnvironmentRpcInput<TTag>>,
  options?: {
          ...
          Stream.switchMap(
            Option.match({
              ...
              onSome: (session) => {
                ...
                > =>
                  Stream.unwrap(
                    input.pipe(Effect.map((sessionInput) => method(sessionInput))),
                  ).pipe(
...
export function subscribe<TTag extends EnvironmentSubscriptionRpcTag>(
  ...
  EnvironmentSupervisor
> {
  return subscribeWithInputEffect(tag, Effect.succeed(input), options);
}
