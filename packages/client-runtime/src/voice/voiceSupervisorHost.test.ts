...
function scheduleNoop(_callback: () => void, _delayMs: number) {
  return () => undefined;
}
...
class FakeTransport implements RealtimeTransportController {
  ...
  connect(input: RealtimeTransportConnectInput) {
    this.input = input;
...
describe("voice supervisor host", () => {
  ...
  it("clears and cancels a pending confirmation when replacing the active generation", async () => {
    ...
    const state = makeState();
    const controller = createVoiceSupervisorHostController({ state, schedule: scheduleNoop });
    controller.start({
      voice: "marin",
      getClientSecret: async () => CLIENT_SECRET,
