...
function makeShell(input: {
  ...
  readonly sessionStatus?: "starting" | "running" | "ready" | "completed" | "idle";
...
describe("canSettle", () => {
  ...
  it("blocks settling while the shell waits on provider background tasks", () => {
    ...
    // Idle runtime alone is not live work; only a non-empty background roster is.
    const idleEmpty = makeShell({
      activityAt: FRESH,
      sessionStatus: "idle",
      pendingBackgroundTasks: [],
    });
    expect(canSettle(idleEmpty, { now: NOW })).toBe(true);

    const idleWaiting = makeShell({
      activityAt: FRESH,
      sessionStatus: "idle",
      pendingBackgroundTasks: [{ taskId: "bg-idle" }],
    });
    expect(canSettle(idleWaiting, { now: NOW })).toBe(false);
  });
