...
import { GHOSTTY_CELL_WIDE, GhosttyTerminalCore, ghosttyCellText } from "./core.ts";
import { loadGhosttyRuntime } from "./runtime.ts";
...
describe("GhosttyTerminalCore snapshots", () => {
  ...
  async function createCore(onData: (data: string) => void = () => {}) {
    const core = await GhosttyTerminalCore.create(
      await loadGhosttyRuntime(testWasmSources),
      12,
      3,
      8,
      16,
  ...
  it("preserves styles, wide cells, and selection after shared memory grows", async () => {
    ...
    const runtime = await loadGhosttyRuntime(testWasmSources);
...
describe("GhosttyTerminalCore replay brackets", () => {
  const cores = new Set<GhosttyTerminalCore>();
  const theme = {
    foreground: { r: 255, g: 255, b: 255 },
    background: { r: 0, g: 0, b: 0 },
  ...
  async function createCore(onData: (data: string) => void) {
    const runtime = await loadGhosttyRuntime(testWasmSources);
    const core = await GhosttyTerminalCore.create(runtime, 12, 3, 8, 16, theme, onData);
    cores.add(core);
    return core;
  }
  ...
  it("answers live queries but not queries inside a resetAndWrite replay", async () => {
    const replies: string[] = [];
    const core = await createCore((data) => replies.push(data));
    core.resetAndWrite("replayed\x1b[5n");
    expect(replies).toEqual([]);
    core.write("\x1b[5n");
    expect(replies).toEqual(["\x1b[0n"]);
  });

  it("keeps the writer detached until the outermost bracket ends", async () => {
    const replies: string[] = [];
    const core = await createCore((data) => replies.push(data));
    core.beginReplay();
    // resetAndWrite opens and closes its own bracket inside the outer one.
    core.resetAndWrite("base\x1b[5n");
    core.write("tail\x1b[5n");
    expect(replies).toEqual([]);
    core.endReplay();
    core.write("\x1b[5n");
    expect(replies).toEqual(["\x1b[0n"]);
    // An unmatched end never double-attaches the writer.
    core.endReplay();
    core.write("\x1b[5n");
    expect(replies).toEqual(["\x1b[0n", "\x1b[0n"]);
  });

  it("clears screen and scrollback without resetting negotiated modes", async () => {
    const core = await createCore(() => {});
    core.write(`\x1b[?1h\x1b[?1000h${"line\r\n".repeat(8)}prompt`);
    expect(core.scrollbarState()?.total).toBeGreaterThan(3);

    core.clearScreen();
    const cleared = core.snapshot();
    expect(cleared.rowData.map((row) => row.cells.map((cell) => cell.text).join(""))).toEqual([
      "",
      "",
      "",
    ]);
    expect(core.scrollbarState()?.total).toBe(3);
    expect(core.isApplicationCursorKeys()).toBe(true);
    expect(core.isMouseTracking()).toBe(true);

    // A full reset is what clearScreen avoids: it drops the modes too.
    core.resetAndWrite("");
