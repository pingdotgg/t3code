...
/**
 * Turns GitHub release notes into the groups the release notes popover shows.
 * Accepts one release body (credited to `fallbackVersion`) or electron-updater's
 * `{ version, note }` list, and keeps only the versions `includeVersion` accepts.
 */
  ...
  const rawNotes = (
    typeof releaseNotes === "string"
      ...
      : Array.isArray(releaseNotes)
        ...
        : []
  ).filter((entry) => includeVersion(entry.version));
