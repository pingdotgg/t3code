...
describe("buildGeneratedWorktreeBranchName", () => {
  ...
  it("drops the placeholder namespace when the generator echoes it", () => {
    expect(buildGeneratedWorktreeBranchName("t3code/add-safer-backoff", "Yekta")).toBe(
      "Yekta/add-safer-backoff",
    );
    expect(buildGeneratedWorktreeBranchName("t3code/add-safer-backoff", "")).toBe(
      "add-safer-backoff",
    );
  });
