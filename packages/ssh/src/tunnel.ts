    ...
    onNone: () =>
      Effect.gen(function* () {
        ...
        yield* Effect.logWarning("ssh.tunnel.httpReady.timedOut", {
          ...
          probeTimeoutMs,
          attempts: attempt,
          lastFailure: Option.getOrUndefined(lastFailure),
        });
        return yield* new SshReadinessTimedOutError({
          ...
          probeTimeoutMs,
          attempts: attempt,
          ...(Option.isSome(lastFailure) ? { cause: lastFailure.value } : {}),
        });
      }),
  });
});
    ...
    Effect.onExit((exit) =>
      Exit.isSuccess(exit)
        ...
        : child
            .kill({
              killSignal: "SIGTERM",
              forceKillAfter: Duration.toMillis(TUNNEL_SHUTDOWN_TIMEOUT),
            })
            .pipe(Effect.ignore),
    ),
  );
