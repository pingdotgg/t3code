...
const clausesToCriteria = (clauses: DecodedClause[]): AutoPullCriteria | null => {
  ...
  // Build result without undefined properties (exactOptionalPropertyTypes)
  return {
 ...
 *  does not emit (→ the editor shows a read-only "advanced rule"). */
...
// exactOptionalPropertyTypes, and TypeScript raises TS2559. Adding the
// required `provider` field makes the type non-weak and suppresses the error.
// Do NOT remove it.
