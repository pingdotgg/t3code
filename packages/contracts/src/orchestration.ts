...
export const PROVIDER_SEND_TURN_MAX_TEXT_ATTACHMENT_BYTES = 2 * 1024 * 1024;
const PROVIDER_SEND_TURN_MAX_IMAGE_DATA_URL_CHARS = 14_000_000;
const PROVIDER_SEND_TURN_MAX_TEXT_DATA_URL_CHARS = 3_000_000;
...
export const ChatTextAttachment = Schema.Struct({
  ...
  id: ChatAttachmentId,
  name: TrimmedNonEmptyString.check(Schema.isMaxLength(255)),
  mimeType: TrimmedNonEmptyString.check(Schema.isMaxLength(100), Schema.isPattern(/^text\//i)),
...
const UploadChatTextAttachment = Schema.Struct({
  ...
  dataUrl: TrimmedNonEmptyString.check(
    Schema.isMaxLength(PROVIDER_SEND_TURN_MAX_TEXT_DATA_URL_CHARS),
  ),
  preview: Schema.String,
});
...
export const OrchestrationThread = Schema.Struct({
  ...
  /**
   * Parent thread id when this thread was spawned as a nested sub-agent.
   * Null for top-level threads and for historical rows that predate the field.
   */
...
const ThreadCreateCommand = Schema.Struct({
  ...
  worktreePath: Schema.NullOr(TrimmedNonEmptyString),
  /**
   * When set, this thread is a sub-agent (or other nested session) of the given
   * parent thread. Optional for back-compat with older clients and top-level
   * sessions; null/absent means top-level.
   */
...
export const OrchestrationEvent = Schema.Union([
  ...
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.executor-model-set"),
    payload: ThreadExecutorModelSetPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
