...
const T3_PROJECT_FILE_GENERATION_PROMPT_MAX_LENGTH = 20_000;
...
export const T3ProjectFileTextGenerationPrompts = Schema.Struct({
  commitMessage: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          "Instructions that replace the default commit message writing rules for this project.",
      },
      T3_PROJECT_FILE_GENERATION_PROMPT_MAX_LENGTH,
    ),
  ),
  changeRequest: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          "Instructions that replace the default pull request and change request writing rules for this project.",
      },
      T3_PROJECT_FILE_GENERATION_PROMPT_MAX_LENGTH,
    ),
  ),
  branchName: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          "Instructions that replace the default generated branch name rules for this project.",
      },
      T3_PROJECT_FILE_GENERATION_PROMPT_MAX_LENGTH,
    ),
  ),
  threadTitle: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          "Instructions that replace the default generated thread title rules for this project.",
      },
      T3_PROJECT_FILE_GENERATION_PROMPT_MAX_LENGTH,
    ),
  ),
  threadTitleRegeneration: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          "Instructions for regenerating a thread title. Defaults to threadTitle when omitted.",
      },
      T3_PROJECT_FILE_GENERATION_PROMPT_MAX_LENGTH,
    ),
  ),
  ...
  description:
    "Project instructions for generated source control text, branch names, and thread titles.",
});
export type T3ProjectFileTextGenerationPrompts = typeof T3ProjectFileTextGenerationPrompts.Type;
...
export const T3ProjectFile = Schema.Struct({
  ...
  textGeneration: Schema.optionalKey(
    Schema.Struct({
      prompts: Schema.optionalKey(T3ProjectFileTextGenerationPrompts),
    }).annotate({
      description: "Project-specific rules for text generated by T3 Code.",
    }),
  ),
