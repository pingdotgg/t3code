...
export const DEFAULT_MODEL_BY_PROVIDER: Partial<Record<ProviderDriverKind, string>> = {
  ...
  // Product slug, not an ACP model id. The Devin adapter treats it as "the session's current model".
...
> = {
  ...
  [OPENCODE_DRIVER_KIND]: "openai/gpt-5",
  // Product slug; the Devin text-generation layer treats it as "the session's current model".
  [DEVIN_DRIVER_KIND]: "default",
};
