...
export type DesktopThemeAppearance = "light" | "dark";
...
export const DesktopThemeAppearanceSchema = Schema.Literals(["light", "dark"]);
const DesktopThemeColorSchema = Schema.String.check(
  Schema.isPattern(/^#(?:[0-9a-f]{3}|[0-9a-f]{4}|[0-9a-f]{6}|[0-9a-f]{8})$/i),
);
export const DesktopThemePaletteSchema = Schema.Struct({
  appearance: DesktopThemeAppearanceSchema,
  background: DesktopThemeColorSchema,
  foreground: DesktopThemeColorSchema,
  accent: DesktopThemeColorSchema,
  titlebarSymbol: Schema.optionalKey(DesktopThemeColorSchema),
});
export const DesktopUpdateChannelSchema = Schema.Literals(["latest", "nightly"]);
export const DesktopAppStageLabelSchema = Schema.Literals(["Alpha", "Dev", "Nightly"]);

export interface DesktopThemePalette {
  readonly appearance: DesktopThemeAppearance;
...
export interface DesktopBridge {
  ...
  setTheme: (theme: DesktopTheme, palette?: DesktopThemePalette) => Promise<void>;
