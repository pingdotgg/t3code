 ...
 * `~/.grok/sessions/**\/updates.jsonl`, OpenCode's `opencode.db` message
 * table) rather than relying on T3 Code's own orchestration projections, so
 ...
 *
 * v5 only adds `grok` and v6 only adds `opencode` to {@link UsageProviderKind};
