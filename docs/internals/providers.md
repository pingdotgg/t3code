...
there. Grok is the exception: its only updater is the CLI itself, which detects its own
installer, so any resolved executable runs `<binary> update`. Cursor runs on the SDK bundled with
T3 Code, so it has no provider update. Anything unproven stays
