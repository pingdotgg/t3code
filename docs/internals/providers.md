...
The `zai` driver is the one exception on transport: Z.ai ships no CLI, so the driver instantiates
the Claude runtime (adapter, text generation, probes) against Z.ai's Anthropic-compatible endpoint,
with a dedicated `CLAUDE_CONFIG_DIR` and its own driver kind stamped on sessions and events.
...
[zai]: ../../apps/server/src/provider/Drivers/ZaiDriver.ts
