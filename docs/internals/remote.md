...
| `P2pConnectionTarget`     | Environments dialed by DHT public key over a peer-to-peer tunnel.        |

Bearer, relay, SSH, and P2P are persisted; primary is platform-managed. Note that Tailscale is not a
...
The environment announces itself on the Hyperswarm DHT under a persistent keypair and relays each
accepted Noise stream to its own HTTP port; clients dial the z-base-32 public key and expose the
encrypted stream as loopback base URLs, then run the ordinary bearer path over them — the same
tunnel-to-loopback shape as SSH access. [`packages/p2p`](../../packages/p2p) owns hyperdht on both
sides: `announcer.ts` on the server (driven by `P2pEndpointRuntime` in
`apps/server/src/remoteAccess/`, seeded from `secrets/p2p-endpoint-seed.bin`), `dialer.ts` in the
Electron main process (`apps/desktop/src/p2p/DesktopP2pEnvironment.ts`, bridged to the renderer over
IPC). Mobile has no Node runtime, so its dialer is a Bare worklet
(`apps/mobile/src/p2p/worklet/`, react-native-bare-kit): hyperdht runs on the Bare thread behind a
loopback `bare-tcp` listener, and `apps/mobile/src/p2p/gateway.ts` drives it over line-delimited
JSON IPC (`protocol.ts`), starting it on first dial, suspending it with the app state, and
terminating it when the last tunnel closes. The worklet bundle is generated by
`apps/mobile/scripts/build-p2p-worklet.mjs` (bare-pack). Native Holepunch addons (`udx-native`,
`sodium-native`, `bare-tcp`, …) are vendored into `react-native-bare-kit` at `pod install` /
Android link time via its `prepare_command` (`ios/link.mjs` / `android/link.mjs`); the repo patches
those scripts so pnpm's virtual store still resolves transitive addons (same flat
`.pnpm/node_modules` stage the worklet packer uses). Browsers without a host bridge cannot dial —
the gateway fails with `ConnectionBlockedError("unsupported")`.

Announcing is off by default and turns on through the `remoteAccess.p2pEnabled` server setting, or
is forced on with `t3 serve --p2p` / `T3CODE_P2P`. Pairing uses a dedicated URL form,
`t3+p2p://<z32-key>/?bootstrap=host:port#token=…` (helpers in
[`shared/remote.ts`](../../packages/shared/src/remote.ts)), carried through the ordinary pairing
flow: dial first, then descriptor fetch and bearer bootstrap over the tunnel. HTTP-layer auth applies
in full — possession of the public key is equivalent to knowing a hostname, nothing more.

The only third-party touchpoint is the DHT itself (public Holepunch bootstrap nodes by default,
overridable with `remoteAccess.p2pBootstrap` or `--p2p-bootstrap`), and it is discovery-only: peers
and relaying DHT nodes ever see ciphertext, since the stream is end-to-end Noise-encrypted between
the client and the environment. Deleting the seed file rotates the endpoint identity; every paired
device must then re-pair. When hole punching fails (hostile NATs), hyperdht falls back to relaying
through DHT nodes — still encrypted, at higher latency.
