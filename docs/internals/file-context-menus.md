...
- **Open** — opens the file with the host's default application. When a surface already renders
  its own primary open row (a markdown chip's "Open in \<editor\>"), this folds into the Open
  with submenu as "Default Application" so the menu keeps one top-level open action.
