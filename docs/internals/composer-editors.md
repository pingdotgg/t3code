...
The composer has two editing surfaces behind one contract. The default surface is Lexical and
renders the prompt as plain Markdown. The `composerRichTextEnabled` client setting switches to
the Tiptap surface, which renders `**bold**`, `*italic*`, `` `code` ``, and `~~strike~~` styled
while the stored prompt keeps its markers.

## The contract both surfaces share

- The draft store owns the prompt as Markdown text. Either surface must serialize to the
  identical string for the same document: same markers, same canonical chip sources, same
  newlines. `ComposerPromptEditor` branches on the setting and both halves initialize from the
  controlled value, so flipping the setting remounts without losing the draft.
...
- Chips (mentions, skills, citations, context references) are inline atoms in Tiptap and
  decorators in Lexical. Arrow keys step over them, Backspace removes the whole chip, and copy
...
two spaces. Running it above both surfaces keeps their behavior and their serialized Markdown
...
runs above the editor via the store, so behavior matches the plain surface.
...
transaction, a caret-element scroll on the Lexical surface). Mashing Enter
