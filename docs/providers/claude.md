...
This guide is for people who want to use more than one Claude setup in more Code.
...
In more Code Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means more Code uses your normal home directory.
...
Then add another Claude provider in more Code:
...
more Code only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so more Code keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in more Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. more Code stores the value as a server secret and does not
...
more Code does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in more Code.
