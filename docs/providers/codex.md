...
This guide is for people who want to use more than one Codex account in more Code.
...
In more Code Settings, name it something obvious:
...
In more Code Settings, add another Codex provider:
...
more Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, more Code treats it
