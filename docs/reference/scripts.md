...
- Dev commands default `MORECODE_T3CODE_STATE_DIR` to `~/.moreCode/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.moreCode-2`
...
Set `MORECODE_T3CODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `MORECODE_T3CODE_DEV_INSTANCE`)
- Example: `MORECODE_T3CODE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `MORECODE_T3CODE_PORT_OFFSET` to a numeric offset.
