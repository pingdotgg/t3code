...
- `vp run --filter t3 build:binary:darwin-arm64` — Builds the self-contained Apple Silicon `t3` CLI with Bun into `apps/server/dist/t3-darwin-arm64`.
- `vp run --filter t3 build:binary:linux-x64` — Builds the self-contained Linux x86_64 `t3` CLI with Bun into `apps/server/dist/t3-linux-x64`.
