...
- When `T3CODE_HOME` is unset, T3 Code uses `$XDG_DATA_HOME/t3code` for application data and `$XDG_CONFIG_HOME/t3code` for settings on Linux and macOS when the corresponding XDG variable is an absolute path. Existing `~/.t3` data or settings remain in use until the matching XDG directory is initialized; new installations use XDG immediately. An unset or invalid XDG variable keeps that category in the legacy tree. `T3CODE_HOME` and `--home-dir <path>` take precedence and keep data and settings together.
