...
| ACP Agent  | Any ACP-compatible CLI                                | Settings       | CLI-specific login    |

Codex and Claude are on by default. Cursor, Grok Build, and OpenCode are off by default; turn
them on in **Settings** → the provider's card when you want to use them.

Add an **ACP Agent** in **Settings** with the CLI command and ACP stdio arguments; install and authenticate it separately on the server machine.
