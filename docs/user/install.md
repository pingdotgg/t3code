...
| Cursor      | Sign in with Cursor from T3 Code's provider settings.                                        |
...
Cursor runs on an SDK that ships with T3 Code, so it needs no CLI. Antigravity can
use its managed runtime without a `PATH` entry.
