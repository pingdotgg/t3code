...
Each card shows its branch, linked projects and threads, recent activity, and whether it is safe to
remove.
...
Use **Automatic cleanup** to choose how long inactive safe worktrees are retained. Enable **Delete
orphaned worktrees immediately** to remove safe worktrees as soon as their last linked thread is
deleted. Protected worktrees remain listed for you to inspect; T3 Code never automatically removes
