...
thinking picker marks Pi's current configured level as the default without overriding it. The
**Pi default** model keeps the model from Pi's own settings. Threads use Pi's native session files
for resume and rollback, so a thread started in T3 Code also appears in Pi's `/resume` list. If a
thread's session file is deleted, the thread continues in a new Pi session without the earlier
context. Extension dialogs appear in the T3 Code composer, and the context meter follows Pi's
own usage reporting after each response and when a turn settles.
...
- **Auto-accept edits** allows Pi's edit and write tools, but asks before commands and extension
  tools.
- **Auto** has the same rules as **Supervised** because Pi has no AI approval reviewer.
