...
client connected to it. Enforcement relies on the provider reporting context usage, so a provider
that never reports it is not limited. When a thread reaches the limit, send `/compact` to shrink
