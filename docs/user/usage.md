...
record are missing from the totals. OpenCode usage is read from its local store.
...
`XDG_DATA_HOME` environment variable. Use absolute paths or `~/` paths in the account's environment settings; relative
