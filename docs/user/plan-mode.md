...
Enable **Plan Mode (Legacy)** in **Settings → General → Legacy features**. The preference follows
the connected environment, so web, desktop, and mobile clients using that environment converge on
the same setting.
