...
Use the composer at the bottom of a pane exactly as you would in the full thread: drafts, attachments, models, modes, approvals, structured questions, sending, and stopping stay scoped to that thread. Select the thread title or the arrow in its header to open the same chat as the main workspace.
