...
- Builds four artifacts in parallel for both channels:
  - macOS `arm64` DMG
  - macOS `x64` DMG
  - Linux `x64` AppImage plus a `.deb` emitted by the same electron-builder run
...
- Required release assets for updater:
  ...
  - the Linux `.deb` ships in the same release but is not an updater payload: in-app updates on Linux
    stay AppImage-only, so `.deb` installs upgrade through `dpkg`/`apt`
