...
- Windows artifacts are WSL-capable by default. The release captures its own Linux Node runtime and audits it with the WSL native runtime inside an Ubuntu 22.04 compatibility container (glibc 2.35) on an Ubuntu 24.04 runner, smoke-tests the same bytes again on Ubuntu 24.04, and Windows packaging fails closed unless every staged Linux runtime/native hash matches that receipt exactly.
...
## 1a) Windows WSL packaging invariant

A normal Windows desktop artifact **includes WSL support by default**. `scripts/build-desktop-artifact.ts`
now treats that as a release contract rather than an optional best-effort feature:

- `--wsl-prebuild <path-to-linux-pty.node>` (or `T3CODE_DESKTOP_WSL_PREBUILD`) is required for a WSL-capable Windows build.
- The prebuild directory must also contain `wsl-native-abi.json`, generated by `scripts/audit-wsl-native-abi.mjs` inside the declared Ubuntu 22.04 compatibility-floor container. The job intentionally uses an Ubuntu 24.04 host runner so the ABI baseline does not depend on the retiring `ubuntu-22.04` runner label.
- The ABI audit checks ELF class/machine, unresolved dynamic dependencies, and required `GLIBC_*`, `GLIBCXX_*`, and `CXXABI_*` symbol versions for the bundled Linux Node executable, `node-pty`, the Linux `fff` binary, and Linux `ffi-rs`.
- The declared glibc ceiling is 2.35. The libstdc++/CXXABI ceilings are derived from the Ubuntu 22.04 compatibility container's actual `libstdc++.so.6` and serialized into the receipt.
- The same runtime/native bytes are smoke-tested on Ubuntu 22.04 and Ubuntu 24.04 before Windows packaging is allowed to proceed; the smoke script itself runs under the bundled Linux Node executable.
- After staged production dependencies are installed on Windows, packaging verifies the server entry, `effect`, bundled Linux Node, `node-pty`, Linux `fff`, and Linux `ffi-rs`, then compares SHA-256 for all four Linux runtime/native artifacts against the ABI receipt. Package-resolution drift therefore fails closed.
- The staged app records the capability contract in `apps/desktop/prod-resources/wsl-capability.json` and embeds the ABI receipt as `apps/desktop/prod-resources/wsl-native-abi.json`.
- Packaged runtime preflight never discovers Node from the user's WSL shell. It converts the packaged `resources/wsl-node/bin/node` path inside the already-pinned distro, hashes and copies those audited bytes into a per-distro Linux cache, restores executable permission there, and uses that exact cached Node for native probing, Linux-side port allocation, and final server launch. Development builds keep the existing user-Node/node-gyp path.
- This bundled executable is the server runtime, not a replacement for every provider package manager. `npm`, `pnpm`, `bun`, and provider CLIs are still discovered from the distro/user PATH when a provider install or update requires them.
- Runtime WSL preflight reports the selected distro's libc family/version before loading `node-pty`: known GNU glibc versions below 2.35 and musl-based distros fail with an actionable compatibility message, while an unknown libc identity is allowed to reach the real native load probe.
- If any required WSL asset is absent, any ABI ceiling is exceeded, any runtime smoke fails, or any staged native hash differs from the audited bytes, the Windows build fails instead of producing an installer whose WSL toggle may fail at runtime.

The release workflow downloads the ABI-audited artifact produced by `build_wsl_node_pty` and passes
`pty.node` to the Windows packaging command; the adjacent Linux `node` executable and ABI receipt are
consumed automatically. Electron Builder promotes the staged Node executable to `resources/wsl-node`
outside `app.asar`, because WSL must read it through the Windows filesystem before copying it into the
Linux cache. Keep that dependency, receipt, external-resource rule, validation job, and argument intact.

For deliberate Windows-only development artifacts, opt out explicitly:

```sh
vp run dist:desktop:artifact -- --win --without-wsl
```

The equivalent environment switch is `T3CODE_DESKTOP_WITHOUT_WSL=true`. Do not use this opt-out for
normal release artifacts. It exists so local Windows packaging can be intentionally host-only without
being mistaken for a WSL-capable build.
