...
more Code has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.moreCode/userdata/logs/server.trace.ndjson`).
...
```bash
export MORECODE_T3CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export MORECODE_T3CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export MORECODE_T3CODE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export MORECODE_T3CODE_TRACE_MIN_LEVEL=Info
export MORECODE_T3CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `MORECODE_T3CODE_OTLP_*`.

macOS app bundle example:

```bash
MORECODE_T3CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
MORECODE_T3CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
MORECODE_T3CODE_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/more Code.app/Contents/MacOS/more Code"
```
...
```bash
tail -f "$MORECODE_T3CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$MORECODE_T3CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$MORECODE_T3CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- `MORECODE_T3CODE_OTLP_TRACES_URL` was not set
...
- `MORECODE_T3CODE_TRACE_FILE`: override trace file path
- `MORECODE_T3CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `MORECODE_T3CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `MORECODE_T3CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `MORECODE_T3CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `MORECODE_T3CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `MORECODE_T3CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `MORECODE_T3CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `MORECODE_T3CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `MORECODE_T3CODE_OTLP_SERVICE_NAME`: service name, default `t3-server`
