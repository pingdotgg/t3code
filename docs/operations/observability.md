...
protocol other than `http/protobuf` or `http/json` such as `grpc`, or headers that are not
`key=value` pairs with percent-encoded values turn that signal's export off with a startup warning,
rather than sending it to the Settings endpoint.
