...
- Generated from `upstream/main` `c1ec1915fc16f3dc1ec5d47d9a97f6210a574526` with starting branch HEAD `71410010cbe2b30023cbbc715e627a9e046e86b7`. The merge is intentionally left uncommitted with `MERGE_HEAD` at `c1ec1915fc16f3dc1ec5d47d9a97f6210a574526`; the previous committed upstream merge remains `37aa959a3ed4b32e42a59828aaf09936e68353da`.
- Shared refs at merge time: `origin/main` and local `main` `e5490591393654b39ac99e97ccae679f7157a765`, branch tracking ref `origin/split/version-control-panel-work` `71410010cbe2b30023cbbc715e627a9e046e86b7`.
- With the no-commit merge open, recorded HEAD is `75 ahead / 3 behind upstream/main`, `75 ahead / 465 behind origin/main`, and `75 ahead / 465 behind local main`. Committing the current merge result would make the branch `76 ahead / 0 behind upstream/main` and `79 ahead / 465 behind` both shared-main refs. Including the staged upstream merge and this documentation refresh, the working-tree fork diff is `56 files changed, 15240 insertions(+), 350 deletions(-)` against the `upstream/main` tree.
- Merge note: the three incoming commits from `ef943a26abe4dd95fabd32d81ccc4e66f62de2e1` through `c1ec1915fc16f3dc1ec5d47d9a97f6210a574526` add a responsive non-truncating chat error-alert layout, platform-appropriate marketing commit shortcuts, and Android mobile support. Android support includes native composer/header/review-diff modules, a Ghostty-backed terminal, Android navigation/menus/dialogs, app shortcuts, platform assets/configuration, and mobile Git-sheet presentation updates. Git merged all files automatically with no conflicts. `apps/web/src/components/chat/ThreadErrorBanner.tsx` now uses upstream's width-constrained alert and correctly nested description/tooltip structure while retaining the fork's thread-scoped source-control error and source-control-only dismissal semantics. The mobile Git sheets remain a separate client surface and do not replace the fork's singleton web Version Control panel, server-owned source-control operations, or panel RPC/contracts. The earlier merge's offline environment/VCS-ref cache, worktree-aware `expectedBranch` compare-and-set updates, and VCS action presentation cleanup remain integrated with the panel's query/command atoms, monotonic metadata routing, and bounded snapshot/detail cache. No Version Control panel customization was retired, and upstream still does not provide an equivalent panel.
