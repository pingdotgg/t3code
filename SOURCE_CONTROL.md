...
The `Actionable` header has a `Fetch` action. The panel runs the same fetch-all-remotes path once when it first opens for a cwd, including after switching to a different conversation/worktree cwd, so upstream changes that have not yet been fetched can appear without requiring the user to press `Fetch`. Periodic remote fetch timing comes from the resolved server Background activity settings and remains subject to the shared background policy when its timer fires. The default settings use a custom profile based on `balanced` with only the Git fetch interval overridden to five minutes, keeping the other balanced policy values intact. Explicit `performance`, `balanced`, and `battery-saver` profile selections use intervals of 15 seconds, 30 seconds, and zero respectively, and Source Control settings may apply another custom interval. A zero interval disables automatic Git fetches while preserving the explicit Fetch action.
