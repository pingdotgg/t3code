...
Background preview mini-player lifecycle changes remain orthogonal to an active singleton Source Control surface. Adding or removing browser surfaces and closing a mini-player must leave the Source Control surface open, visible, singleton, and targeted to the same environment, thread, and effective repository cwd.
