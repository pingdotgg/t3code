...
T3 Code is a minimal GUI for coding agents. A Node WebSocket server wraps provider CLIs (Codex, Claude Code, Cursor, Devin, Grok, OpenCode) and serves web, desktop, and mobile clients.
...
- **provider** means the agent runtime or harness T3 Code talks to, such as Codex, Claude, Cursor, Devin, or OpenCode.
...
- **Providers.** Codex, Claude, Cursor, Devin, Devin Cloud, Grok, and OpenCode each have an adapter. Provider-shaped features need a decision per adapter, even if the decision is "not supported here".
