...
T3 Code is a minimal GUI for coding agents. A Node WebSocket server wraps provider CLIs (Codex, Claude Code, Cursor, Grok, OpenCode, Z.ai) and serves web, desktop, and mobile clients.
...
- **provider** means the agent runtime or harness T3 Code talks to, such as Codex, Claude, Cursor, Z.ai, or OpenCode.
...
- **Providers.** Codex, Claude, Cursor, Grok, OpenCode, and Z.ai each have an adapter. Provider-shaped features need a decision per adapter, even if the decision is "not supported here".
