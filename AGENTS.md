...
- UA Control Center (first slice, task #635): desktop **View → Open Unified Agent Control Center** loads the hub `/ui` in the existing preview pane (default `http://127.0.0.1:8765/ui`; set `T3CODE_UA_CONTROL_CENTER_URL` to point at a remote hub, e.g. one reached over Tailscale). Auth is hub `ua_ui` cookie via BrowserSession — not MCP bearer. Origin/CORS follow-ups are task #636 if embed is blocked.
