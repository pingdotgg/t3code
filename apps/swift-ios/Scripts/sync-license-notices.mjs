...
const resolved = JSON.parse(await readFile(new URL(
  "T3Code.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved", app
), "utf8"));
const bundled = [
  {
    "name": "GhosttyKit",
...
const swiftPackages = resolved.pins.map((pin) => ({
  name: ({ "clerk-ios": "Clerk", nuke: "Nuke", phonenumberkit: "PhoneNumberKit" })[pin.identity] ?? pin.identity,
  version: pin.state.version,
  revision: pin.state.revision,
  url: pin.location.replace("https://github.com/", "https://raw.githubusercontent.com/")
    .replace(/\.git$/, "") + "/" + pin.state.revision + "/LICENSE",
}));

const ghosttyNotice = await readFile(new URL(
  "../mobile/modules/t3-terminal/THIRD_PARTY_NOTICES.md", app
), "utf8");
...
const phoneNumberVersion = resolved.pins.find((pin) => pin.identity === "phonenumberkit").state.version;
bundled.find((source) => source.name === "libphonenumber metadata").version = "PhoneNumberKit " + phoneNumberVersion;
...
if (process.argv.includes("--check")) {
  ...
  async function download(url) {
    if (!downloads.has(url)) {
      downloads.set(url, fetch(url).then(async (response) => {
  ...
  async function notice(source) {
    ...
    if (source.files) {
      ...
      const listing = (await run("tar", ["-tf", archive], { maxBuffer: 8 * 1024 * 1024 })).stdout.split("\n");
      for (const suffix of source.files) {
        const matches = listing.filter((path) => path === suffix || path.endsWith("/" + suffix))
          .sort((a, b) => a.split("/").length - b.split("/").length);
        if (!matches[0]) throw new Error(source.name + " is missing " + suffix);
        const path = matches[0];
        texts.push({ sourceURL: source.url + "#" + path,
    ...
    for (const notice of texts) {
      ...
      if (source.preamble && notice === texts[0]) notice.text = source.preamble + "\n\n" + notice.text;
      if (notice.text.trim().length < 100) throw new Error("Missing license text: " + source.name);
    }
    return { name: source.name, version: source.version, ...(source.revision ? { revision: source.revision } : {}), notices: texts };
  }
  try {
    const result = [];
