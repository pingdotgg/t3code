...
enum FeatureCommandDrawerGesture {
    ...
    static let topGrabHeight: CGFloat = 50
    /// Band above the open drawer's edge, covering its handle.
    static let handleGrabHeight: CGFloat = 28
    ...
    static func grabBand(reveal: CGFloat, topInset: CGFloat) -> ClosedRange<CGFloat> {
        ...
        return max(topInset, edge - handleGrabHeight)...(edge + scrimGrabHeight)
    }
...
enum FeatureCommandDrawerFocus {
    ...
    ///
    /// The caller records one renewal after the drawer settles. Bounding the
    /// retry prevents an unavailable field or inactive window from producing a
    /// focus request on every render pass.
    static func needsFocusRenewal(
        state: FeatureCommandDrawerState,
        isFocused: Bool,
        hasRenewedAfterSettle: Bool
    ) -> Bool {
        state.isOpen && !isFocused && !hasRenewedAfterSettle
    }
}
