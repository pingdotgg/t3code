    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          MORECODE_T3CODE_PORT: "9999",
          MORECODE_T3CODE_MODE: "desktop",
          MORECODE_T3CODE_DESKTOP_LAN_HOST: "192.168.1.50",
  ...
  it.effect("resolves backend start config with a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.MORECODE_T3CODE_PORT);
        assert.isUndefined(first.env.MORECODE_T3CODE_MODE);
        assert.isUndefined(first.env.MORECODE_T3CODE_DESKTOP_LAN_HOST);
