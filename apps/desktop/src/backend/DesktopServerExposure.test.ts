...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return makeDesktopEnvironmentLayer({
    ...
    Layer.provide(
      Layer.mergeAll(
        NodeServices.layer,
        DesktopConfig.layerTest({ MORECODE_T3CODE_HOME: baseDir, ...env }),
      ),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { MORECODE_T3CODE_HOME: input.baseDir, ...input.env };
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        MORECODE_T3CODE_DESKTOP_LAN_HOST: "10.0.0.7",
        MORECODE_T3CODE_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
