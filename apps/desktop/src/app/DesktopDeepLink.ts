 ...
 * `<scheme>://threads/<environmentId>/<threadId>` (or host `app`).
 *
 * The environment id is always a server-generated UUID. The thread id is one
 * URI-encoded segment that decodes to a non-empty string — besides UUIDs the
 * app emits `import:<provider>:<session>` ids for imported threads, and agent
 * awareness ships them URI-encoded in `/threads/...` links.
 *
 ...
 * missing path segments, a non-UUID environment, an empty thread segment,
 * query strings, fragments, and trailing slashes — so the URL scheme cannot
 * become an arbitrary-navigation primitive and OAuth callback URLs pass
 * through to their own listeners.
 */
export function parseThreadDeepLink(
  ...
  const pattern = new RegExp(
    `^${scheme}://(?:${ElectronProtocol.DESKTOP_HOST}|threads)/(${UUID_SEGMENT})/(${THREAD_ID_SEGMENT})$`,
    "i",
  );
  ...
  return {
    ...
    threadId: new RegExp(`^${UUID_SEGMENT}$`, "i").test(threadId)
...
export const make = Effect.gen(function* () {
  ...
  // Retain the newest link until a renderer acknowledges listener delivery.
  ...
  const subscribe = (sender: DesktopIpc.DesktopIpcSenderWebContents | undefined) =>
    Effect.gen(function* () {
      ...
      const payload = yield* Ref.get(pending);
      return {
  ...
  return DesktopDeepLink.of({
    configure: Effect.gen(function* () {
      ...
      const open = (payload: DesktopThreadDeepLinkPayload) =>
        Effect.gen(function* () {
          const deliveryGeneration = yield* Ref.updateAndGet(generation, (value) => value + 1);
          ...
          const delivered = yield* Effect.sync(() => {
            ...
            try {
              subscriber.send(DEEP_LINK_CHANNEL, payload, deliveryGeneration);
              return true;
            } catch {
              return false;
            }
          ...
          yield* logInfo("thread deep link sent to renderer", {
            environmentId: payload.environmentId,
            threadId: payload.threadId,
          });
        });
      ...
      yield* ipc
        ...
        .pipe(
          Effect.catch((error) =>
            logWarning("deep link acknowledgment registration failed", { message: error.message }),
          ),
        );
      ...
      yield* electronApp.on("open-url", (event: { preventDefault: () => void }, url: string) => {
        ...
        return runPromise(open(payload));
      });
