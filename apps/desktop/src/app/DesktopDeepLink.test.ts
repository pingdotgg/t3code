...
describe("parseThreadDeepLink", () => {
  ...
  it("accepts the threads host emitted by agent awareness and mobile widgets", () => {
    assert.deepEqual(
      DesktopDeepLink.parseThreadDeepLink(
        `t3code://threads/${ENVIRONMENT_ID}/${THREAD_ID}`,
        "t3code",
      ),
      PAYLOAD,
    );
    assert.deepEqual(
      DesktopDeepLink.parseThreadDeepLink(
        `t3code-dev://threads/${ENVIRONMENT_ID}/${THREAD_ID}`,
        "t3code-dev",
      ),
      PAYLOAD,
    );
  ...
  it("rejects the primary alias rather than guessing an environment", () => {
    assert.isNull(
      DesktopDeepLink.parseThreadDeepLink(`t3code://threads/primary/${THREAD_ID}`, "t3code"),
    );
  });

  it("accepts an encoded import thread id, as agent awareness emits it", () => {
    assert.deepEqual(
      DesktopDeepLink.parseThreadDeepLink(
        `t3code://threads/${ENVIRONMENT_ID}/${encodeURIComponent("import:codex:session-1")}`,
        "t3code",
      ),
      { environmentId: ENVIRONMENT_ID, threadId: "import:codex:session-1" },
    );
  });

  it("rejects an empty or undecodable thread segment", () => {
    assert.isNull(
      DesktopDeepLink.parseThreadDeepLink(`t3code://threads/${ENVIRONMENT_ID}/%20`, "t3code"),
    );
    assert.isNull(
      DesktopDeepLink.parseThreadDeepLink(`t3code://threads/${ENVIRONMENT_ID}/%zz`, "t3code"),
    );
  });
  ...
  it("rejects hosts other than app and threads", () => {
    assert.isNull(
...
type AppListener = (...args: ReadonlyArray<unknown>) => void | Promise<void>;
...
const makeServices = (harness: TestHarness) => {
  ...
  const electronApp = {
    on: (eventName: string, listener: AppListener) =>
      Effect.sync(() => {
        harness.listeners.set(eventName, (...args) => {
          const delivery = listener(...args);
          if (delivery !== undefined) harness.deliveries.push(delivery);
          return delivery;
        });
      }),
  ...
  const electronWindow = {
    fromWebContentsId: (_webContentsId: number) => Effect.succeedSome(harness.window),
...
// Await the actual operation returned by each registered OS listener.
const settleDelivery = (harness: TestHarness) =>
  Effect.promise(() => Promise.all(harness.deliveries.splice(0)));
...
describe("DesktopDeepLink", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        let replyGate = Promise.withResolvers<void>();
        let firstSubscribed = Promise.withResolvers<void>();
        ...
        // oxlint-disable-next-line t3code/no-manual-effect-runtime-in-tests -- The real preload invokes Promise-based Electron IPC; bridge its calls into the test context just as DesktopIpc does.
        const runIpc = Effect.runPromiseWith(context);
        preloadElectron.on.mockImplementation((channel, listener) => events.on(channel, listener));
        preloadElectron.removeListener.mockImplementation((channel, listener) =>
          events.removeListener(channel, listener),
        );
        renderer.send.mockImplementation((channel, ...args) => events.emit(channel, {}, ...args));
        preloadElectron.invoke.mockImplementation((channel: string, raw: unknown) => {
          const handler = harness.ipcHandlers.get(channel);
          assert.isDefined(handler);
          const delayReply = channel === DEEP_LINK_SUBSCRIBE_CHANNEL && first;
          if (delayReply) first = false;
          const call = runIpc(handler!(raw, { sender: renderer.sender })).then(async (result) => {
            if (delayReply) {
        ...
        const active = vi.fn();
        const cleanup = bridge.onDeepLink!(discarded);
        yield* Effect.promise(() => firstSubscribed.promise);
        cleanup();
        const cleanupReplacement = bridge.onDeepLink!(active);
        replyGate.resolve();
        // Drain concrete IPC operations, including acknowledgments created
        // by their replies. No timer can stand in for completion of these handlers.
        while (calls.length > 0) yield* Effect.promise(() => Promise.all(calls.splice(0)));
        assert.equal(discarded.mock.calls.length, 0);
        assert.deepEqual(active.mock.calls, [[PAYLOAD]]);
        active.mockClear();

        yield* Effect.promise(() =>
          Promise.resolve(
            harness.listeners.get("open-url")!(
              { preventDefault: vi.fn() },
              `t3code://threads/${ENVIRONMENT_ID}/${OTHER_THREAD_ID}`,
            ),
          ),
        );
        while (calls.length > 0) yield* Effect.promise(() => Promise.all(calls.splice(0)));
        assert.deepEqual(active.mock.calls, [
          [{ environmentId: ENVIRONMENT_ID, threadId: OTHER_THREAD_ID }],
        ]);
        cleanupReplacement();
        while (calls.length > 0) yield* Effect.promise(() => Promise.all(calls.splice(0)));
        yield* Effect.promise(() =>
        ...
        assert.equal(active.mock.calls.length, 1);
        assert.deepEqual(yield* subscribeAs(harness, renderer), PAYLOAD);

        // A live listener can receive a newer push before its buffered reply.
        // The delayed reply must not navigate back to the older thread.
        first = true;
        replyGate = Promise.withResolvers<void>();
        firstSubscribed = Promise.withResolvers<void>();
        const latest = vi.fn();
        const cleanupLatest = bridge.onDeepLink!(latest);
        yield* Effect.promise(() => firstSubscribed.promise);
        yield* Effect.promise(() =>
          Promise.resolve(
            harness.listeners.get("open-url")!(
        ...
        replyGate.resolve();
        while (calls.length > 0) yield* Effect.promise(() => Promise.all(calls.splice(0)));
        assert.deepEqual(latest.mock.calls, [
          [{ environmentId: ENVIRONMENT_ID, threadId: OTHER_THREAD_ID }],
        ]);
        cleanupLatest();
        while (calls.length > 0) yield* Effect.promise(() => Promise.all(calls.splice(0)));
  ...
  it.effect("registers the OS listeners and the subscribe handler", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(
        ...
        [DEEP_LINK_ACK_CHANNEL, DEEP_LINK_SUBSCRIBE_CHANNEL, DEEP_LINK_UNSUBSCRIBE_CHANNEL],
      );
    });
  });
  ...
  it.effect("pushes a valid open-url link to the subscribed renderer", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* settleDelivery(harness);

      assert.equal(preventDefault.mock.calls.length, 1);
      assert.deepEqual(harness.revealed, [harness.window]);
      assert.deepEqual(renderer.send.mock.calls, [[DEEP_LINK_CHANNEL, PAYLOAD, 1]]);
    });
  });

  it.effect(
    "retains an unacknowledged push across teardown and ignores stale acknowledgments",
    () => {
      const harness = makeHarness();
      const renderer = makeSender(7);
      const next = makeSender(8);
      return Effect.gen(function* () {
        yield* configureWith(makeServices(harness));
        ...
        yield* settleDelivery(harness);
        yield* harness.ipcHandlers.get(DEEP_LINK_ACK_CHANNEL)!(1, { sender: next.sender });
        assert.deepEqual(yield* subscribeAs(harness, next), {
          environmentId: ENVIRONMENT_ID,
          threadId: OTHER_THREAD_ID,
        });
        yield* harness.ipcHandlers.get(DEEP_LINK_ACK_CHANNEL)!(2, { sender: next.sender });
        assert.isNull(yield* subscribeAs(harness, next));
      });
    },
  );
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        // A teardown before acknowledgment must not consume the buffered link.
        assert.deepEqual(yield* subscribeAs(harness, renderer), PAYLOAD);
        assert.deepEqual(yield* subscribeAs(harness, renderer), PAYLOAD);
        yield* harness.ipcHandlers.get(DEEP_LINK_ACK_CHANNEL)!(1, { sender: renderer.sender });
        assert.isNull(yield* subscribeAs(harness, renderer));
      });
    },
  );
  ...
  it.effect("drops destroyed subscribers and buffers for the next one", () => {
    ...
    return Effect.gen(function* () {
      yield* configureWith(makeServices(harness));
      const onDestroyed = vi.spyOn(first.sender, "once");
      for (let cycle = 0; cycle < 12; cycle += 1) {
        yield* subscribeAs(harness, first);
        yield* unsubscribeAs(harness, first);
      }
      yield* subscribeAs(harness, first);
      assert.equal(onDestroyed.mock.calls.length, 1);
      first.destroy();
