...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    ...
    '    [ -f "$runtime_root/node_modules/effect/package.json" ]',
    "}",
    ...
    `runtime_lock_candidate=$(mktemp -d "$runtime_parent/.${safeRuntimeId}.install-lock.XXXXXX")`,
    'printf \'%s\\n\' "$$" > "$runtime_lock_candidate/pid"',
    'cleanup_runtime_lock_candidate() { rm -rf "$runtime_lock_candidate"; }',
    "trap cleanup_runtime_lock_candidate EXIT HUP INT TERM",
    'while ! mv -T "$runtime_lock_candidate" "$runtime_lock" 2>/dev/null; do',
    "  if runtime_is_ready; then",
    `    printf 'runtimeRoot:%s\\n' "$runtime_root"`,
    "    exit 0",
    "  fi",
    '  runtime_lock_pid=$(cat "$runtime_lock/pid" 2>/dev/null || true)',
    '  case "$runtime_lock_pid" in',
    "    ''|*[!0-9]*) runtime_lock_owner_alive=false ;;",
    '    *) if kill -0 "$runtime_lock_pid" 2>/dev/null; then runtime_lock_owner_alive=true; else runtime_lock_owner_alive=false; fi ;;',
    "  esac",
    '  if [ "$runtime_lock_owner_alive" = false ]; then',
    `    runtime_lock_stale=$(mktemp -d "$runtime_parent/.${safeRuntimeId}.stale-lock.XXXXXX")`,
    '    rmdir "$runtime_lock_stale"',
    '    if mv -T "$runtime_lock" "$runtime_lock_stale" 2>/dev/null; then',
    '      rm -rf "$runtime_lock_stale"',
    "    fi",
    "  fi",
    "  sleep 0.1",
    "done",
    'cleanup_runtime_lock() { rm -rf "$runtime_lock_candidate" "$runtime_lock"; }',
    "trap cleanup_runtime_lock EXIT HUP INT TERM",
    "if runtime_is_ready; then",
    `  printf 'runtimeRoot:%s\\n' "$runtime_root"`,
    "  exit 0",
    "fi",
    ...
    'cleanup_runtime_install() { rm -rf "$runtime_tmp" "$runtime_lock"; }',
    "trap cleanup_runtime_install EXIT HUP INT TERM",
    `tar -xzf ${shellQuote(linuxArchivePath)} -C "$runtime_tmp"`,
    'test -f "$runtime_tmp/apps/server/dist/bin.mjs"',
    'test -f "$runtime_tmp/node_modules/effect/package.json"',
