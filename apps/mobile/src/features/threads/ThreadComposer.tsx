...
// Existing threads can switch model, interaction mode, and runtime mode, so
// every built-in slash command is offered here.
const THREAD_BUILT_IN_COMMANDS: ReadonlyArray<ComposerBuiltInCommand> = [
  "model",
  "plan",
  "default",
];
...
export const ThreadComposer = memo(function ThreadComposer(props: ThreadComposerProps) {
  ...
  const composerTriggerMenu = useComposerTriggerMenu({
    text: props.draftMessage,
    environmentId: props.environmentId,
    projectCwd: props.projectCwd,
    provider: selectedProviderStatus,
    builtInCommands: THREAD_BUILT_IN_COMMANDS,
    onChangeText: props.onChangeDraftMessage,
    onSelectInteractionMode: props.onUpdateInteractionMode,
  });
  ...
  return (
    ...
    >
      ...
      >
        {composerTriggerMenu.trigger && composerTriggerMenu.items.length > 0 ? (
          <View className="absolute inset-x-0 bottom-full z-10 mb-2">
            <ComposerCommandPopover
              items={composerTriggerMenu.items}
              triggerKind={composerTriggerMenu.trigger.kind}
