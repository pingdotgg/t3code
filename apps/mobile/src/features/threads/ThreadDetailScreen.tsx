...
const ThreadWaitingBadge = memo(function ThreadWaitingBadge(props: {
  readonly waiting: NonNullable<NonNullable<ThreadDetailScreenProps["selectedThread"]["session"]>["waiting"]>;
}) {
  const [nowMs, setNowMs] = useState(() => Date.now());
  useEffect(() => {
    const intervalId = setInterval(() => setNowMs(Date.now()), 1_000);
  ...
  const label = props.waiting.target.kind === "event"
  ...
  return (
    <View className="items-center px-4 pb-2 pt-2" style={{ flexShrink: 0 }}>
      <View accessible accessibilityLabel={label} className="flex-row items-center gap-2 rounded-full border border-sky-500/20 bg-sky-500/10 px-3 py-2">
...
export const ThreadDetailScreen = memo(function ThreadDetailScreen(props: ThreadDetailScreenProps) {
  ...
  const activeWorkIndicatorHeight =
    props.activeWorkStartedAt && !threadHealth?.stalled && props.selectedThread.session?.status !== "waiting"
  ...
  return (
    <GestureDetector gesture={headerDrawerGesture}>
      <View className="flex-1">
        ...
        {props.selectedThread.session?.status === "waiting" && props.selectedThread.session.waiting ? (
