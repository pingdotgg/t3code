...
import { sortPinnedThreads } from "@t3tools/client-runtime/state/thread-sort";
...
export function buildThreadListV2Items(input: {
  ...
  for (const thread of input.threads) {
    ...
    // its original spot in the synced pinned order.
  ...
  for (const thread of sortPinnedThreads(pinned)) {
    items.push({
      thread,
      variant: "card",
      snoozed: false,
