...
describe("composer settings toolbar layout", () => {
  it("does not overlay when the model chip is showing", () => {
    expect(
      composerSettingsToolbarLayout({ isExpanded: true, isVoicePresented: false }).overlayEditor,
    ).toBe(false);
    expect(
      composerSettingsToolbarLayout({ isExpanded: true, isVoicePresented: true }).overlayEditor,
    ).toBe(false);
    expect(
  ...
  it("overlays only the compact dictation strip so the hidden editor does not leave a gap", () => {
    expect(
  ...
  it("mounts the row only when the draft is expanded or dictation is showing", () => {
    ...
    expect(composerSettingsToolbarLayout({ isExpanded: true, isVoicePresented: false }).mount).toBe(
      true,
    );
    expect(composerSettingsToolbarLayout({ isExpanded: false, isVoicePresented: true }).mount).toBe(
      true,
    );
  });
});
