...
import { shouldUseMixedTaskArrangement } from "./taskOrder";
...
export function ThreadArrangementHost() {
  ...
  const enabled = useThreadListV2Enabled();
  ...
  const Sheet = shouldUseMixedTaskArrangement(enabled, tasks, capableIds)
    ? TaskArrangementSheet
    : ThreadArrangementSheet;
  return open ? (
    <Sheet onClose={() => appAtomRegistry.set(threadArrangementOpenAtom, false)} />
  ) : null;
}
