...
vi.mock("../../state/atom-registry", () => ({
  appAtomRegistry: {
    get: (atom: object) => {
      if (atom === mocks.threadShellsAtom) return mocks.threadShells;
      if (atom === mocks.queuedThreadKeysAtom) return mocks.queuedThreadKeys;
      return mocks.serverConfigs;
    },
  },
}));
...
vi.mock("../../state/use-thread-outbox", () => ({
  queuedThreadKeysAtom: mocks.queuedThreadKeysAtom,
}));
...
describe("useThreadListActions merged archive and settlement contract", () => {
  beforeEach(() => {
    ...
    mocks.queuedThreadKeys.clear();
    mocks.serverConfigs.set("environment-1", {
      environment: { capabilities: { threadSettlement: true, threadSnooze: true } },
    });
  });

  it.each(["settle", "snooze", "unsnooze", "unarchive", "delete"] as const)(
    "waits for the registered row dismissal before dispatching %s",
    async (action) => {
      ...
      const unregister = registerThreadDismissal("environment-1:thread-1", dismiss);
      const live = useThreadListActions();
      const archived = useArchivedThreadListActions();
      try {
        const result =
          action === "settle"
            ? live.settleThread(thread)
            : action === "snooze"
              ? live.snoozeThread(thread, "2099-01-01T00:00:00.000Z")
              : action === "unsnooze"
                ? live.unsnoozeThread(thread)
                : action === "unarchive"
                  ? archived.unarchiveThread(thread)
                  : archived.deleteThread(thread);
        expect(dismiss).toHaveBeenCalledOnce();
        await Promise.resolve();
        expect(mocks.threadCommands.mutations[action]).not.toHaveBeenCalled();
        finished.resolve();
        await result;
        expect(mocks.threadCommands.mutations[action]).toHaveBeenCalledOnce();
  ...
  it("keeps the void archive adapter deduplicated and refreshes after success", async () => {
    ...
    mocks.threadCommands.mutations.archive.mockImplementationOnce(() => {
      dispatched.resolve();
    ...
    mocks.refreshArchivedThreadsForEnvironment.mockImplementationOnce(() => refreshed.resolve());
