...
export function ConnectionsNewRouteScreen({
  ...
  const connectAndClose = useCallback(
    async (pairingUrl: string, replaceWithHome: boolean, label?: string) => {
      setIsSubmitting(true);
      onChangeConnectionPairingUrl(pairingUrl);
      try {
        const result = await onConnectPress(pairingUrl, label);
  ...
  const handleSubmit = useCallback(async () => {
    await connectAndClose(buildPairingUrl(hostInput, codeInput), false, nameInput);
  }, [codeInput, connectAndClose, hostInput, nameInput]);
