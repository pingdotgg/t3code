...
import { getTerminalThemeForColors } from "../../../terminal/terminalTheme";
...
export function TerminalAppearancePreview(props: { readonly fontSize: number }) {
  const scheme = useAppearanceColorScheme();
  const { themeColors } = useAppearancePreferences();
  const theme = getTerminalThemeForColors(scheme, themeColors);
