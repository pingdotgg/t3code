...
var fragment=document.createElement('div');fragment.appendChild(range.cloneContents());fragment.querySelectorAll('.actions,.file-actions,.file-menu,.inline-icon').forEach(function(node){node.remove();});fragment.querySelectorAll('.source').forEach(function(node){var display=node.closest('.display');if(node.hidden||(display&&display.querySelector('.equation')))node.remove();});fragment.querySelectorAll('[data-copy-source]').forEach(function(node){node.replaceWith(document.createTextNode(node.getAttribute('data-copy-source')));});fragment.querySelectorAll('.equation').forEach(function(node){node.replaceWith(document.createTextNode(node.getAttribute('data-source')));});
