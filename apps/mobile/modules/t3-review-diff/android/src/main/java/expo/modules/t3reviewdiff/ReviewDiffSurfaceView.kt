...
class ReviewDiffSurfaceView(context: Context) : ViewGroup(context) {
  private val canvasView = DiffCanvasView(context)
  var onDebug: ((Map<String, Any>) -> Unit)? = null
  var onVisibleFileChange: ((Map<String, Any>) -> Unit)? = null
  var onToggleFile: ((Map<String, Any>) -> Unit)? = null
  var onToggleViewedFile: ((Map<String, Any>) -> Unit)? = null
  var onPressLine: ((Map<String, Any>) -> Unit)? = null
  var onToggleComment: ((Map<String, Any>) -> Unit)? = null
  ...
  init {
    contentDescription = "t3-review-renderer-smoke"
  ...
  override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
    canvasView.measure(widthMeasureSpec, heightMeasureSpec)
    ...
    when (row.kind) {
      "file" -> {
        ...
        if (target == RowTapTarget.VIEWED_CHECKBOX) {
          onToggleViewedFile?.invoke(mapOf("fileId" to row.resolvedFileId))
        } else {
          onToggleFile?.invoke(mapOf("fileId" to row.resolvedFileId))
        }
      }
      "comment" -> onToggleComment?.invoke(mapOf("commentId" to row.id))
      "line" -> {
  ...
  private fun emitVisibleFile(firstVisibleIndex: Int) {
    ...
    onVisibleFileChange?.invoke(mapOf("fileId" to fileId))
  }
