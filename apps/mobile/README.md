...
CI uses Expo fingerprinting to reuse an existing compatible build when possible, or start a new build when native runtime inputs change. Preview, production, and default local builds all use the fingerprint runtime policy unless `MOBILE_VERSION_POLICY` explicitly overrides it.
