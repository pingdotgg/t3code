...
| `orchestration.getArchivedShellSnapshot` | `{ cursor?:int, limit?:int }` (`OrchestrationGetArchivedShellSnapshotInput`; offset cursor, `limit` ≤ 200; omitting `limit` returns everything) | `OrchestrationArchivedShellPage` = `OrchestrationShellSnapshot` fields + `archivedTotal:int`, `nextCursor:int?`          | no      | Pages are most-recently-archived first (`archived_at DESC, thread_id DESC`); `nextCursor` null means last page.        |
...
providers:{codex,claudeAgent,grok,kimi,chatgpt}, providerInstances:record,
