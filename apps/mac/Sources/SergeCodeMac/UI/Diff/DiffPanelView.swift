...
// MARK: - Two-axis AppKit scroll host

/// NSScrollView wrapper for content that must scroll on both axes with
/// native trackpad panning. The document is an NSHostingView pinned to the
/// clip view's top-leading corner and constrained to at least the viewport
/// size, so content narrower than the pane still fills it (full-width row
/// backgrounds) while wider content engages the horizontal scroller.
private struct PanScrollView<Content: View>: NSViewRepresentable {
