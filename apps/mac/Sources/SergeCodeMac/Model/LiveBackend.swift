...
//    (claudeAgent/codex/cursor/grok/kimi), with exact instance mappings for
//    separately selectable profiles such as `claude-work`. That override runs
//    before the general Claude mapping so it stays a distinct profile. The
//    removed `claudex` driver still appears in persisted threads, so it also
//    maps onto Claude Code. Drivers with no ProviderKind equivalent are
//    dropped from providers() — ProviderKind is a closed enum with no
//    `.other`. See `providerKindForServerProvider` and `providerKind(for:)`.
...
public enum LiveBackendMode: Sendable {
    case localSidecar(allowLanAccess: Bool, tailscaleServeEnabled: Bool)
...
private extension LiveBackendMode {
    ...
    var allowLanAccess: Bool {
        if case let .localSidecar(allowLanAccess, _) = self { return allowLanAccess }
        return false
    }

    var tailscaleServeEnabled: Bool {
        if case let .localSidecar(_, tailscaleServeEnabled) = self { return tailscaleServeEnabled }
        return false
    }
}
...
public actor LiveBackend: BackendService {
    ...
    public init(
        ...
        self.init(
            mode: .localSidecar(
                ...
                tailscaleServeEnabled: tailscaleServeEnabled),
    ...
    private static func uiVcsStatus(local: VcsStatusLocal, remote: VcsStatusRemote?) -> VcsStatus {
        VcsStatus(
            ...
            prMergeStateStatus: remote?.pr?.mergeStateStatus)
    }
    ...
    public func mintMobilePairing(label: String) async throws -> MobilePairingInfo {
                ...
                httpBaseUrl: remoteEndpoint.httpBaseUrl, credential: minted.credential)
        {
            ...
            guard
                let lanURL = URL(
                    string: "http://\(address):\(port)/pair#token=\(minted.credential)")
            else {
                throw LiveBackendError.noLanAddress
            }
            url = lanURL
    ...
    nonisolated static func providerKindForServerProvider(
        ...
        // `claudex` was removed as a driver; persisted threads and sessions
        // still name it, so keep mapping it onto Claude Code rather than
        // dropping those threads from the provider filter.
