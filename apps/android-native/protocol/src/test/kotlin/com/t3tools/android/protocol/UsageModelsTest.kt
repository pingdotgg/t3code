...
class UsageModelsTest {
  ...
  fun `decodes the usage v3 response`() {
    ...
    assertEquals(3, summary.contractVersion)
    assertEquals(null, summary.buckets.single().hourStart)
  ...
  @Test
  fun `decodes the usage v4 hourly bucket`() {
    val summary = Json.parseToJsonElement(
      """
      {
        "contractVersion": 4,
    ...
    assertEquals("2026-08-10T11:00:00.000Z", summary.buckets.single().hourStart)
  }
}
