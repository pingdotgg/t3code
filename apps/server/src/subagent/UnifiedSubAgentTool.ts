...
- 'spawn': Create a sub-agent on ANY provider, including different ones from yourself (e.g., Claude can spawn Codex/Grok, vice versa). Returns immediately with threadId. In Advisor/Planner mode with an executor model configured, spawned agents automatically run on the configured executor model with write access; providerInstanceId/model inputs are overridden.
