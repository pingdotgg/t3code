...
const makeWorkflowTerminalRetentionSweeper = (
  ...
  Effect.gen(function* () {
    ...
    // Per-agent session teardown for swept terminal tickets (A8). Optional so
    // leaner stacks without these services still build.
      ...
      Effect.gen(function* () {
        ...
        targets: for (const target of orderedRetentionTargets) {
          ...
          for (const ticket of ticketsToProcess) {
            ...
            // Budget bounds real work (deletes + failed delete attempts), NOT
            // candidates examined. A ticket that passed the coarse SQL cutoff
            // but failed the stale re-check is skipped without charging the
            // budget, so a lane full of stale-but-undeletable tickets can no
            // longer starve deletions in other lanes within one sweep.
...
export const makeWorkflowTerminalRetentionSweeperLive = (
  options?: WorkflowTerminalRetentionSweeperLiveOptions,
) => Layer.effect(WorkflowTerminalRetentionSweeper, makeWorkflowTerminalRetentionSweeper(options));
