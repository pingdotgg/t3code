...
function decodeCursor(
  ...
  const messageIndex = Number(cursorMatch[1]);
  const textOffset = Number(cursorMatch[2]);
...
export const threadRead = Effect.fn("ThreadReferenceToolkit.threadRead")(function* (
  ...
  const sourceThreadOption = yield* snapshotQuery.getThreadDetailById(invocation.threadId).pipe(
    Effect.tapError((cause) =>
      Effect.logWarning("failed to read task reference source", {
        sourceThreadId: invocation.threadId,
        targetThreadId: threadId,
        cause,
      }),
    ),
    Effect.mapError(() => new ThreadReferenceUnavailableError({ threadId })),
  );
  if (
  ...
  const threadOption = yield* snapshotQuery.getThreadDetailById(threadId).pipe(
    Effect.tapError((cause) =>
      Effect.logWarning("failed to read task reference target", {
        sourceThreadId: invocation.threadId,
        targetThreadId: threadId,
        cause,
      }),
