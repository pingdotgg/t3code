...
export const PreviewSnapshotTool = readonlyBrowserTool(
  Tool.make("preview_snapshot", {
    description:
      "Inspect a page before interacting. Pass tabId to inspect a specific tab; omit it to use this agent session's current tab. Returns page state, semantic elements, diagnostics, action history, and a PNG screenshot. On very large pages the metadata is reduced to fit a size limit, least useful fields first, and `truncation` lists what was omitted or trimmed.",
    parameters: PreviewAutomationTabTargetInput,
