...
export function buildAutoReviewFindingsPrompt(input: AutoReviewFindingsPromptInput) {
  const prompt = [
    ...
    "- only use a line that appears in the diff hunks below; read it off the",
    "  @@ header and count added/context lines. Use null when unsure — a line",
    "  outside the diff cannot be anchored and the comment moves to the summary",
    "- comments[].side is LEFT for removed lines, RIGHT for added/context lines, or null",
