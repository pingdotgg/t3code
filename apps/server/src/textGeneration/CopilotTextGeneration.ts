...
import { type CopilotSettings, type ModelSelection } from "@t3tools/contracts";
...
export const makeCopilotTextGeneration = Effect.fn("makeCopilotTextGeneration")(function* (
  ...
  const runCopilotJson = <S extends Schema.Top>({
    ...
    prompt: string;
    outputSchemaJson: S;
    // ponytail: modelSelection ignored — Copilot text generation runs on the
    // session's current model; a wrong guess would fail `session/set_model`.
    ...
    Effect.gen(function* () {
      ...
      const promptResult = yield* Effect.gen(function* () {
        ...
        Effect.flatMap(
          Option.match({
            onNone: () =>
              Effect.fail(
                new TextGenerationError({ operation, detail: "Copilot ACP request timed out." }),
              ),
        ...
        Effect.mapError((cause: EffectAcpErrors.AcpError | TextGenerationError) =>
          isTextGenerationError(cause)
            ...
            : new TextGenerationError({
                operation,
                detail: "Copilot ACP request failed.",
                cause,
              }),
        ),
      );
        ...
        return yield* new TextGenerationError({
          ...
          detail:
            promptResult.stopReason === "cancelled"
              ...
              : "Copilot Agent returned empty output.",
        });
      }
        ...
        Effect.catchTags({
          SchemaError: (cause) =>
            Effect.fail(
              new TextGenerationError({
                operation,
                detail: "Copilot Agent returned invalid structured output.",
                cause,
              }),
            ),
        }),
      ...
      Effect.mapError((cause) =>
        isTextGenerationError(cause)
          ...
          : new TextGenerationError({
              operation,
              detail: "Copilot ACP text generation failed.",
              cause,
            }),
      ),
      Effect.scoped,
