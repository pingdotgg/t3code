...
describe("buildCodexExecArgs (Codex no-tool guarantee)", () => {
  ...
  it("board-proposal posture ignores the user config and disables shell + web tools", () => {
    const args = buildCodexExecArgs({ ...base, noToolPosture: true });
    // `--ignore-user-config` is the Codex analog of Claude's strict-mcp
    // suppression: it stops config-driven MCP servers/developer_instructions
    // from loading. Auth still uses CODEX_HOME; model + effort are passed
    // explicitly on the CLI, so they survive. (Skills and hooks.json load
    // from CODEX_HOME independently of config.toml — user-authored local
    // config, accepted under full trust.)
    expect(args).toContain("--ignore-user-config");
    // `-s read-only` alone still permits command execution with whole-disk
    // READS (it blocks writes and network); disabling the shell tool closes
    // the prompt-injection read-to-rationale exfil channel — the Codex analog
    // of Claude's `--tools ""`.
    const disableIndex = args.indexOf("--disable");
    expect(disableIndex).toBeGreaterThanOrEqual(0);
    expect(args[disableIndex + 1]).toBe("shell_tool");
    // Provider-side web browsing (`web.run`) is direct network egress that
    // the sandbox does not cover; it must be off too.
    expect(args.join(" ")).toContain("--config tools.web_search=false");
  ...
  it("git-op posture keeps the user config and tools (unchanged behavior)", () => {
    const args = buildCodexExecArgs(base);
    expect(args).not.toContain("--ignore-user-config");
    expect(args).not.toContain("--disable");
    expect(args.join(" ")).not.toContain("tools.web_search=false");
    // The read-only sandbox stays on in both postures.
    const sandboxIndex = args.indexOf("-s");
    expect(args[sandboxIndex + 1]).toBe("read-only");
  });
});
