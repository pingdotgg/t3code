...
it.layer(Layer.merge(NodeServices.layer, CursorRequestLifetime.layer))(
  ...
  (it) => {
    ...
    it.effect(
      "keeps an unsettled workspace on scope close and removes it once the SDK settles",
      () =>
        Effect.gen(function* () {
          const fileSystem = yield* FileSystem.FileSystem;
          ...
          const trackingFileSystem = FileSystem.FileSystem.of({
            ...
            remove: (path, options) =>
              fileSystem
                .remove(path, options)
                .pipe(Effect.tap(() => Effect.sync(signalCleanupDone))),
          });
          const textGeneration = yield* makeCursorTextGeneration(cursorSettings, {
            CURSOR_API_KEY: "test-cursor-key",
          }).pipe(
