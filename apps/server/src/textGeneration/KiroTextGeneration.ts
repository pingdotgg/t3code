 ...
 * titles via a short-lived `kiro-cli acp` session.
 *
 * Kiro has no API-key HTTP surface (auth is `kiro-cli login`), so structured
 * output is collected the same way the Cursor and Grok drivers do it: prompt
 * the agent for JSON and accumulate `agent_message_chunk` text.
 *
...
export const makeKiroTextGeneration = Effect.fn("makeKiroTextGeneration")(function* (
  ...
  const runKiroJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      // Only assistant text carries the JSON payload; Kiro's reasoning chunks
      // must stay out of it.
        ...
        yield* applyKiroAcpModelSelection({
          ...
          mapError: (cause) =>
            new TextGenerationError({
              operation,
              detail: "Failed to set the Kiro model for text generation.",
              cause,
            }),
        });
        ...
        Effect.catchTags({
          SchemaError: (cause) =>
            Effect.fail(
              new TextGenerationError({
                operation,
                detail: "Kiro returned invalid structured output.",
                cause,
              }),
            ),
        }),
