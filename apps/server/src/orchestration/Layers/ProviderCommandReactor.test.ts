...
describe("ProviderCommandReactor", () => {
  ...
  effectIt.effect("lets /compact and messages queued behind it bypass the context limit", () =>
    Effect.gen(function* () {
      ...
      harness.sendTurn.mockImplementation(() =>
        Effect.succeed({ threadId, turnId: asTurnId("turn-1") }).pipe(
          Effect.tap(() => Deferred.succeed(++sentCount === 1 ? firstSent : queuedSent, undefined)),
        ),
      );
      const now = "2026-01-01T00:00:00.000Z";
      const dispatchTurn = (id: string, text: string, createdAt: string) =>
      ...
      yield* dispatchTurn("before-over-limit-compact", "hello", now);
      yield* Deferred.await(firstSent);
      yield* harness.engine.dispatch({
        type: "thread.session.set",
      ...
      yield* dispatchTurn("over-limit-compact", "/compact", "2026-01-01T00:00:01.000Z");
      yield* Deferred.await(compactionStarted);
      yield* dispatchTurn("during-over-limit-compact", "continue", "2026-01-01T00:00:02.000Z");
      yield* Effect.promise(harness.drain);
      // The provider reports the compacted context before the queue replays.
      yield* harness.engine.dispatch({
        type: "thread.activity.append",
        commandId: CommandId.make("cmd-over-limit-compaction"),
        threadId,
        activity: {
          id: EventId.make("over-limit-compaction"),
          kind: "context-compaction",
          tone: "info",
          summary: "Context compacted",
          payload: { state: "compacted", beforeTokens: 300_000, afterTokens: 50_000 },
  ...
  effectIt.effect(
    "rejects a queued message replayed while the thread stays over the context limit",
    () =>
      Effect.gen(function* () {
        const compactionStarted = yield* Deferred.make<void>();
        const releaseCompaction = yield* Deferred.make<void>();
        const blocked = yield* Deferred.make<void>();
        const blockedAgain = yield* Deferred.make<void>();
        let blockedCount = 0;
        const harness = yield* Effect.promise(() =>
          createHarness({
            compactThreadEffect: () =>
            ...
            afterActivityAppend: (kind) =>
              kind === "provider.turn.start.failed"
                ? Deferred.succeed(++blockedCount === 1 ? blocked : blockedAgain, undefined).pipe(
                    Effect.asVoid,
                  )
                : Effect.void,
          }),
        ...
        harness.sendTurn.mockImplementation(() =>
          Effect.succeed({ threadId, turnId: asTurnId("turn-1") }).pipe(
            Effect.tap(() => Deferred.succeed(firstSent, undefined)),
          ),
        );
        const now = "2026-01-01T00:00:00.000Z";
        const dispatchTurn = (id: string, text: string, createdAt: string) =>
        ...
        yield* dispatchTurn("initial", "hello", now);
        yield* Deferred.await(firstSent);
        yield* harness.engine.dispatch({
          type: "thread.session.set",
          commandId: CommandId.make("cmd-session-ready-still-over-limit"),
          threadId,
          session: {
            threadId,
            status: "ready",
        ...
        yield* harness.engine.dispatch({
          type: "thread.activity.append",
          commandId: CommandId.make("cmd-still-over-limit-context"),
          threadId,
          activity: {
            id: EventId.make("still-over-limit-context"),
            kind: "context-window.updated",
            tone: "info",
            summary: "Context updated",
            payload: { usedTokens: 300_000, maxTokens: 400_000 },
        ...
        yield* dispatchTurn("still-over-limit-compact", "/compact", "2026-01-01T00:00:01.000Z");
        yield* Deferred.await(compactionStarted);
        yield* dispatchTurn("queued-still-over-limit", "continue", "2026-01-01T00:00:02.000Z");
        // The compaction finishes but reports a context still above the limit.
        yield* harness.engine.dispatch({
          type: "thread.activity.append",
          commandId: CommandId.make("cmd-still-over-limit-compaction"),
          threadId,
          activity: {
            id: EventId.make("still-over-limit-compaction"),
            kind: "context-compaction",
            tone: "info",
            summary: "Context compacted",
            payload: { state: "compacted", beforeTokens: 300_000, afterTokens: 280_000 },
            turnId: null,
            createdAt: "2026-01-01T00:00:03.000Z",
          },
          createdAt: "2026-01-01T00:00:03.000Z",
        ...
        yield* Effect.promise(() => harness.drain());
        expect(harness.sendTurn).toHaveBeenCalledTimes(1);
        // The rejected replay still releases the queue, so the next fresh
        // message is gated directly instead of queueing behind nothing.
        yield* dispatchTurn("after-still-over-limit", "continue again", "2026-01-01T00:00:04.000Z");
        yield* Deferred.await(blockedAgain);
        yield* Effect.promise(() => harness.drain());
        expect(harness.sendTurn).toHaveBeenCalledTimes(1);
        const thread = (yield* Effect.promise(() => harness.readModel())).threads.find(
          (entry) => entry.id === threadId,
