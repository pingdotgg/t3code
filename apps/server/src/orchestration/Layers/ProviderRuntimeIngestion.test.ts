...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("keeps a running turn when loaded-session wake reports a prior state", async () => {
    const harness = await createHarness();
    const threadId = asThreadId("thread-1");
    const turnId = asTurnId("turn-loaded-wake");

    await harness.emitAndDrain([
      {
        type: "turn.started",
        eventId: asEventId("evt-loaded-wake-turn-started"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        turnId,
        createdAt: "2026-01-01T00:00:00.000Z",
      ...
      {
        type: "session.state.changed",
        eventId: asEventId("evt-loaded-wake-stale-ready"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: "2026-01-01T00:00:01.000Z",
        payload: {
      ...
      {
        type: "session.state.changed",
        eventId: asEventId("evt-loaded-wake-stale-error"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: "2026-01-01T00:00:02.000Z",
        payload: {
  ...
  it("unsettles a settled loaded session on explicit wake activity and stays ready", async () => {
    const harness = await createHarness();
    const threadId = asThreadId("thread-1");
    const snoozedUntil = "2027-01-08T00:00:00.000Z";

    await harness.dispatch({
      type: "thread.settle",
      commandId: CommandId.make("cmd-settle-wake"),
      threadId,
    });
    await harness.dispatch({
      type: "thread.snooze",
      commandId: CommandId.make("cmd-snooze-wake"),
      threadId,
      snoozedUntil,
    });
    ...
    let thread = await waitForThread(
      harness.readModel,
      (entry) => entry.settledOverride === "settled" && entry.snoozedUntil === snoozedUntil,
    );
    ...
    await harness.emitAndDrain([
      {
        type: "session.state.changed",
        eventId: asEventId("evt-wake-starting"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: "2026-01-01T00:00:02.000Z",
        payload: { state: "starting", reason: "Waking loaded Codex session." },
      },
      {
        type: "session.state.changed",
        eventId: asEventId("evt-wake-ready"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: "2026-01-01T00:00:03.000Z",
        payload: { state: "ready", reason: "Codex session is already loaded." },
      },
    ]);
    ...
    expect(thread.session?.status).toBe("ready");
    expect(thread.session?.activeTurnId).toBeNull();
    expect(thread.latestTurn?.state === "running").toBe(false);
    expect(thread.snoozedUntil).toBe(snoozedUntil);
    expect(thread.messages).toEqual([]);
  });

  it("projects running output and completion from a restored session without a T3 turn", async () => {
    const harness = await createHarness();
    const threadId = asThreadId("thread-1");
    const turnId = asTurnId("codex-queued-turn");
    const now = "2026-01-01T00:00:02.000Z";

    await harness.emitAndDrain([
      {
        type: "session.state.changed",
        eventId: asEventId("evt-restore-starting"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: now,
        payload: { state: "starting", reason: "Starting Codex App Server session." },
      },
      {
        type: "session.state.changed",
        eventId: asEventId("evt-restore-ready"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: now,
        payload: { state: "ready", reason: "Codex App Server session ready." },
      },
      {
        type: "turn.started",
        eventId: asEventId("evt-restore-turn-started"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: now,
        turnId,
      ...
      {
        type: "content.delta",
        eventId: asEventId("evt-restore-output"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: now,
        turnId,
        itemId: asItemId("assistant-1"),
        payload: { streamKind: "assistant_text", delta: "Queued work is done." },
      },
      {
        type: "item.completed",
        eventId: asEventId("evt-restore-item-completed"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: now,
        turnId,
      ...
      {
        type: "turn.completed",
        eventId: asEventId("evt-restore-turn-completed"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        createdAt: now,
        turnId,
    ...
    expect(
      thread.messages.some(
        (message) => message.role === "assistant" && message.text.includes("Queued work is done."),
      ),
    ).toBe(true);
