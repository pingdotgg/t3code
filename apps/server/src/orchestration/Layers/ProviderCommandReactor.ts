...
const make = Effect.gen(function* () {
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    // /compact bypasses the usage-limit gate below. A message that arrives while
    // compaction is still running skips the gate only at entry; the compaction
    // queue replays it afterwards and the gate re-evaluates it then, once the
    // post-compaction usage is known.
