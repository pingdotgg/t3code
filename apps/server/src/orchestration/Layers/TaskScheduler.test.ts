...
const fakeProjectionLayer = Layer.succeed(ProjectionSnapshotQuery, {
  listDueTasks: () => Effect.succeed(state.dueTasks),
} as unknown as ProjectionSnapshotQueryShape);

const fakeEngineLayer = Layer.succeed(OrchestrationEngineService, {
...
const schedulerLayerWithSettings = (settingsOverrides?: DeepPartial<ServerSettings>) =>
  makeTaskSchedulerLive({ tickIntervalMs: 60_000 }).pipe(
...
it.layer(schedulerLayerWithSettings().pipe(Layer.provideMerge(NodeServices.layer)))(
  "task scheduler",
  (it) => {
...
it.layer(
  schedulerLayerWithSettings({ enableScheduledTasks: false }).pipe(
    Layer.provideMerge(NodeServices.layer),
  ),
)("task scheduler with the feature switch off", (it) => {
  it.effect("skips the due scan and dispatches nothing", () =>
    Effect.gen(function* () {
      state.dispatched = [];
      state.dueTasks = [
        makeDueTask({ taskId: "task-off", nextFireAt: "2026-01-01T00:00:00.000Z" }),
      ];
