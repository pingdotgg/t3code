...
function mapProjectShellRow(
  ...
  return {
    ...
    ...(row.githubAccountId != null ? { githubAccountId: row.githubAccountId } : {}),
...
pending_approval_requests AS (
  ...
  const getRecentActivitySummaries: NonNullable<
    ProjectionSnapshotQueryShape["getRecentActivitySummaries"]
  > = (limit) =>
    listRecentActivitySummaryRows({ limit }).pipe(
      ...
      Effect.map((rows): ReadonlyArray<ProjectionActivitySummary> =>
        rows.map((row) => ({
          id: row.id,
          threadId: row.threadId,
          turnId: row.turnId,
            ...
            : repositoryIdentityResolver.resolve(option.value.workspaceRoot).pipe(
                Effect.map((repositoryIdentity) =>
                  Option.some({
                    ...
                    ...(option.value.githubAccountId != null
                      ? { githubAccountId: option.value.githubAccountId }
                      : {}),
