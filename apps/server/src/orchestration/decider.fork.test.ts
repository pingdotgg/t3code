...
it.layer(NodeServices.layer)("thread fork decider", (it) => {
  ...
  it.effect("defaults to the latest completed assistant turn and excludes streaming output", () =>
    Effect.gen(function* () {
      const event = requireForkEvent(
        yield* decideOrchestrationCommand({
          command: forkCommand(),
      ...
      expect(event.payload.inheritedMessages.every((message) => !message.streaming)).toBe(true);
    }),
  );
