...
it.layer(NodeServices.layer)("decider project scripts", (it) => {
  ...
  it.effect("projects legacy thread.executor-model-set events", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const initial = createEmptyReadModel(now);
      const withProject = yield* projectEvent(initial, {
  ...
  it.effect("accepts thread.executor-model.set and keeps the current cap when absent", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const initial = createEmptyReadModel(now);
      const withProject = yield* projectEvent(initial, {
      ...
      // A model-only update keeps the thread's current sub-agent cap.
      const readModelWithExecutor = yield* projectEvent(readModel, {
        ...singleResult,
        sequence: 3,
      } as Parameters<typeof projectEvent>[1]);
      const followUp = yield* decideOrchestrationCommand({
        ...
        readModel: readModelWithExecutor,
      });
      const followUpSingle = Array.isArray(followUp) ? null : followUp;
      if (followUpSingle === null) {
        throw new Error("Expected a single executor-model-set event.");
      }
      expect(followUpSingle).toMatchObject({
        type: "thread.executor-model-set",
        payload: {
          threadId: ThreadId.make("thread-1"),
          executorModelSelection: null,
