...
export function evaluateTurnStartLimits(input: {
  ...
  if (usedTokens !== undefined && usedTokens >= contextTokenLimit) {
    return {
      code: "context-limit",
      detail: `T3 usage limit: this thread has used ${usedTokens.toLocaleString("en-US")} context tokens. The hard limit is ${contextTokenLimit.toLocaleString("en-US")}. Send /compact to shrink it, or start a new thread so the old conversation is not sent to the provider again.`,
    };
  }
  return undefined;
}
