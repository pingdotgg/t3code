...
// Scans the read model's activities. The projector keeps only the kinds read
// here (REQUEST_ACTIVITY_KINDS in projector.ts), capped at the most recent 500
// plus pending async questions. Async questions remain actionable while the
// agent works, so they must not expire with the activity window.
