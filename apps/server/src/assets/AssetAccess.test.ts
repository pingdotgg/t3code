...
describe("AssetAccess", () => {
  ...
  it.effect("rejects generated-image resources without a renderable image", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3-asset-generated-image-invalid-",
      });
      const resource = {
        _tag: "generated-image" as const,
        threadId: ThreadId.make("thread-1"),
      ...
      const invalidTypeError = yield* issueAssetUrl({
        resource,
        generatedImagePath: path.join(root, "generated.txt"),
      }).pipe(Effect.flip);
