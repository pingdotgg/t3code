...
Give parallel tasks disjoint scopes, call \`operator_wait\` once instead of polling, and use the returned handoffs to create any requested integration task. These tools operate T3 Code directly, so do not drive the UI to create or monitor sidebar tasks.
`;
