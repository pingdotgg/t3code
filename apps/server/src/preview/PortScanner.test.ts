...
const makeProbeFailureLayer = (
  ...
  fetch: typeof globalThis.fetch = successfulWebFetch,
) =>
...
effectIt.effect("does not swallow terminal-triggered scan interruption", () => {
...
effectIt.effect(
  "rescans once after terminal PIDs settle without repeating unchanged probes",
  () => {
    let probeCount = 0;
...
effectIt.effect("preserves the terminal settle scan while no client is retained", () => {
  let probeCount = 0;
  const replayedPorts: Array<ReadonlyArray<number>> = [];
  const layer = makeProbeFailureLayer(() =>
    Effect.sync(() => {
...
effectIt.effect("uses the active polling interval after the initial scan finds a server", () => {
  let probeCount = 0;
  const layer = makeProbeFailureLayer(() =>
    Effect.sync(() => {
      probeCount += 1;
...
effectIt.effect("uses the active interval after a terminal scan finds a server", () => {
  let probeCount = 0;
  const layer = makeProbeFailureLayer(() =>
    Effect.sync(() => {
      probeCount += 1;
...
effectIt.effect("serializes snapshot replay with concurrent broadcasts", () =>
  Effect.gen(function* () {
...
effectIt.effect("allows snapshot listeners to trigger scans without deadlocking", () => {
...
effectIt.effect("allows scan-broadcast listeners to trigger scans without deadlocking", () => {
...
effectIt.effect("allows scan-broadcast listeners to call scan without deadlocking", () => {
...
effectIt.effect(
  "does not replay an earlier retained snapshot when the next retain scan fails",
  () => {
...
effectIt.effect("serializes final-retainer invalidation with an in-flight scan", () => {
...
effectIt.effect("removes listeners when initial snapshot replay fails", () => {
...
effectIt.effect("removes listeners when initial snapshot replay is interrupted", () => {
...
effectIt.effect("interrupts blocked listeners when their subscription scope closes", () => {
...
effectIt.effect("acknowledges delivery when a listener stops after its callback completes", () => {
  ...
  return Effect.gen(function* () {
    ...
    Effect.provide(layer),
    // Force frequent handoffs while leaving room for the scheduler's own resume operations.
    Effect.provideService(Scheduler.MaxOpsBeforeYield, 8),
  );
});

effectIt.effect("serializes concurrent scans before publishing snapshots", () =>
  Effect.gen(function* () {
