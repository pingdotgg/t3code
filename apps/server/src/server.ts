...
import { handoffRouteLayer } from "./handoff/HandoffHttpServer.ts";
...
export const makeRoutesLayer = Layer.mergeAll(
  ...
  // Authenticates against the live McpSessionRegistry via its module-level
  // handle, so it shares the MCP transport's per-session credentials without
  // needing the registry service in the router context.
  handoffRouteLayer,
).pipe(
