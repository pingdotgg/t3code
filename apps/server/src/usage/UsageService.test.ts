...
describe("UsageService", () => {
  for (const projectKey of [undefined, null]) {
    it.live(
      `prices a cold breakdown before any summary (${projectKey === null ? "outside" : "all"})`,
      () =>
        Effect.gen(function* () {
          const { transcript, settings, home } = yield* setup;
          yield* Effect.promise(() =>
            NodeFSP.writeFile(transcript, claudeLine(1, 5, "claude-fable-5", "/work/outside")),
          );
          let ratesFetches = 0;
          yield* Effect.gen(function* () {
            const service = yield* UsageService.make;
            const result = yield* service.readThreadBreakdown({
              ...WINDOW,
              ...(projectKey === undefined ? {} : { projectKey }),
            });
            assert.strictEqual(ratesFetches, 1);
            assert.strictEqual(result.rows.length, 1);
            assert.closeTo(result.rows[0]?.costUsd ?? -1, 0.00035, 1e-12);
            assert.strictEqual(result.rows[0]?.cacheWriteUsd, 0);
          }).pipe(
            Effect.provide(
              serviceLayers({
                prefix: "usage-cold-breakdown-pricing",
                home,
                settings,
                ratesDocument: {
