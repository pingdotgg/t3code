...
/**
 * `.claude.json` also stores `projects.<path>.history`, one entry per prompt
 * the user has ever typed, so on an active machine it reaches tens of
 * megabytes. Reading and parsing that on every settings request would block the
 * event loop; past this size the file is treated as unreadable instead.
 */
const MAX_CONFIG_FILE_BYTES = 8 * 1024 * 1024;

const readJsonObject = Effect.fn("ClaudeMcpConfig.readJsonObject")(function* (
  filePath: string,
): Effect.fn.Return<JsonFileRead, never, FileSystem.FileSystem> {
  const fileSystem = yield* FileSystem.FileSystem;
  // A failed stat (permissions, transient FS error) is *not* the same as a
  // missing file: the contents stay unknown, so report `unusable`.
 ...
 */
export const resolveClaudeMcpConfigFilePath = Effect.fn(
  "ClaudeMcpConfig.resolveClaudeMcpConfigFilePath",
)(function* (
 ...
 */
export const readClaudeMcpServers = Effect.fn("ClaudeMcpConfig.readClaudeMcpServers")(function* (
  config: Pick<ClaudeSettings, "homePath">,
  environment: NodeJS.ProcessEnv,
  cwd?: string,
  ...
  // cwd misses whenever it reaches us through a symlink — `/tmp` on macOS, or a
  // linked worktree — and a silent miss drops the whole `local` scope.
  const resolvedCwd = yield* fileSystem.realPath(cwd).pipe(Effect.orElseSucceed(() => cwd));

  const projects = claudeJson?.projects;
  const projectEntry =
    typeof projects === "object" && projects !== null
      ? ((projects as Record<string, unknown>)[resolvedCwd] ??
