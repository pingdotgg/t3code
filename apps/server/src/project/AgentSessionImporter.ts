  ...
  yield* Stream.runForEach(threads, (outcome) =>
    Effect.gen(function* () {
      ...
      const imported = yield* Effect.gen(function* () {
        ...
        // The cursor write is the hand-off point for a concurrent real session.
        // Re-read both projections before publishing imported history so a turn
        // that starts while the cursor write is in flight wins the race.
        const currentThread = yield* snapshots.getThreadDetailById(threadId);
        const currentBinding = yield* directory.getBinding(threadId);
        if (Option.isSome(currentThread) && currentThread.value.projectId !== input.projectId) {
          return yield* new AgentSessionThreadProjectConflictError({
            threadId,
            expectedProjectId: input.projectId,
            actualProjectId: currentThread.value.projectId,
          });
        }
        const currentImportedHistoryPresent = Option.isSome(currentThread)
          ? hasImportedHistory(currentThread.value)
          : false;
        if (
          Option.isSome(currentThread) &&
          currentImportedHistoryPresent &&
        ...
        if (
          Option.isSome(currentThread) &&
          hasImportBlockingActivity(currentThread.value, currentImportedHistoryPresent)
        ) {
          return yield* new AgentSessionThreadModifiedError({ threadId });
        }
