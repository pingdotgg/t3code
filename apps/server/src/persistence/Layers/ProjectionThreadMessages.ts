...
import { ChatAttachment, ResolvedSkillReference } from "@t3tools/contracts";
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextResolvedSkillsJson =
        row.resolvedSkills !== undefined ? JSON.stringify(row.resolvedSkills) : null;
      return sql`
        ...
        VALUES (
          ...
          COALESCE(
            ${nextResolvedSkillsJson},
            (
              SELECT resolved_skills_json
              FROM projection_thread_messages
              WHERE message_id = ${row.messageId}
            )
          ),
        ...
        DO UPDATE SET
          ...
          resolved_skills_json = COALESCE(
            excluded.resolved_skills_json,
            projection_thread_messages.resolved_skills_json
          ),
