...
export default Effect.gen(function* () {
  ...
  yield* sql`
    CREATE TABLE IF NOT EXISTS github_waitpoints (
      ...
      state TEXT NOT NULL CHECK (
        state IN ('pending', 'delivering', 'delivered', 'expired', 'cancelled')
      ),
      ...
      completed_at TEXT,
      CHECK (
        (state = 'delivering' AND delivery_lease_token IS NOT NULL AND delivery_lease_expires_at IS NOT NULL)
        OR
        (state != 'delivering' AND delivery_lease_token IS NULL AND delivery_lease_expires_at IS NULL)
      )
    )
  `;
  ...
  yield* sql`
    CREATE INDEX IF NOT EXISTS idx_github_waitpoints_thread
    ON github_waitpoints(thread_id, created_at DESC)
  `;
});
