...
it.layer(Layer.fresh(NodeSqliteClient.layerMemory()))(
  "053_ProjectionThreadSessionStatusDetail (existing database)",
  (it) => {
    it.effect("adds the column to a database that already applied 052", () =>
      Effect.gen(function* () {
        // The thread title-state migration owns 052 on main, so an installed
        // database can already be at 052 with no status_detail column.
...
it.layer(Layer.fresh(NodeSqliteClient.layerMemory()))(
  "053_ProjectionThreadSessionStatusDetail (fresh database)",
  (it) => {
