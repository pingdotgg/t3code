...
layer("052_ProjectionTurnsAssistantMessageIndex", (it) => {
  it.effect("indexes correlated canonical assistant lookups", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;

      yield* runMigrations({ toMigrationInclusive: 51 });
      const applied = yield* runMigrations({ toMigrationInclusive: 52 });
      assert.deepStrictEqual(applied, [[52, "ProjectionTurnsAssistantMessageIndex"]]);
