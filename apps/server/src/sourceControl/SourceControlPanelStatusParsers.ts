...
// @effect-diagnostics-next-line nodeBuiltinImport:off - Git paths use POSIX separators on every host; this parser is synchronous.
import * as NodePath from "node:path";

export function fileChangesRelativeToCwd(
  files: readonly VcsPanelFileChange[],
  prefix: string,
): VcsPanelFileChange[] {
  return files.map((file) => ({
    ...file,
    path: prefix ? NodePath.posix.relative(prefix, file.path) : file.path,
    originalPath:
      prefix && file.originalPath
        ? NodePath.posix.relative(prefix, file.originalPath)
        : file.originalPath,
  }));
}
...
export function parseNumstat(
  ...
  if (output.includes("\0")) {
    ...
    while (index < output.length) {
      ...
      stats.set(prefix ? NodePath.posix.relative(prefix, pathField.value) : pathField.value, {
  ...
  for (const line of output.split("\n")) {
    ...
    if (!path) continue;
    stats.set(prefix ? NodePath.posix.relative(prefix, path) : path, {
      insertions: parseCount(insertionsRaw),
      deletions: parseCount(deletionsRaw),
    });
  }
...
export function untrackedPathsFromPorcelain(status: string): string[] {
  return status
    .split(/\r?\n/u)
    .flatMap((line) => (line.startsWith("? ") ? [decodeGitQuotedPath(line.slice(2))] : []));
}
