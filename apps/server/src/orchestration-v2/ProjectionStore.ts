  ...
  Effect.gen(function* () {
    ...
    const readCanonicalProjection = (
      ...
      Effect.gen(function* () {
        ...
        // Cohort ids come from the stored previews when the caller cap
        // re-compacted the row — truncating a long id would break the joins —
        // and reuse the decoded item otherwise so unbounded rows parse once.
        const cohortPayloads = windowedTurnItemRows.map((row, index) =>
          boundedTurnItemRows[index]!.payload_json === row.payload_json
            ? (turnItems[index]! as Record<string, unknown>)
