...
export function assertOpenCode2FormReplyWithoutEventOutput(
  ...
  if (requestItem?.type !== "user_input_request") {
    throw new Error("OpenCode 2 form request item is missing");
  }
  assert.strictEqual(requestItem.questions[0]?.id, "question-0-handoff-model");
  assert.strictEqual(requestItem.status, "completed");
  assertAssistantTextIncludes(projection, "form reply fixture complete");
}
