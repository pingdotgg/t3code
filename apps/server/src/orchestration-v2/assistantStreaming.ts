...
// Adapters publish cumulative message snapshots. Deliver each stable prefix once;
// the completion snapshot always carries the final text, including any unfinished block.
export function makeAssistantEventDelivery(mode: ResponseStreamingMode) {
  ...
  return (event: ProviderAdapterV2Event): ProviderAdapterV2Event | undefined => {
    ...
    if (!ready || delivered.get(key) === ready) return undefined;
    delivered.set(key, ready);
    return event.type === "message.updated"
      ? { ...event, message: { ...event.message, text: ready } }
      : event.type === "turn_item.updated" && event.turnItem.type === "assistant_message"
        ? { ...event, turnItem: { ...event.turnItem, text: ready } }
        : event;
  };
}
