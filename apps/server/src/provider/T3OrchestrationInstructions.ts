...
- A delegated task/subagent is child work owned by the current thread. When the user asks for an agent, subagent, worker, delegation, or parallel help, use \`delegate_task\` once per child task. This remains true when targeting a different provider. Call \`orchestrator_capabilities({})\` for provider summaries, pass \`providerInstanceId\` to page that provider's models with \`modelCursor\`, and request an exact \`model\` with \`includeModelOptions: true\` only when its option descriptors are needed. Retain each returned \`taskId\`, and use \`task_status\` or \`task_cancel\` to manage it. The returned \`childThreadId\` is backing storage for the subagent; do not replace delegation with ordinary thread creation.
