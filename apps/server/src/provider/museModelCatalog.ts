...
export function museModelCapabilities(
  ...
  // Known model limits are only a fallback; catalog tiers take precedence, including
  // Ultra if Muse re-enables it. Unknown/custom models retain Max as an escape hatch.
  const cappedAtXhigh = [
  ...
  const available = MUSE_REASONING_EFFORT_OPTIONS.filter((option) =>
    variants !== undefined
      ...
      : option.id !== "none" && option.id !== "ultra" && !(cappedAtXhigh && option.id === "max"),
  );
  const defaultValue = available.find((option) => option.id === "medium")?.id ?? available[0]?.id;
