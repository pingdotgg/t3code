...
it.effect("isolates provider bindings and preserves opaque credentials", () =>
  Effect.gen(function* () {
    ...
    const a = yield* ProviderCredentialStore.makeProviderCredentialStore(
      "cursor",
      "../../personal",
    ).pipe(Effect.provideService(ServerSecretStore, secretStore));
    const b = yield* ProviderCredentialStore.makeProviderCredentialStore("cursor", "work").pipe(
      Effect.provideService(ServerSecretStore, secretStore),
    );
    const c = yield* ProviderCredentialStore.makeProviderCredentialStore(
      "other",
      "../../personal",
    ).pipe(Effect.provideService(ServerSecretStore, secretStore));
    ...
    const long = yield* ProviderCredentialStore.makeProviderCredentialStore(
      "a".repeat(64),
      "b".repeat(64),
    ).pipe(Effect.provideService(ServerSecretStore, secretStore));
    assert.isBelow(long.binding.key.length, 255);
    const delimiter = yield* ProviderCredentialStore.makeProviderCredentialStore(
      "cur",
      "sor../../personal",
    ).pipe(Effect.provideService(ServerSecretStore, secretStore));
