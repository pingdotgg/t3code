/**
 * PiDriver — `ProviderDriver` for the Pi Agent (`pi`) runtime.
 *
 * Pi exposes an ACP-based CLI. The driver is still a plain value, but
 ...
 *
 * Text generation is supported via the ACP runtime — `makePiTextGeneration`
    ...
    Effect.gen(function* () {
      ...
      const snapshot = yield* makeManagedServerProvider<PiSettings>({
        ...
        initialSnapshot: (settings) =>
          buildInitialPiProviderSnapshot(settings).pipe(Effect.map(stampIdentity)),
        checkProvider,
        // Keep version-advisory enrichment off the initial provider snapshot
        // so Pi never blocks server startup or settings rendering.
