...
export const CursorDriver: ProviderDriver<CursorSettings, CursorDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const sdkRunner = yield* CursorAgentSdk.CursorAgentSdkRunner.pipe(
        Effect.provide(CursorAgentSdk.cursorAgentSdkRunnerLiveLayer),
      );
      ...
      // Every SDK agent uses this instance's credential. Sign-out closes the
      // agents it opened before the credential is cleared.
