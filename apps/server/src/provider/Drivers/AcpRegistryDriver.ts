          ...
          onModelsDiscovered: (models) =>
            Effect.gen(function* () {
              ...
              const currentInstall = yield* getInstallState(entry.id).pipe(
                ...
                Effect.catch(() => Effect.succeed(undefined as typeof installState)),
              );
              if (currentInstall) {
          ...
          Effect.mapError(
            (cause) =>
              new ProviderDriverError({
                driver: driverKind,
                instanceId,
                detail: `Failed to build ${entry.name} snapshot: ${cause.message ?? String(cause)}`,
                cause,
              }),
          ),
        );
