/**
 * KiroDriver — `ProviderDriver` for the Kiro CLI (`kiro-cli acp`) runtime.
 *
 * Kiro speaks ACP over stdio, so the driver is thin: it wires the adapter, the
 * probe-based snapshot, and text generation together for one configured
 * instance. Unlike the other ACP drivers, the snapshot is built from plain CLI
 * probes (`--version`, `whoami`, `chat --list-models`) rather than by booting
 * an ACP session, which keeps provider refreshes cheap.
 *
 * `KIRO_HOME` scopes an instance's agents, settings, and sessions — but not its
 * credentials, which Kiro stores outside its home directory.
 *
