...
const cursorAgent = vi.hoisted(() => ({
  openedKeys: [] as Array<string | undefined>,
  closed: 0,
}));
    ...
    Agent: {
      create: async (options: { readonly apiKey?: string }) => {
        ...
        return {
          ...
          send: async () => {
            throw new Error("This test only opens a session");
          },
          close: () => {
            cursorAgent.closed += 1;
          },
        };
      },
    },
...
it.layer(testLayer)("CursorDriver", (it) => {
  it.effect(
    "persists browser credentials, uses them for sessions, and closes the SDK agent on logout",
    () =>
      Effect.gen(function* () {
        ...
        const instance = yield* CursorDriver.create(input);
        ...
        const startSession = instance.adapter.startSession({
          threadId: ThreadId.make("cursor-browser-thread"),
          providerInstanceId: input.instanceId,
          cwd: process.cwd(),
          modelSelection: { instanceId: input.instanceId, model: "auto" },
          runtimeMode: "full-access",
        });
        yield* startSession;
        expect(cursorAgent.openedKeys).toEqual(["instance-browser-key"]);
        expect(cursorAgent.closed).toBe(0);
        ...
        expect(cursorAgent.closed).toBe(1);
        ...
        expect((yield* Effect.exit(startSession))._tag).toBe("Failure");
        expect(cursorAgent.openedKeys).toEqual(["instance-browser-key"]);
        expect(cursorAgent.closed).toBe(1);
      }).pipe(Effect.scoped),
  );
