...
For browser work, first call \`preview_status\`. If no automation-capable preview is attached, call \`preview_open\` before concluding that the browser is unavailable. Then use \`preview_navigate\`, \`preview_snapshot\`, the focused interaction tools, and \`preview_diagnostics\` for console, network, performance, or memory evidence. When a task specifically needs network traffic from an upcoming navigation or action, call \`preview_diagnostics\` once before it to arm the existing T3 CDP session. Prefer snapshot-provided locators over coordinates.
