...
export interface CursorAgentSdkOpenInput {
  ...
  /** Labels this provider session in the native event log. */
...
export interface CursorAgentSdkRunnerShape {
  /** The caller's scope owns the opened agent. Wrappers may close it with that scope. */
  readonly open: (
    input: CursorAgentSdkOpenInput,
  ) => Effect.Effect<CursorAgentSdkSession, CursorAgentSdkRunnerError, Scope.Scope>;
...
>()("t3/provider/CursorAgentSdk/CursorAgentSdkRunner") {}
...
function isCursorCancellationError(cause: unknown): boolean {
  let current = cause;
...
function loggedCursorAgentOptions(options: AgentOptions): CursorAgentSdkLoggedAgentOptions {
  return {
...
function loggedCursorSendOptions(
 ...
 * transcript.
 */
function makeCursorAgentSdkRunner(
