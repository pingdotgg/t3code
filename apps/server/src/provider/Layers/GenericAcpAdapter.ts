...
import {
  ...
  currentGenericAcpModelIdFromSessionSetup,
  makeGenericAcpRuntime,
  resolveGenericAcpModelId,
  type GenericAcpSpawnSettings,
...
export interface GenericAcpAdapterSettings extends GenericAcpSpawnSettings {
...
const resolveNotificationTurnId = (ctx: GenericAcpSessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: GenericAcpSessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, GenericAcpSessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
function parseGenericAcpResume(raw: unknown): { sessionId: string } | undefined {
...
export function genericAcpPromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = genericAcpPromptSettlementBelongsToContext({
    ...
    const startSession: ProviderAdapterShape<ProviderAdapterError>["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          if (!settings.enabled || settings.command.trim().length === 0) {
            return yield* new ProviderAdapterValidationError({
              provider: PROVIDER,
              operation: "startSession",
          ...
          const resumeSessionId = parseGenericAcpResume(input.resumeCursor)?.sessionId;
          ...
          const acp = yield* makeGenericAcpRuntime({
            ...
            ...(options.authMethodId?.trim() ? { authMethodId: options.authMethodId.trim() } : {}),
          ...
          const requestedStartModelId = selectedModel?.model
            ? resolveGenericAcpModelId(selectedModel.model, PROVIDER)
            : undefined;
          const boundModelId = yield* applyGenericAcpModelSelection({
            runtime: acp,
            currentModelId: currentGenericAcpModelIdFromSessionSetup(started.sessionSetupResult),
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveGenericAcpModelId(boundModelId, PROVIDER) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          const ctx: GenericAcpSessionContext = {
            threadId: input.threadId,
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolveGenericAcpModelId(turnModelSelection.model, PROVIDER)
                : undefined;
              const currentModelId = yield* applyGenericAcpModelSelection({
                runtime: ctx.acp,
              ...
              const displayModel = currentModelId
                ? resolveGenericAcpModelId(currentModelId, PROVIDER)
                : undefined;
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "ACP prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "ACP prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "ACP session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "ACP prompt request failed.",
                }),
              );
