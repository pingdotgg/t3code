...
import {
  ...
  currentOmpReasoningEffortFromSessionSetup,
  makeOmpAcpRuntime,
...
// ACP does not expose the agent's private reasoning phase. Once OMP has
...
const DEFAULT_OMP_TURN_INACTIVITY_TIMEOUT_MS = 10 * 60 * 1_000;
...
const DEFAULT_OMP_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS = 30 * 60 * 1_000;
...
interface OmpTurnLivenessSignal {
...
interface OmpSessionContext {
  ...
  readonly livenessSignals: Queue.Queue<OmpTurnLivenessSignal>;
...
export function selectOmpPermissionOptionId(
  ...
  const pick = (kind: string) =>
    request.options.find((entry) => entry.kind === kind && entry.optionId.trim())?.optionId.trim();
  const preferredId = pick(preferredKind);
  if (preferredId) {
    return preferredId;
  }
  // Agents sometimes omit allow_always. T3 still offers "Always allow this session".
  if (decision === "acceptForSession" || decision === "acceptAlways") {
    const onceId = pick("allow_once");
    if (onceId) {
      return onceId;
    }
  }
  ...
  return Effect.gen(function* () {
    ...
    const turnInactivityTimeoutMs =
      ...
      Number.isFinite(requestedTurnInactivityTimeoutMs)
        ...
        : DEFAULT_OMP_TURN_INACTIVITY_TIMEOUT_MS;
    ...
    const activeToolInactivityTimeoutMs =
      ...
      Number.isFinite(requestedActiveToolInactivityTimeoutMs)
        ...
        : DEFAULT_OMP_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS;
    ...
    const signalTurnLiveness = (ctx: OmpSessionContext, turnId: TurnId) =>
      Queue.offer(ctx.livenessSignals, { turnId }).pipe(Effect.asVoid);

    const beginTurnLiveness = (ctx: OmpSessionContext, turnId: TurnId) =>
      Effect.sync(() => {
        ctx.livenessTurnId = turnId;
        // Do not start a deadline until ACP has made observable progress.
        // The agent's private reasoning phase is not present in the ACP stream.
        ctx.lastTurnActivityAtNanos = undefined;
        ctx.activeToolCallIds.clear();
      });

    const clearTurnLiveness = (ctx: OmpSessionContext) => {
    ...
    const recordTurnActivity = Effect.fn("OmpAdapter.recordTurnActivity")(function* (
      ctx: OmpSessionContext,
      turnId: TurnId,
      event: Extract<
    ...
    const hasLivenessPause = (ctx: OmpSessionContext) =>
      ctx.pendingApprovals.size > 0 || ctx.livenessUpdatesInFlight > 0;

    const livenessTimeoutFor = (ctx: OmpSessionContext) =>
    ...
    const resumeSessionTurnLiveness = Effect.fn("OmpAdapter.resumeSessionTurnLiveness")(function* (
      threadId: ThreadId,
      ...
      // An approval wait can last longer than the watchdog. Its resolution
      // gives the provider a fresh window to resume output.
      ctx.lastTurnActivityAtNanos = yield* Clock.monotonicTimeNanos;
      yield* signalTurnLiveness(ctx, turnId);
    });

    const refreshSessionTurnLiveness = Effect.fn("OmpAdapter.refreshSessionTurnLiveness")(
    ...
    const markPromptResponseReady = Effect.fn("OmpAdapter.markPromptResponseReady")(function* (
      threadId: ThreadId,
      acpSessionId: string,
      turnId: TurnId,
    ) {
    ...
    const consumePromptResponseReady = (ctx: OmpSessionContext) => {
    ...
    const isLiveTurn = (ctx: OmpSessionContext, turnId: TurnId) =>
    ...
    const settleStalledTurn = Effect.fn("OmpAdapter.settleStalledTurn")(function* (
      ctx: OmpSessionContext,
      turnId: TurnId,
    ) {
      return yield* withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          yield* settlePromptInFlight(ctx.threadId, turnId, ctx.acpSessionId, {
            errorMessage: `OMP ACP turn stalled without content or tool progress for ${livenessTimeoutFor(ctx).milliseconds}ms.`,
    ...
    const runTurnLivenessWatchdog = Effect.fn("OmpAdapter.runTurnLivenessWatchdog")(
      function* (ctx: OmpSessionContext) {
        while (true) {
          if (ctx.stopped) {
            return;
          }
    ...
    const startSession: OmpAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
            ...
            yield* acp.handleRequestPermission((params) =>
              mapAcpCallbackFailure(
                Effect.gen(function* () {
                  ...
                  if (rawInputFields?.variant === "Bash") {
                    const { description: _description, ...shellInput } = rawInputFields;
                  ...
                  const approvalKey =
                    command ||
                    (rawInputFields !== undefined && Object.keys(rawInputFields).length > 0)
                  ...
                  if (input.runtimeMode === "full-access" || alreadyApproved) {
                    const autoApprovedOptionId =
                      input.runtimeMode === "full-access"
                        ...
                        : selectOmpPermissionOptionId(params, "accept");
          ...
          const currentStartReasoningEffort = currentOmpReasoningEffortFromSessionSetup(
          ...
          const ctx: OmpSessionContext = {
            ...
            promptsInFlight: 0,
            livenessSignals: yield* Queue.sliding<OmpTurnLivenessSignal>(1),
            ...
            currentReasoningEffort:
              requestedStartReasoningEffort !== undefined
                ? normalizeOmpReasoningEffort(requestedStartReasoningEffort)
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              if (requestedTurnReasoningEffort !== undefined) {
                ctx.currentReasoningEffort = normalizeOmpReasoningEffort(
    ...
    const respondToUserInput: OmpAdapterShape["respondToUserInput"] = (threadId, requestId) =>
      Effect.gen(function* () {
        ...
        // OMP exposes no structured user-input extension method, so no
        // user-input request can ever be pending on this adapter.
        return yield* new ProviderAdapterRequestError({
          provider: PROVIDER,
