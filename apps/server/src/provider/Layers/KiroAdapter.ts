...
// Kiro streams no ACP progress while a model thinks. Once it has emitted
// standard ACP progress, ten silent minutes is long enough to avoid treating
// legitimate reasoning as a stalled stream.
const DEFAULT_KIRO_TURN_INACTIVITY_TIMEOUT_MS = 10 * 60 * 1_000;
...
const DEFAULT_KIRO_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS = 30 * 60 * 1_000;
// Kiro attaches the model's stated purpose to every tool input. It changes
// between otherwise identical calls, so it must not be part of an approval key.
const KIRO_TOOL_PURPOSE_KEY = "__tool_use_purpose";
...
interface KiroTurnLivenessSignal {
...
interface KiroSessionContext {
  ...
  readonly livenessSignals: Queue.Queue<KiroTurnLivenessSignal>;
...
function selectAutoApprovedPermissionOption(
  ...
  return (
    selectKiroPermissionOptionId(request, "acceptForSession") ??
    selectKiroPermissionOptionId(request, "accept")
  );
}
...
/**
 * Identity of a tool operation for "Always allow this session". Kiro's
 * per-call purpose annotation is dropped so the same command matches again.
 */
export function kiroApprovalOperationInput(rawInput: unknown): unknown {
  if (!isRecord(rawInput)) {
    return rawInput;
  }
  const { [KIRO_TOOL_PURPOSE_KEY]: _purpose, ...operationInput } = rawInput;
  return operationInput;
}

function kiroPromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
    ...
    const turnInactivityTimeoutMs =
      ...
      Number.isFinite(requestedTurnInactivityTimeoutMs)
        ...
        : DEFAULT_KIRO_TURN_INACTIVITY_TIMEOUT_MS;
    ...
    const activeToolInactivityTimeoutMs =
      ...
      Number.isFinite(requestedActiveToolInactivityTimeoutMs)
        ...
        : DEFAULT_KIRO_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS;
    ...
    const signalTurnLiveness = (ctx: KiroSessionContext, turnId: TurnId) =>
      Queue.offer(ctx.livenessSignals, { turnId }).pipe(Effect.asVoid);

    const beginTurnLiveness = (ctx: KiroSessionContext, turnId: TurnId) =>
      Effect.sync(() => {
        ctx.livenessTurnId = turnId;
        // Do not start a deadline until ACP has made observable progress.
        // Kiro emits nothing while its model reasons.
        ctx.lastTurnActivityAtNanos = undefined;
        ctx.activeToolCallIds.clear();
      });

    const clearTurnLiveness = (ctx: KiroSessionContext) => {
    ...
    const recordTurnActivity = Effect.fn("KiroAdapter.recordTurnActivity")(function* (
      ctx: KiroSessionContext,
      turnId: TurnId,
      event: Extract<
    ...
    const hasLivenessPause = (ctx: KiroSessionContext) =>
      ctx.pendingApprovals.size > 0 || ctx.livenessUpdatesInFlight > 0;

    const livenessTimeoutFor = (ctx: KiroSessionContext) =>
    ...
    const resumeSessionTurnLiveness = Effect.fn("KiroAdapter.resumeSessionTurnLiveness")(function* (
      threadId: ThreadId,
    ...
    const refreshSessionTurnLiveness = Effect.fn("KiroAdapter.refreshSessionTurnLiveness")(
    ...
    const markPromptResponseReady = Effect.fn("KiroAdapter.markPromptResponseReady")(function* (
      threadId: ThreadId,
      acpSessionId: string,
      turnId: TurnId,
    ) {
    ...
    const consumePromptResponseReady = (ctx: KiroSessionContext) => {
    ...
    const isLiveTurn = (ctx: KiroSessionContext, turnId: TurnId) =>
    ...
    const settleStalledTurn = Effect.fn("KiroAdapter.settleStalledTurn")(function* (
      ctx: KiroSessionContext,
      turnId: TurnId,
    ) {
      return yield* withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          yield* settlePromptInFlight(ctx.threadId, turnId, ctx.acpSessionId, {
            errorMessage: `Kiro ACP turn stalled without content or tool progress for ${livenessTimeoutFor(ctx).milliseconds}ms.`,
    ...
    const runTurnLivenessWatchdog = Effect.fn("KiroAdapter.runTurnLivenessWatchdog")(
      function* (ctx: KiroSessionContext) {
        while (true) {
          if (ctx.stopped) {
            return;
          }
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
            ...
            yield* acp.handleRequestPermission((params) =>
              mapAcpCallbackFailure(
                Effect.gen(function* () {
                  ...
                  const operationInput = kiroApprovalOperationInput(rawInput);
                  ...
                  if (input.runtimeMode === "full-access" || alreadyApproved) {
                    const autoApprovedOptionId =
                      input.runtimeMode === "full-access"
                        ...
                        : selectKiroPermissionOptionId(params, "accept");
          ...
          const requestedStartModelId = kiroModelSelection?.model
            ...
            : undefined;
          const currentStartModelId = currentKiroModelIdFromSessionSetup(
          ...
          const ctx: KiroSessionContext = {
            ...
            livenessSignals: yield* Queue.sliding<KiroTurnLivenessSignal>(1),
        ...
        const prepared = yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              // Kiro's prompt capabilities accept images only. Generic files
    ...
    // Kiro asks its questions as assistant text, so there is never a pending
    // structured request to answer.
