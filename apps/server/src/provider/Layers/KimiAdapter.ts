...
export function makeKimiAdapter(kimiSettings: KimiSettings, options?: KimiAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const threadLocksRef = yield* SynchronizedRef.make(
      new Map<string, { readonly semaphore: Semaphore.Semaphore; readonly refCount: number }>(),
    );
    ...
    // Reference-counted per-thread locks: each withThreadLock user holds a
    // registry reference from acquire to release, and the entry is removed
    // only when the LAST user releases. Long-lived servers therefore don't
    // retain a semaphore per stopped thread, and — unlike deleting the entry
    // inside stopSessionInternal, which runs while the lock is still held —
    // no concurrent operation can ever mint a second semaphore for a thread
    // that still has active holders or waiters.
    const acquireThreadSemaphore = (threadId: string) =>
      SynchronizedRef.modifyEffect(threadLocksRef, (current) => {
        const existing = current.get(threadId);
        if (existing) {
          const next = new Map(current);
          next.set(threadId, { semaphore: existing.semaphore, refCount: existing.refCount + 1 });
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const nf = yield* Stream.runDrain(
            ...
            // Fork into the session scope, not the calling fiber: startSession
            // may run in a short-lived caller fiber, and the pump must keep
            // forwarding ACP events until the session itself stops.
    ...
    const sendTurn: KimiAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        const prepared = yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              // Track the switch immediately: if prompt validation below fails,
              // the ACP session has still changed models, and a later turn
              // requesting the same model must not skip session/set_model
              // against a stale tracked id. Sync the session's reported model
              // in the same step so listSessions never diverges from the ACP
              // session either.
              ctx.currentModelId = currentModelId;
              const displayModel = currentModelId
                ? resolveKimiAcpBaseModelId(currentModelId)
                : undefined;
              if (displayModel && ctx.session.model !== displayModel) {
        ...
        // Plain mutable flag flipped synchronously with the append: the
        // settlement path below yields between appending and marking
        // promptSettled, and an interruption in that window must not let the
        // ensuring finalizer append the same prompt/result pair again.
        let promptResultAppended = false;
          ...
          const result = yield* prepared.acp
            ...
            .pipe(
              Effect.tap((promptResult) =>
                // Store the result before marking success: the finalizer treats
                // promptRpcSucceeded with an empty promptResultRef as "nothing
                // to settle", so an interruption between the two writes must
                // never leave success marked without its result.
                Effect.all([
          ...
          return yield* withThreadLock(
            ...
            Effect.gen(function* () {
              ...
              ctx.session = {
                ...
                updatedAt: yield* nowIso,
                ...(ctx.currentModelId
                  ? { model: resolveKimiAcpBaseModelId(ctx.currentModelId) }
                  : {}),
              };
              if (ctx.interruptedTurnIds.has(prepared.turnId)) {
                yield* Ref.set(promptSettled, true);
              ...
              // Delegate slot decrement and terminal-event emission to
              // settlePromptInFlight: only the last remaining prompt settles
              // the merged turn (a steer-superseded prompt just releases its
              // slot), and keeping promptsInFlight untouched until settlement
              // actually runs means an interruption anywhere before it leaves
              // state the ensuring finalizer can still settle from.
    ...
    const respondToRequest: KimiAdapterShape["respondToRequest"] = (
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          if (!settled) {
            // Interrupt/stop already resolved this approval as cancelled; the
            // caller's decision was not delivered, so don't report success.
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "session/request_permission",
              detail: `Approval request already settled: ${requestId}`,
            });
          }
    ...
    // Standard ACP has no user-input extension (unlike Grok's xAI
    // ask_user_question), so no user-input request can ever be pending.
