...
import { AcpConnection, type AcpConnectionShape } from "../acp/AcpConnection.ts";
...
export const makeAcpRegistryAdapter = Effect.fn("makeAcpRegistryAdapter")(function* (
  ...
  const makeEventStamp = () =>
    Effect.all({
      eventId: Effect.map(Random.nextUUIDv4, (id) => EventId.make(id)),
      createdAt: nowIso,
    });
  ...
  const acquireConnection = (spawnTarget: SpawnTarget, cwd: string) =>
    Effect.gen(function* () {
      ...
      const connection = yield* Effect.service(AcpConnection).pipe(
  ...
  const startSession: AcpRegistryAdapterShape["startSession"] = (input) =>
    withThreadLock(
      ...
      Effect.gen(function* () {
        ...
        yield* Scope.addFinalizer(sessionScope, pooled.connection.releaseSession(sessionIdForCleanup));
