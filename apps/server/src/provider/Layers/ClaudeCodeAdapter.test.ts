...
describe("ClaudeCodeAdapterLive", () => {
  ...
  it.effect("ends the prompt async iterable cleanly when stopping a session", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeCodeAdapter;
      ...
      assert.equal(prompt !== undefined, true);
      if (!prompt) {
        return;
      }

      const iterator = prompt[Symbol.asyncIterator]();
      const nextPrompt = iterator.next();

      yield* adapter.stopSession(session.threadId);

      const nextResult = yield* Effect.promise(() => nextPrompt);
      assert.equal(nextResult.done, true);
      assert.equal(nextResult.value, undefined);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
