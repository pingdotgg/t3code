...
import { resolveCopilotAcpBaseModelId } from "./CopilotProvider.ts";
...
const resolveCallbackTurnId = (ctx: CopilotSessionContext): TurnId | undefined => ctx.activeTurnId;
...
export function copilotPromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = copilotPromptSettlementBelongsToContext({
        ...
        const settleTurnId = liveCtx.activeTurnId ?? turnId;
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to record GitHub Copilot native notification.", { cause }),
        ),
      );
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          yield* applyCopilotAcpModelSelection({
            ...
            selections: copilotModelSelection?.options,
            mapError: (context) =>
          ...
          const boundModelId =
            currentCopilotModelIdFromSessionSetup(started.sessionSetupResult) ??
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveCopilotAcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
        ...
        const prepared = yield* withThreadLock(
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const currentModelId = turnModelSelection?.model
                ? resolveCopilotAcpBaseModelId(turnModelSelection.model)
                : ctx.currentModelId;
              ...
              const displayModel = currentModelId
                ? resolveCopilotAcpBaseModelId(currentModelId)
                : undefined;
