...
const OPEN_CODE_DRIVER_KINDS = new Set([
  ProviderDriverKind.make("opencode"),
  ProviderDriverKind.make("opencode2"),
]);

const shouldRetainMissingProviderModels = (provider: ServerProvider): boolean => {
  if (!OPEN_CODE_DRIVER_KINDS.has(provider.driver)) {
    return true;
  }

  // OpenCode snapshots are deliberately non-authoritative while their first
  // probe is still running. A probe error from an installed CLI/server is
  // likewise partial: it could not establish the current inventory.
...
const mergeProviderModels = (
  ...
  const mergedModels = nextModels.map((model) => {
    ...
    if (
      !previousModel ||
      shouldClearSuccessfulOpenCode2Capabilities ||
