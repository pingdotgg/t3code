...
describe("agent browser access", () => {
        ...
        issueMcpCredential: (request) =>
          Effect.sync(() => {
            issued.push({
              ...
              capabilities: ["monitor", ...request.capabilities].toSorted(),
            });
            return undefined;
          }),
      }).pipe(
  ...
  it.effect("issues a credential without preview when agent browser access is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["monitor", "pull-requests"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "monitor", "preview", "pull-requests"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("drops only the preview capability when browser access alone is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["device", "monitor", "pull-requests"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when the project overrides browser access to on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["monitor", "preview", "pull-requests"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
