...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("enriches task frames with the SDK fields the agent-run card needs", () => {
    ...
    return Effect.gen(function* () {
      ...
      const updatesForTaskOne = runtimeEvents.filter(
        (event) => event.type === "task.updated" && event.payload.taskId === "task-1",
      );
      assert.equal(
        updatesForTaskOne.length,
        2,
        "running and killed patches remain visible as distinct lifecycle updates",
      );
      const running = updatesForTaskOne[0];
      if (running?.type === "task.updated") {
        assert.equal(running.payload.status, "running");
      }
      const killed = updatesForTaskOne[1];
      if (killed?.type === "task.updated") {
        assert.equal(killed.payload.status, "cancelled");
