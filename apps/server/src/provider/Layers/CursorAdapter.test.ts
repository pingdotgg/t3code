...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
  ...
  it.effect("steers a concurrent sendTurn that arrives before the first turn is published", () =>
    Effect.gen(function* () {
      const adapter = yield* CursorAdapter;
      const settings = yield* ServerSettingsService;
      const threadId = ThreadId.make("cursor-concurrent-steer-thread");
      ...
      // Give the first prompt an attachment so it suspends on file I/O inside
      // the window between reserving the turn and publishing it.
      const serverConfig = yield* ServerConfig;
      const attachmentId = "cursor-concurrent-steer-attachment";
      ...
      // Both prompts are dispatched without waiting for the first to publish
      // its active turn id, so the second lands inside the first's
      // configuration/validation window. The turn id is reserved with the
      // in-flight counter, so the second must steer rather than open a turn.
      const firstTurnFiber = yield* adapter
        .sendTurn({
          threadId,
          input: "run 5 commands",
      ...
      yield* Effect.gen(function* () {
        ...
        throw new Error("Timed out waiting for the prompts to be in flight.");
      });
      ...
      // Exactly one turn boundary: a second turn.started here would leave one
      // of the two turn ids permanently unsettled.
      assert.equal(turnStartedEvents.length, 1);
      assert.equal(String(turnStartedEvents[0]?.turnId), String(firstTurn.turnId));
      assert.equal(turnCompletedEvents.length, 1);
      assert.equal(String(turnCompletedEvents[0]?.turnId), String(firstTurn.turnId));
