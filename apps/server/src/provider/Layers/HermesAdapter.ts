...
import type { ProviderAdapterShape as GenericProviderAdapterShape } from "../Services/ProviderAdapter.ts";
import { resolveHermesAcpBaseModelId } from "../acp/HermesAcpSupport.ts";
import { type EventNdjsonLogger, makeEventNdjsonLogger } from "./EventNdjsonLogger.ts";
const encodeUnknownJsonStringExit = Schema.encodeUnknownExit(Schema.fromJsonString(Schema.Unknown));

const PROVIDER = ProviderDriverKind.make("hermes");
type ProviderAdapterShape = GenericProviderAdapterShape<ProviderAdapterError>;
...
export function makeHermesAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const sendTurn: ProviderAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          const resolvedModel = resolveHermesAcpBaseModelId(model);
          yield* applyRequestedSessionConfiguration({
            runtime: ctx.acp,
    ...
    const interruptTurn: ProviderAdapterShape["interruptTurn"] = (threadId) =>
      Effect.gen(function* () {
    ...
    const respondToRequest: ProviderAdapterShape["respondToRequest"] = (
      threadId,
      requestId,
      decision,
    ) =>
    ...
    const respondToUserInput: ProviderAdapterShape["respondToUserInput"] = (
      threadId,
      requestId,
      answers,
    ) =>
    ...
    const readThread: ProviderAdapterShape["readThread"] = (threadId) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
        return { threadId, turns: ctx.turns };
      });

    const rollbackThread: ProviderAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
    ...
    const stopSession: ProviderAdapterShape["stopSession"] = (threadId) =>
      withThreadLock(
        threadId,
        Effect.gen(function* () {
          const ctx = yield* requireSession(threadId);
    ...
    const listSessions: ProviderAdapterShape["listSessions"] = () =>
      Effect.sync(() => Array.from(sessions.values(), (c) => ({ ...c.session })));

    const hasSession: ProviderAdapterShape["hasSession"] = (threadId) =>
      Effect.sync(() => {
