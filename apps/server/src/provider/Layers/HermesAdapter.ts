/**
 * HermesAdapter — per-instance Hermes adapter.
 *
 * Maps the Hermes ACP stdio server (`hermes acp`) onto the canonical
 * `ProviderRuntimeEvent` stream via the shared ACP session runtime
 * ({@link ../acp/AcpSessionRuntime}).
 *
 * Design decisions (verified against Hermes' ACP source in
 * acp_adapter/{server,permissions,auth}.py and its public docs):
 *
 * - **Transport.** `hermes acp` is an ACP stdio server. It advertises a
 *   terminal setup auth method (`hermes-setup`) but reuses Hermes' own
 *   runtime credentials, so T3 authenticates with `hermes-setup` and ignores
 *   the returned auth info.
 * - **Model switching.** Hermes implements `session/set_model`, so
 *   `capabilities.sessionModelSwitch` is `"in-session"`. Hermes encodes model
 *   ids as `provider:model`; `session/set_model` resolves T3's
 *   `provider/model` slugs through `parse_model_input`.
 * - **Approvals.** Hermes presents stable permission option ids:
 *   `allow_once`, `allow_session` (session-scoped "Allow for session"),
 *   `allow_always`, `deny`, `deny_always`. T3 decisions map accept →
 *   `allow_once`, acceptForSession → `allow_session` (falling back to
 *   `allow_always`), decline → `deny`. `full-access` runtime mode
 *   auto-approves with the session-scoped option.
 * - **Steering.** A prompt sent while Hermes is already running is folded
 *   into the active turn (Hermes redirects or queues it), so sendTurn reuses
 *   the active turn id — same contract as the Grok adapter.
 * - **rollback.** Hermes ACP has no turn rollback, so `rollbackThread` fails
 *   with `ProviderAdapterValidationError`; checkpoint revert still restores
 *   the workspace.
 * - **user input.** Hermes has no ask-user extension surface, so
 *   `respondToUserInput` fails clean.
 *
...
/**
 * Select the Hermes permission option id for a T3 approval decision.
 *
 * Option ids are stable in Hermes' ACP adapter (acp_adapter/permissions.py):
 * `allow_once`, `allow_session`, `allow_always`, `deny`, `deny_always`.
 * Fall back to standard ACP kinds, then to the shared outcome string
 * (`acpPermissionOutcome`), for servers that present different ids — Hermes
 * maps unknown option ids to deny, so the fallback can only downgrade.
 */
function selectHermesPermissionOptionId(
  request: EffectAcpSchema.RequestPermissionRequest,
  decision: Exclude<ProviderApprovalDecision, "cancel">,
): string | undefined {
  const preferredIds =
    decision === "acceptForSession"
      ? (["allow_session", "allow_always"] as const)
      : decision === "accept"
        ? (["allow_once"] as const)
        : (["deny", "deny_always"] as const);
  for (const optionId of preferredIds) {
    if (request.options.some((entry) => entry.optionId === optionId)) {
      return optionId;
    }
  }
  const kind =
    ...
    withThreadLock(
      ...
      Effect.gen(function* () {
        ...
        const acp = yield* makeHermesAcpRuntime({
          ...
          skipConfiguredMcp: mcpSession !== undefined,
          ...(mcpSession
            ? {
                mcpServers: [
                  {
            ...
            mapAcpCallbackFailure(
              Effect.gen(function* () {
                ...
                if (input.runtimeMode === "full-access") {
                  const autoApprovedOptionId = selectAutoApprovedHermesPermissionOption(params);
        ...
        yield* offerRuntimeEvent({
          ...
          payload: {
            config: {
              ...
              launchArgs: hermesSettings.launchArgs,
    ...
    Effect.gen(function* () {
      const prepared = yield* withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // A sendTurn while a prompt is in flight is a steer: Hermes folds
          // the new prompt into the ongoing work (redirect or queue), so the
          ...
          return yield* Effect.gen(function* () {
            ...
            const displayModel = requestedTurnModelId ?? currentModelId ?? undefined;
  ...
  const respondToUserInput: HermesAdapterShape["respondToUserInput"] = () =>
    Effect.fail(
      new ProviderAdapterValidationError({
        ...
        issue:
          "Hermes ACP exposes no user-input extension surface; there are no user-input requests to answer.",
      }),
    );
  ...
  const rollbackThread: HermesAdapterShape["rollbackThread"] = (threadId, numTurns) =>
    Effect.gen(function* () {
      ...
      return yield* new ProviderAdapterValidationError({
        provider: PROVIDER,
        operation: "rollbackThread",
        issue: "Hermes ACP has no turn rollback; checkpoint revert restores the workspace.",
      });
    });
