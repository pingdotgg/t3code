...
async function waitForFileContent(filePath: string, attempts = 40) {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    // Microtask yield, matching the previous Effect.yieldNow pacing: each loop
    // iteration already awaits real fs I/O, so no wall-clock timer is needed.
...
describe("selectOmpPermissionOptionId", () => {
  ...
  ): Parameters<typeof selectOmpPermissionOptionId>[0] => ({ options }) as never;
...
ompAdapterTestLayer("OmpAdapterLive", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // ChildProcess.kill does not wait for the child to exit, so poll
        // until both signal handlers have appended their SIGTERM entry.
        let exitLog = "";
        for (let attempt = 0; attempt < 400; attempt += 1) {
          exitLog = yield* Effect.promise(() => NodeFSP.readFile(exitLogPath, "utf8"));
          if ((exitLog.match(/SIGTERM/g)?.length ?? 0) >= 2) {
            break;
          }
          // This test runs under a frozen TestClock, so poll with a
          // clock-independent yield: each iteration already awaits real fs
          // I/O, which is what actually advances the child's exit writes.
          yield* Effect.yieldNow;
        }
  ...
  it.effect(
    "cancels a turn during preparation before dispatch and releases the dispatch lock",
    () =>
      Effect.gen(function* () {
        const adapter = yield* OmpAdapter;
        const serverSettings = yield* ServerSettingsService;
        const threadId = ThreadId.make("omp-prepare-cancel-serialized");
        const tempDir = yield* Effect.promise(() =>
          NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "omp-acp-")),
        );
        const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
        const argvLogPath = NodePath.join(tempDir, "argv.txt");
        yield* Effect.promise(() => NodeFSP.writeFile(requestLogPath, "", "utf8"));
        // A slow set_config_option keeps the turn inside the dispatch permit
        // (configuration write) so the interrupt lands before dispatch.
        const wrapperPath = yield* Effect.promise(() =>
          makeProbeWrapper(requestLogPath, argvLogPath, {
            T3_ACP_SET_CONFIG_OPTION_DELAY_MS: "500",
          }),
        ...
        assert.equal(
          threadEvents.some((event) => event.type === "turn.started"),
          false,
          "a turn cancelled during preparation never stamps turn.started",
        );

        const preCancelRequests = yield* Effect.promise(() => readJsonLines(requestLogPath));
        assert.equal(
          preCancelRequests.filter((entry) => entry.method === "session/prompt").length,
          0,
          "cancelled-during-prepare turn must never reach session/prompt",
        );

        // The dispatch permit must be released even though the first turn
        // died mid-configuration: a follow-up turn still dispatches.
        ...
        assert.isAtLeast(
          requests.filter((entry) => entry.method === "session/prompt").length,
          1,
          "the follow-up turn reaches session/prompt",
        );
  ...
  it.effect("serializes each turn's configuration write with its prompt dispatch", () =>
    Effect.gen(function* () {
      const adapter = yield* OmpAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("omp-dispatch-serialization");
      const tempDir = yield* Effect.promise(() =>
        NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "omp-acp-")),
      );
      const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
      const argvLogPath = NodePath.join(tempDir, "argv.txt");
      yield* Effect.promise(() => NodeFSP.writeFile(requestLogPath, "", "utf8"));
      // Slow configuration writes widen the window in which a concurrent
      // sendTurn could interleave its own model write before the first
      // turn's prompt dispatches.
      const wrapperPath = yield* Effect.promise(() =>
        makeProbeWrapper(requestLogPath, argvLogPath, {
      ...
      const order = requests
        .filter(
          (entry) =>
            entry.method === "session/set_config_option" || entry.method === "session/prompt",
        )
        .map((entry) => {
          ...
          if (params && typeof params === "object" && "configId" in params) {
            return String(params.configId ?? "");
          }
          return "";
        });
      // The dispatch lock must keep each turn's model write adjacent to its
      // own dispatch: set(model A) -> prompt -> set(model B) -> prompt. An
      // interleaving such as set(A) -> set(B) -> prompt would mean a turn
      // ran under another turn's model.
      const modelWrites = order.filter((entry) => entry !== "prompt");
      const prompts = order.filter((entry) => entry === "prompt").length;
      assert.isAtLeast(modelWrites.length, 2, "both turns wrote their model");
      assert.equal(prompts, 2, "both turns dispatched");
      for (let index = 0; index < order.length; index += 1) {
        if (order[index] === "prompt") {
          continue;
        }
        if (index + 1 < order.length) {
          assert.equal(
            order[index + 1],
            "prompt",
            `model write at ${index} must be immediately followed by its own dispatch`,
          );
        }
      }

      yield* adapter.stopSession(threadId);
      // Live clock so the mock's real-time configuration delay applies.
    }).pipe(TestClock.withLive),
  );
