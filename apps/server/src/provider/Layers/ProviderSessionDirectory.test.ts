...
it.layer(makeDirectoryLayer(SqlitePersistenceMemory))("ProviderSessionDirectoryLive", (it) => {
  ...
  it("touchLastSeen bumps last_seen_at for a live binding", () =>
    Effect.gen(function* () {
      ...
      const touched = yield* directory.touchLastSeen(threadId);
  ...
  it("touchLastSeen does not resurrect a stopped binding", () =>
    Effect.gen(function* () {
      ...
      // Reported as a no-op so callers do not mistake it for a live refresh.
      const touched = yield* directory.touchLastSeen(threadId);
  ...
  it("touchLastSeen reports no-op for an absent binding", () =>
    Effect.gen(function* () {
      const directory = yield* ProviderSessionDirectory;

      const touched = yield* directory.touchLastSeen(ThreadId.make("thread-touch-absent"));
      assert.equal(touched, false);
    }));
