...
const runAntigravityVersionCommand = (
  ...
  Effect.gen(function* () {
    const command = antigravitySettings.binaryPath || "antigravity-acp-server";
...
export const checkAntigravityProviderStatus = Effect.fn("checkAntigravityProviderStatus")(
  ...
  > {
    ...
    if (Result.isFailure(versionResult)) {
      ...
      yield* Effect.logWarning("Antigravity ACP health check failed.", {
        errorTag: error._tag,
      });
      return buildServerProvider({
        presentation: ANTIGRAVITY_PRESENTATION,
        ...
        probe: {
          ...
          message: isCommandMissingCause(error)
            ? `Antigravity ACP server binary (\`${antigravitySettings.binaryPath || "antigravity-acp-server"}\`) is not installed or not on PATH.`
            : "Failed to execute Antigravity ACP health check.",
        },
      });
    }
    ...
    if (Option.isNone(versionResult.success)) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "error",
          auth: { status: "unknown" },
          message: "Antigravity ACP server is installed but timed out while running `--version`.",
        },
      });
    }
    ...
    if (versionOutput.code !== 0) {
      yield* Effect.logWarning(
        "Antigravity ACP server version probe exited with a non-zero status.",
        {
      ...
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "error",
          auth: { status: "unknown" },
          message: "Antigravity ACP server is installed but failed to run.",
        },
      });
    }
