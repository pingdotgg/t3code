...
interface AntigravitySessionContext {
  ...
  notificationFiber: Fiber.Fiber<void, unknown> | undefined;
...
const resolveNotificationTurnId = (ctx: AntigravitySessionContext): TurnId | undefined =>
  ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: AntigravitySessionContext): TurnId | undefined =>
  ctx.activeTurnId;
...
export function antigravityPromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = antigravityPromptSettlementBelongsToContext({
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native Antigravity notification log.", {
            cause,
            threadId,
            method,
          }),
    ...
    const startSession: AntigravityAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const requestedStartModelId = antigravityModelSelection?.model
            ? resolveAntigravityAcpBaseModelId(antigravityModelSelection.model)
            : undefined;
          const boundModelId = yield* applyAntigravityAcpModelSelection({
            runtime: acp,
          ...
          const session: ProviderSession = {
            ...
            runtimeMode: input.runtimeMode,
            cwd,
            ...(boundModelId ? { model: resolveAntigravityAcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          ctx.currentModelId = boundModelId;
            ...
            return yield* Effect.gen(function* () {
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Antigravity prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "Antigravity prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "Antigravity session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Antigravity session changed before the turn completed.",
                });
              }
              for (let yieldAttempt = 0; yieldAttempt < 8; yieldAttempt += 1) {
                yield* Effect.yieldNow;
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "Antigravity prompt request failed.",
                }),
              );
    ...
    const respondToUserInput: AntigravityAdapterShape["respondToUserInput"] = (
      ...
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "session/respond_to_user_input",
          detail: `Antigravity ACP sessions do not support interactive user questions: ${requestId}`,
        });
      });
    ...
    const rollbackThread: AntigravityAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "Antigravity ACP sessions do not support provider-side rollback yet.",
        });
      });
