...
function ompReasoningOptionsFromModel(model: EffectAcpSchema.ModelInfo): {
  ...
  for (const entry of advertisedOptions) {
    ...
    const value =
      rawValue && isValidOmpReasoningEffortToken(rawValue)
        ? rawValue
        : rawId && isValidOmpReasoningEffortToken(rawId)
          ? rawId
          : undefined;
...
export function buildOmpModelCapabilities(model: EffectAcpSchema.ModelInfo): ModelCapabilities {
  const reasoning = ompReasoningOptionsFromModel(model);
    ...
    return buildServerProvider({
      ...
      probe: {
        ...
        message: isCommandMissingCause(error)
          ? "OMP CLI (`omp`) is not installed or not on PATH. Install it or set the binary path in settings."
