/**
 * PiAdapterLive — Pi CLI (`pi acp`) via ACP.
 *
...
export interface PiAdapterLiveOptions {
   ...
   * Defaults to the legacy built-in instance id (`pi`).
   */
   ...
   * the `piSettings` captured at construction.
   *
   ...
   */
  readonly resolveSettings?: Effect.Effect<PiSettings>;
}
...
export function makePiAdapter(piSettings: PiSettings, options?: PiAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.pi.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const emitPlanUpdate = (
      ...
      rawPayload: unknown,
      source: "acp.jsonrpc" | "acp.pi.extension",
      method: string,
    ) =>
      Effect.gen(function* () {
    ...
    const startSession: PiAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          // Resolve the PiSettings used to spawn the ACP child. Production
          ...
          // `updateSettings({ providers: { pi: { binaryPath } } })` calls
          // actually take effect when the next session spawns.
          const effectivePiSettings = options?.resolveSettings
            ? yield* options.resolveSettings
            : piSettings;

          const acp = yield* makePiAcpRuntime({
            piSettings: effectivePiSettings,
    ...
    const sendTurn: PiAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        yield* offerRuntimeEvent({
          ...
          threadId: input.threadId,
          turnId,
          payload: { model: ctx.session.model ?? "pi-default" },
        });
