...
// ACP does not expose Devin's private `streaming_reasoning` phase. Once it has
...
const DEFAULT_DEVIN_TURN_INACTIVITY_TIMEOUT_MS = 10 * 60 * 1_000;
...
const DEFAULT_DEVIN_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS = 30 * 60 * 1_000;
...
interface DevinTurnLivenessSignal {
...
interface DevinSessionContext {
  ...
  readonly livenessSignals: Queue.Queue<DevinTurnLivenessSignal>;
...
function clearProposedPlanFallback(ctx: DevinSessionContext): void {
  ctx.lastKnownProposedPlanMarkdown = undefined;
  ctx.lastKnownProposedPlanTurnId = undefined;
  ctx.planModeActive = false;
}

/** Detect Devin's enter_plan_mode tool call from ACP tool state. */
export function isDevinEnterPlanModeToolCall(toolCall: {
...
export function nextDevinPlanModeActive(
  ...
  if (!isDevinEnterPlanModeToolCall(toolCall)) {
...
export function selectDevinPermissionOptionId(
  ...
  // Devin 4.6 often omits allow_always. T3 still offers "Always allow this session".
  ...
  return Effect.gen(function* () {
    ...
    const turnInactivityTimeoutMs =
      ...
      Number.isFinite(requestedTurnInactivityTimeoutMs)
        ...
        : DEFAULT_DEVIN_TURN_INACTIVITY_TIMEOUT_MS;
    ...
    const activeToolInactivityTimeoutMs =
      ...
      Number.isFinite(requestedActiveToolInactivityTimeoutMs)
        ...
        : DEFAULT_DEVIN_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS;
    ...
    const signalTurnLiveness = (ctx: DevinSessionContext, turnId: TurnId) =>
      Queue.offer(ctx.livenessSignals, { turnId }).pipe(Effect.asVoid);

    const beginTurnLiveness = (ctx: DevinSessionContext, turnId: TurnId) =>
      Effect.sync(() => {
        ctx.livenessTurnId = turnId;
        // Do not start a deadline until ACP has made observable progress.
        // Devin's private reasoning phase is not present in the ACP stream.
        ctx.lastTurnActivityAtNanos = undefined;
        ctx.activeToolCallIds.clear();
      });

    const clearTurnLiveness = (ctx: DevinSessionContext) => {
    ...
    const recordTurnActivity = Effect.fn("DevinAdapter.recordTurnActivity")(function* (
      ctx: DevinSessionContext,
      turnId: TurnId,
      event: Extract<
    ...
    const hasLivenessPause = (ctx: DevinSessionContext) =>
      ctx.pendingApprovals.size > 0 ||
      ctx.pendingUserInputs.size > 0 ||
      ctx.livenessUpdatesInFlight > 0;

    const livenessTimeoutFor = (ctx: DevinSessionContext) =>
    ...
    const resumeSessionTurnLiveness = Effect.fn("DevinAdapter.resumeSessionTurnLiveness")(
    ...
    const refreshSessionTurnLiveness = Effect.fn("DevinAdapter.refreshSessionTurnLiveness")(
      function* (threadId: ThreadId, turnId: TurnId | undefined) {
        const ctx = sessions.get(threadId);
        if (
          !ctx ||
    ...
    const markPromptResponseReady = Effect.fn("DevinAdapter.markPromptResponseReady")(function* (
      threadId: ThreadId,
      acpSessionId: string,
      turnId: TurnId,
    ) {
    ...
    const consumePromptResponseReady = (ctx: DevinSessionContext) => {
    ...
    const isLiveTurn = (ctx: DevinSessionContext, turnId: TurnId) =>
    ...
    const settleStalledTurn = Effect.fn("DevinAdapter.settleStalledTurn")(function* (
      ctx: DevinSessionContext,
      turnId: TurnId,
    ) {
      return yield* withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          yield* settlePromptInFlight(ctx.threadId, turnId, ctx.acpSessionId, {
            errorMessage: `Devin ACP turn stalled without content or tool progress for ${livenessTimeoutFor(ctx).milliseconds}ms.`,
    ...
    const runTurnLivenessWatchdog = Effect.fn("DevinAdapter.runTurnLivenessWatchdog")(
      function* (ctx: DevinSessionContext) {
        while (true) {
          if (ctx.stopped) {
            return;
          }
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
                  ...
                  if (input.runtimeMode === "full-access" || alreadyApproved) {
                    const autoApprovedOptionId =
                      input.runtimeMode === "full-access"
                        ...
                        : selectDevinPermissionOptionId(params, "accept");
          ...
          const requestedStartModelId = devinModelSelection?.model
            ...
            : undefined;
          const currentStartModelId = currentDevinModelIdFromSessionSetup(
            started.sessionSetupResult,
          );
          const currentStartReasoningEffort = currentDevinReasoningEffortFromSessionSetup(
            started.sessionSetupResult,
          );
          ...
          const ctx: DevinSessionContext = {
            ...
            livenessSignals: yield* Queue.sliding<DevinTurnLivenessSignal>(1),
            ...
            currentReasoningEffort:
              requestedStartReasoningEffort !== undefined
                ? normalizeDevinReasoningEffort(requestedStartReasoningEffort)
              ...
              Effect.gen(function* () {
                ...
                switch (event._tag) {
                  ...
                  case "ToolCallUpdated": {
                    ...
                    ctx.planModeActive = nextDevinPlanModeActive(
                      ctx.planModeActive,
                      event.toolCall,
                    );
                    return;
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              if (requestedTurnReasoningEffort !== undefined) {
                ctx.currentReasoningEffort = normalizeDevinReasoningEffort(
