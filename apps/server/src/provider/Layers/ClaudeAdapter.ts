...
const buildUserMessageEffect = Effect.fn("buildUserMessageEffect")(function* (
  ...
  // text; image blocks ahead of it ride along and survive the expansion.
  // Leading with the text instead made every turn that carried an image fall
  // back to a plain prompt, so a typed `/skill args` reached the agent as
  // literal text with no command expansion and no error.
  if (dispatch) {
