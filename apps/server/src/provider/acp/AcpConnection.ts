...
import {
  trackChildProcess,
  untrackChildProcess,
} from "../../acpRegistry/childProcessRegistry.ts";
...
export class AcpConnection extends Context.Service<AcpConnection, AcpConnectionShape>()(
  "t3/provider/acp/AcpConnection",
) {
  static layer(
    options: AcpConnectionOptions,
  ): Layer.Layer<AcpConnection, EffectAcpErrors.AcpError, ChildProcessSpawner.ChildProcessSpawner> {
    return Layer.effect(AcpConnection, makeAcpConnection(options));
  }
}
...
> =>
  Effect.gen(function* () {
    ...
    const sessionNotFoundError = (sessionId: string) =>
      new EffectAcpErrors.AcpRequestError({
        code: -32602,
        errorMessage: `No handlers registered for session "${sessionId}"`,
        data: { sessionId },
      });
    ...
    yield* acp.handleRequestPermission((request) =>
      Effect.gen(function* () {
        ...
        if (!handlers?.onRequestPermission) {
          return yield* Effect.fail(sessionNotFoundError(request.sessionId));
        }
    ...
    return {
      ...
      prompt: (payload) =>
        runLoggedRequest("session/prompt", payload, acp.agent.prompt(payload)),
