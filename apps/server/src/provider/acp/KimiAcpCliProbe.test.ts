 ...
 * Enable with: T3_KIMI_ACP_PROBE=1 bun run test KimiAcpCliProbe
 *
 * The probe assumes either `KIMI_API_KEY` is set in the environment or
 * the user has previously run `kimi login`. Without credentials the
...
describe.runIf(process.env.T3_KIMI_ACP_PROBE === "1")("Kimi ACP CLI probe", () => {
  it.effect("initialize and authenticate against real kimi acp", () =>
    Effect.gen(function* () {
  ...
  it.effect("session/new advertises typed SessionModelState with at least one model", () =>
    Effect.gen(function* () {
      ...
      // Kimi advertises models through the typed `SessionModelState` field.
  ...
  it.effect("session/set_model accepts a no-op switch to the current model", () =>
    Effect.gen(function* () {
      ...
      // succeed against every Kimi build that implements `session/set_model`.
