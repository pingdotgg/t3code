...
describe("XAiAcpExtension", () => {
  ...
  it.effect.each([
    {
      name: "an xAI rate-limit completion (#8358)",
      notification: { stopReason: "rate_limit" },
      expected: { code: -32003, errorMessage: "Grok usage limit reached. Try again later." },
    },
    {
      name: "an xAI error completion with the agent's message",
      notification: { stopReason: "error", agentResult: { message: "Model overloaded" } },
      expected: { code: -32603, errorMessage: "Model overloaded" },
    },
  ])("fails a hung prompt from $name", ({ notification, expected }) =>
    Effect.gen(function* () {
      const handlers = new Map<string, (notification: unknown) => Effect.Effect<void>>();
      const hungPrompt = yield* Deferred.make<never>();
      const baseRuntime = {
      ...
      const promptCompleteHandler = handlers.get("x.ai/session/prompt_complete");
      expect(promptCompleteHandler).toBeDefined();
      yield* promptCompleteHandler!({ sessionId: "root-session", ...notification });
      const error = yield* Fiber.join(promptFiber).pipe(Effect.flip);
      expect(error).toMatchObject({ _tag: "AcpRequestError", ...expected });
    }),
  );
