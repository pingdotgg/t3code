...
export function resolveOmpAcpBaseModelId(model: string | null | undefined): string {
  ...
  // current model through ACP session state, so callers always pass a
  // concrete id here and empty input normalizes to "no model".
  return trimmed ? (normalizeModelSlug(trimmed, OMP_DRIVER_KIND) ?? trimmed) : "";
}
