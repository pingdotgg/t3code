...
describe("AcpRuntimeModel", () => {
  ...
  it("ignores malformed initialize model and mode state in synthetic load responses", () => {
    const response = syntheticLoadSessionResponseFromInitialize({
      protocolVersion: 1,
      _meta: {
        modelState: {
...
describe("extractMcpToolCallIdentity", () => {
  ...
  it("recovers T3 identity from Gemini and qwen MCP-server title templates", () => {
    ...
    const gemini = toolCallFromUpdate({
      ...
      toolCallId: "gemini-1",
      kind: "other",
      title: "link_pull_request (t3-code MCP Server)",
      status: "in_progress",
    });
    const qwen = toolCallFromUpdate({
      ...
      toolCallId: "qwen-1",
      kind: "other",
      title: 'list_thread_pull_requests (t3-code MCP Server): {"taskId":"node:delegated-task:1"}',
      status: "pending",
    ...
    expect(extractMcpToolCallIdentity(qwen)).toEqual({
      server: "t3-code",
  ...
  it("recovers T3 identity across the registry agents' naming conventions", () => {
    ...
    for (const title of [
      "t3-code___link_pull_request",
      "t3-code-link_pull_request",
      'mcp__t3_code__link_pull_request: {"mode":"async"}',
      't3-code__link_pull_request: {"mode":"async"}',
      "link_pull_request_t3-code",
      "t3-code/link_pull_request",
      'link_pull_request: {"mode":"async"}',
    ]) {
      const toolCall = toolCallFromUpdate({
        sessionUpdate: "tool_call",
  ...
  it("recovers T3 identity from goose _meta despite LLM-rewritten titles", () => {
    ...
    const toolCall = toolCallFromUpdate({
      ...
      _meta: {
        goose: {
          toolCall: { toolName: "t3-code__list_thread_pull_requests", extensionName: "t3-code" },
          messageId: "message-1",
        },
      },
    });
  ...
  it("recovers T3 identity from qwen serverId meta regardless of prefix format", () => {
    ...
    const toolCall = toolCallFromUpdate({
      ...
      status: "pending",
      _meta: { toolName: "mcp::t3-code::preview_snapshot", serverId: "t3-code", provenance: "mcp" },
    });
