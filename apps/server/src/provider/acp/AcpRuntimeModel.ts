...
/** Session title and update time from ACP `session_info_update`. `null` clears the value. */
interface AcpSessionInfoMetadata {
  readonly title?: string | null;
  readonly updatedAt?: string | null;
}

/** Tools of T3's MCP server (`apps/server/src/mcp/toolkits`). Gates name-based MCP matches. */
const T3_MCP_TOOL_NAMES: ReadonlySet<string> = new Set([
  "link_pull_request",
  "unlink_pull_request",
...
export type AcpParsedSessionEvent =
  ...
  | {
      ...
      readonly metadata: AcpSessionInfoMetadata;
      readonly rawPayload: unknown;
    }
  | {
