...
export interface AcpMultiSessionUserHandlers {
  readonly onRequestPermission?: AcpConnectionSessionHandlers["onRequestPermission"];
  readonly onElicitation?: AcpConnectionSessionHandlers["onElicitation"];
  readonly onReadTextFile?: AcpConnectionSessionHandlers["onReadTextFile"];
  readonly onWriteTextFile?: AcpConnectionSessionHandlers["onWriteTextFile"];
}
...
export const makeAcpMultiSession = (
  ...
  Effect.gen(function* () {
    ...
    const onSessionUpdate: AcpConnectionSessionHandlers["onSessionUpdate"] = (notification) =>
      Effect.gen(function* () {
    ...
    yield* Ref.set(
      configOptionsRef,
      session.sessionSetupResult.configOptions ?? [],
    );
    ...
    > =>
      validateConfigOptionValue(configId, value).pipe(
        ...
        Effect.flatMap((configOptions) => {
          ...
          return options.connection.setSessionConfigOption(payload).pipe(
            Effect.tap((response) =>
