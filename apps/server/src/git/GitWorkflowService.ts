...
export const make = Effect.gen(function* () {
  ...
  const resolveFreshWorktreeBase = Effect.fn("GitWorkflowService.resolveFreshWorktreeBase")(
    function* (input: VcsCreateWorktreeInput) {
      ...
      return { ...input, refName: remoteBase.commitSha };
    },
  );
  ...
  return GitWorkflowService.of({
    ...
    createWorktree: (input) =>
      ensureGitCommand("GitWorkflowService.createWorktree", input.cwd).pipe(
        // A remote-based worktree must be based on a fetch performed for this
        // exact creation. Resolve the refreshed ref to a commit before adding
        // the worktree so another fetch cannot move the start point between
        // validation and checkout. Fetch failures abort creation rather than
        // silently putting a new thread on stale code.
        Effect.andThen(resolveFreshWorktreeBase(input)),
        Effect.flatMap(git.createWorktree),
      ),
