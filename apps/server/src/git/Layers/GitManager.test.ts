...
import { type GitRunStackedActionInput } from "@t3tools/contracts";
...
interface FakeGitTextGeneration {
  generateCommitMessage: (input: CommitMessageGenerationInput) => Effect.Effect<
    { subject: string; body: string; branch?: string | undefined },
    TextGenerationError
  >;
  generatePrContent: (input: PrContentGenerationInput) => Effect.Effect<
...
interface FakeCopilotGitTextGeneration {
  generateCommitMessage: FakeGitTextGeneration["generateCommitMessage"];
  generatePrContent: FakeGitTextGeneration["generatePrContent"];
}
...
function createCopilotTextGeneration(
  overrides: Partial<FakeCopilotGitTextGeneration> = {},
): CopilotTextGenerationShape {
  const implementation: FakeCopilotGitTextGeneration = {
    generateCommitMessage: (input) =>
      Effect.succeed({
        subject: `Copilot: ${input.model ?? "default"}`,
        body: "",
        ...(input.includeBranch ? { branch: "feature/copilot-default" } : {}),
      }),
    generatePrContent: (input) =>
      Effect.succeed({
        title: `Copilot PR: ${input.model ?? input.headBranch}`,
      ...
      implementation.generateCommitMessage(input).pipe(
        Effect.mapError(
          (cause) =>
            new TextGenerationError({
              operation: "generateCommitMessage",
              detail: "fake copilot text generation failed",
              ...(cause !== undefined ? { cause } : {}),
            }),
        ),
      ),
...
function makeManager(input?: {
  ...
  copilotTextGeneration?: Partial<FakeCopilotGitTextGeneration>;
}) {
  const { service: gitHubCli, ghCalls } = createGitHubCliWithFakeGh(input?.ghScenario);
  const textGeneration = createTextGeneration(input?.textGeneration);
  const copilotTextGeneration = createCopilotTextGeneration(input?.copilotTextGeneration);
  ...
  const managerLayer = Layer.mergeAll(
    ...
    Layer.succeed(CopilotTextGeneration, copilotTextGeneration),
    gitCoreLayer,
    NodeServices.layer,
  );
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  ...
  it.effect("uses Copilot commit generation with the selected model when requested", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      fs.writeFileSync(path.join(repoDir, "README.md"), "hello\ncopilot\n");
      let codexCommitCalls = 0;
      let copilotCommitInput:
        | Parameters<FakeCopilotGitTextGeneration["generateCommitMessage"]>[0]
        | undefined;
      ...
      const { manager } = yield* makeManager({
        textGeneration: {
          generateCommitMessage: (input) =>
            Effect.sync(() => {
              codexCommitCalls += 1;
              return {
                subject: `Codex should not run ${input.model ?? "unknown"}`,
                body: "",
              };
            }),
        },
        copilotTextGeneration: {
          generateCommitMessage: (input) =>
            Effect.sync(() => {
              copilotCommitInput = input;
              return {
                subject: "Copilot commit title",
                body: "- generated with the selected Copilot model",
              };
            }),
        },
      });
      ...
      const result = yield* runStackedAction(manager, {
        ...
        providerOptions: { copilot: { cliPath: "/tmp/copilot", configDir: "/tmp/copilot-config" } },
      });

      expect(result.commit.status).toBe("created");
      expect(result.commit.subject).toBe("Copilot commit title");
      expect(codexCommitCalls).toBe(0);
      expect(copilotCommitInput?.provider).toBe("copilot");
      expect(copilotCommitInput?.model).toBe("gpt-5.4");
      expect(copilotCommitInput?.modelOptions).toEqual({
        copilot: { reasoningEffort: "high" },
      });
      expect(copilotCommitInput?.providerOptions).toEqual({
        copilot: { cliPath: "/tmp/copilot", configDir: "/tmp/copilot-config" },
      });
    }),
  );
  ...
  it.effect("uses Copilot PR generation with the selected model when requested", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      ...
      const remoteDir = yield* createBareRemote();
      yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
      fs.writeFileSync(path.join(repoDir, "changes.txt"), "copilot pr\n");
      yield* runGit(repoDir, ["add", "changes.txt"]);
      yield* runGit(repoDir, ["commit", "-m", "Feature commit"]);
      yield* runGit(repoDir, ["push", "-u", "origin", "feature-copilot-pr"]);
      yield* runGit(repoDir, ["config", "branch.feature-copilot-pr.gh-merge-base", "main"]);
      let codexPrCalls = 0;
      let copilotPrInput:
        | Parameters<FakeCopilotGitTextGeneration["generatePrContent"]>[0]
        | undefined;
      ...
      expect(result.pr.status).toBe("created");
      expect(codexPrCalls).toBe(0);
      expect(copilotPrInput?.provider).toBe("copilot");
      expect(copilotPrInput?.model).toBe("claude-sonnet-4.6");
      expect(
        ghCalls.some((call) =>
          call.includes("pr create --base main --head feature-copilot-pr --title Copilot PR title"),
        ),
      ).toBe(true);
    }),
  );
