...
export const make = Effect.gen(function* () {
  ...
  // the epoch (invalidateStatus) to bypass the cache immediately. Epochs are
  // keyed by normalized cwd so a refresh invalidates every project-scoped
  // entry for that repository at once.
  ...
  const bumpPrLookupEpoch = (cwd: string) =>
    normalizeStatusCwd(cwd).pipe(
      Effect.map((normalizedCwd) => {
        prLookupEpochByCwd.set(normalizedCwd, prLookupEpoch(normalizedCwd) + 1);
      }),
    );
  // Cache keys are NUL-joined [cwd, projectId, branch, upstreamRef, epoch] —
  // none of the segments can contain a NUL byte, and refs are never empty, so
  // "" decodes back to a null upstreamRef (and "" to an absent projectId).
  // The projectId segment keeps lookups distinct per project because a
  // project-level remote override changes which provider answers the lookup.
  const prLookupCacheKey = (
    ...
    [
      ...
      details.branch,
      details.upstreamRef ?? "",
      String(prLookupEpoch(input.cwd)),
    ].join(STATUS_CACHE_KEY_SEPARATOR);
  const prLookupCache = yield* Cache.makeWith(
    (key: string) => {
      const [cwd = "", projectId = "", branch = "", upstreamRef = ""] = key.split(
        STATUS_CACHE_KEY_SEPARATOR,
      );
      const input: VcsStatusInput = projectId
        ? { cwd, projectId: projectId as VcsStatusInput["projectId"] }
        : { cwd };
      const details = {
        branch,
      ...
      return resolveBranchHeadContext(cwd, details).pipe(
        Effect.flatMap((headContext) =>
          findLatestPrForHeadContext(input, headContext).pipe(
  ...
  const lookupStatusPr = Effect.fn("lookupStatusPr")(function* (
    ...
    const cwd = input.cwd;
    // Keyed by (cwd, projectId, branch) only: the upstream ref changing (e.g.
    // a first `push -u`) must not orphan the fallback value for the same
    // branch. The projectId keeps fallbacks apart when project remote
    // overrides point the same repository at different providers.
    const branchKey = [cwd, input.projectId ?? "", details.branch].join(STATUS_CACHE_KEY_SEPARATOR);
    return yield* Cache.get(prLookupCache, prLookupCacheKey(input, details)).pipe(
        ...
        const pr = wantsPr
          ? yield* progress
              ...
              .pipe(
                ...
                Effect.flatMap(() =>
                  runPrStep(textGenerationSettings, input, currentBranch, progress.emit),
                ),
              )
          : { status: "skipped_not_requested" as const };
