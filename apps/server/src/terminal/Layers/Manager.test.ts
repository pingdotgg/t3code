...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "forma-terminal-" });
...
it.layer(NodeServices.layer, { excludeTestServices: true })("TerminalManager", (it) => {
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(spawnInput.env.FORMA_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      assert.equal(spawnInput.env.FORMA_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.FORMA_WORKTREE_PATH, "/repo/worktree-a");
