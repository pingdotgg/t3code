...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const rebaseOntoUpstream = Effect.fn("rebaseOntoUpstream")(function* (cwd: string) {
    ...
    if (result.exitCode !== 0) {
      yield* executeGit("GitVcsDriver.syncCurrentBranch.rebaseAbort", cwd, ["rebase", "--abort"], {
        allowNonZeroExit: true,
      });
      return yield* new GitCommandError({
        ...gitCommandContext({
          operation: "GitVcsDriver.syncCurrentBranch.rebase",
          cwd,
          args: ["pull", "--rebase"],
        }),
        detail:
  ...
  const syncCurrentBranch: GitVcsDriver.GitVcsDriver["Service"]["syncCurrentBranch"] = Effect.fn(
    "syncCurrentBranch",
    ...
    if (!refName) {
      return yield* new GitCommandError({
        ...
        detail: "Cannot sync from detached HEAD.",
      });
    }
