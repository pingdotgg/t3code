  ...
  withGeminiHarness(
    (harness) =>
      Effect.gen(function* () {
        ...
        const projectId = asProjectId("project-gemini-integ");
        const threadId = ThreadId.makeUnsafe("thread-gemini-integ");
...
it.live("handles Gemini mid-turn user approval requests end-to-end", () =>
  withGeminiHarness(
    (harness) =>
      Effect.gen(function* () {
        const projectId = asProjectId("project-gemini-appr-integ");
        const threadId = ThreadId.makeUnsafe("thread-gemini-appr-integ");
        const approvalRequestId = asApprovalRequestId("req-gemini-approval-integ");
        const createdAt = nowIso();
        ...
        const approvalTurnResponse: TestTurnResponse = {
          events: [
            ...
            },
            {
              ...
              threadId,
              turnId: "turn-1",
              requestId: String(approvalRequestId),
              requestKind: "command",
              detail: "ls -la",
            },
            {
