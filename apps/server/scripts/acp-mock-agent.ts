...
let currentModelId = antigravityProfile ? "gemini-test-low" : grokProfile ? "grok-4.6" : "default";
...
function configOptions(): ReadonlyArray<AcpSchema.SessionConfigOption> {
  ...
  return [
    ...
    },
    {
      ...
      currentValue: currentModelId,
      options: grokProfile
        ? grokAcpModels.map((model) => ({ value: model.modelId, name: model.name }))
        : [
            { value: "default", name: "Auto" },
            { value: "composer-2", name: "Composer 2" },
...
/** cursor-agent speaks ACP v1, so its model list names option ids `id`. */
function availableModels() {
  return [
    ...
    value: model.value,
    name: model.name,
    configOptions: modelConfigOptionsFor(model.value).map(({ configId, ...option }) => ({
      id: configId,
      ...option,
    })),
  }));
