...
type SyncedPlanModeHydrationInput<E> = SyncedClientPreferenceHydrationInput<"planModeEnabled", E>;
const createSyncedPlanModeHydrationController = (
  scheduleRetry?: (retry: () => void, delayMs: number) => () => void,
) => createSyncedClientPreferenceHydrationController("planModeEnabled", scheduleRetry);
...
const createSyncedPlanModeWrite = (
  input: Omit<Parameters<typeof createSyncedClientPreferenceWrite<"planModeEnabled">>[0], "field">,
) => createSyncedClientPreferenceWrite({ field: "planModeEnabled", ...input });
const resolveSyncedPlanModeHydrationAction = (
  input: Omit<
    Parameters<typeof resolveSyncedClientPreferenceHydrationAction<"planModeEnabled">>[0],
    "field"
  >,
) => resolveSyncedClientPreferenceHydrationAction({ field: "planModeEnabled", ...input });
...
describe("synced plan mode", () => {
  ...
  it("writes one stamped value to the local and environment stores", () => {
    expect(
      ...
      request: {
        commandId: planModeCommandId(false, "2026-08-14T12:01:00.000Z"),
  ...
  it("uploads an offline write when patch access becomes available", async () => {
    ...
    expect(patch).toHaveBeenCalledWith({
      environmentId: primaryEnvironmentId,
      input: {
        commandId: planModeCommandId(true, "2026-08-14T12:01:00.000Z"),
        patch: { planModeEnabled: true },
        updatedAt: "2026-08-14T12:01:00.000Z",
      },
    });
