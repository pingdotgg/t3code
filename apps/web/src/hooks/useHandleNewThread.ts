    ...
    (
      ...
      const seedNewDraftModelState = (draftId: Parameters<typeof setModelSelection>[0]) => {
        if (projectDefaultModelSelection) {
          setModelSelection(draftId, projectDefaultModelSelection, { replaceOptions: true });
        } else {
          ...
          if (
            ...
            setModelSelection(draftId, createDefaultModelSelection(), { replaceOptions: true });
          } else {
            applyStickyState(draftId);
          }
        }
        if (carryModelSelection) {
          // The viewed thread's exact selection (model + options like effort
          // and context window) wins over the project default or the sticky
          // one. replaceOptions: the carried selection is a complete
          // snapshot — absent options mean "no options", not "keep whatever
          // was just seeded".
