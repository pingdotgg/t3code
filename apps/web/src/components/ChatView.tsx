...
function ChatViewContent(props: ChatViewProps) {
  ...
  useEffect(() => {
    const currentIds = new Set(
      threadShells.map((thread) => `${thread.environmentId}:${thread.id}`),
    );
    if (!embedded && activeThread && activeThreadRef) {
      for (const thread of threadShells) {
        const key = `${thread.environmentId}:${thread.id}`;
        if (
          !knownThreadIdsRef.current.has(key) &&
          thread.environmentId === activeThread.environmentId &&
          thread.projectId === activeThread.projectId &&
          thread.forkedFrom?.threadId === activeThread.id &&
          Date.parse(thread.createdAt) >= mountedAtRef.current - 2_000
        ) {
          useRightPanelStore.getState().openThread(activeThreadRef, thread.id, thread.title);
        }
      }
    }
    knownThreadIdsRef.current = currentIds;
  }, [activeThread, activeThreadRef, embedded, threadShells]);
  ...
  const onForkToSide = useCallback(async () => {
      ...
      if (!isAtomCommandInterrupted(result)) {
        ...
        toastManager.add(
          stackedThreadToast({
            ...
            description: error instanceof Error ? error.message : "The thread could not be forked.",
          }),
        );
      }
      return;
        ...
        {!embedded ? (
          ...
          >
            ...
            <ChatHeader
              ...
              {...(isServerThread ? { onForkToSide, forkToSideDisabled: isForkingToSide } : {})}
            />
          </header>
        ) : null}
