...
function ChatViewContent(props: ChatViewProps) {
  ...
  const seenPendingUserInputRequestIdsRef = useRef<{
    routeThreadKey: string;
    requestIds: ReadonlySet<string>;
  }>({ routeThreadKey, requestIds: EMPTY_REQUEST_ID_SET });
  useEffect(() => {
    const reconciliation = reconcilePendingUserInputRequestSnapshot({
      previous: seenPendingUserInputRequestIdsRef.current,
      routeThreadKey,
      currentRequestIds: pendingUserInputs.map((entry) => String(entry.requestId)),
    });
    for (const requestId of reconciliation.removedRequestIds) {
      clearPendingUserInputDraft(requestId);
    }
    seenPendingUserInputRequestIdsRef.current = reconciliation.next;
  }, [pendingUserInputs, routeThreadKey]);
