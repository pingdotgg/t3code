  ...
  const gitStatusQuery = useEnvironmentQuery(
    gitStatusCwd === null
      ...
      : vcsEnvironment.status({
          environmentId,
          input: { cwd: gitStatusCwd, ...(activeProject ? { projectId: activeProject.id } : {}) },
        }),
  );
          ...
          server: isElectron
            ? {
                upsertKeybinding: (rule) =>
                  throwOnAtomCommandFailure(upsertKeybinding({ environmentId, input: rule })),
                removeKeybinding: (target) =>
                  throwOnAtomCommandFailure(removeKeybinding({ environmentId, input: target })),
              }
            : null,
        }),
      );
