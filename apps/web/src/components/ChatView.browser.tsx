...
function createMockEnvironmentApi(input: {
  ...
  return {
    ...
    orchestration: {
      ...
      getTurnDiffState: (() => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["getTurnDiffState"],
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("renders all seven right-panel surface branches and preserves inactive surfaces", async () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(document.querySelectorAll(".thread-terminal-drawer")).toHaveLength(1);
      });
  ...
  it("cleans up browser and terminal resources when closing all surfaces", async () => {
    ...
    try {
      ...
      await page.getByLabelText("Actions for Files").click();
