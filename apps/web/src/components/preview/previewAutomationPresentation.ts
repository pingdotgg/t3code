...
export function revealPreviewAutomationTab(ref: ScopedThreadRef, tabId: string): void {
  ...
  usePreviewMiniPlayerStore.getState().open(ref, browserMiniPlayerSource(tabId));
}
...
function readPreviewAutomationPresentation(input: PreviewAutomationPresentationTarget) {
  ...
  return { panel, miniPlayerTabId, presentation };
}
...
export function readPreviewAutomationPresentationDiagnostics(
  ...
  const { panel, miniPlayerTabId, presentation } = readPreviewAutomationPresentation(input);
  ...
  return {
    ...
    activeSurfaceId:
      miniPlayerTabId !== null
        ? `mini-player:${miniPlayerTabId}`
        : panel.isOpen
          ? panel.activeSurfaceId
          : null,
    inlinePreviewOpen: miniPlayerTabId !== null,
    inlinePreviewTabId: miniPlayerTabId ?? null,
...
export async function waitForBrowserSurfaceVisibility(
  ...
  while (Date.now() <= deadline) {
    ...
    if (isPreviewAutomationTabPresented(input)) {
      ...
      const { panel, miniPlayerTabId } = readPreviewAutomationPresentation(input);
      const selectedInPanel = panel.isOpen && panel.activeSurfaceId === `browser:${input.tabId}`;
      if (!selectedInPanel && miniPlayerTabId !== input.tabId) {
