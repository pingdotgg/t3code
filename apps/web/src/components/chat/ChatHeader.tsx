...
export const ChatHeader = memo(function ChatHeader({
  ...
  const commitRename = useCallback(
    (title: string) => {
      ...
      if (resolution.action === "reject-empty") {
        toastManager.add({ type: "warning", title: t("chat.header.titleRequired") });
        return;
      }
      ...
      void updateThreadMetadata({
        ...
        if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
          ...
          toastManager.add({
            type: "error",
            title: t("chat.header.renameFailed"),
  ...
  return (
    ...
    >
      <WorkspaceBreadcrumb ariaLabel={t("chat.header.breadcrumb")} className="flex-1">
              ...
              <Tooltip>
                <TooltipTrigger
                  render={
                    <button
                      type="button"
                      aria-label={t("chat.header.newThreadIn", { project: activeProjectName })}
