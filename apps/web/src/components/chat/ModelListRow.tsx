    ...
    <Tooltip>
      <TooltipTrigger
        render={
          <button
            className={cn(
              "relative flex size-4 shrink-0 cursor-pointer items-center justify-center rounded-md text-muted-foreground/70 opacity-64 transition-[background-color,color,opacity] hover:bg-background/70 hover:text-foreground hover:opacity-100 group-hover:opacity-100 pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11",
  ...
  const row = (
    <div className="group relative w-full">
      <ComboboxItem
        ...
        contentClassName="flex w-full items-center gap-2.5 pr-6 sm:gap-3"
        className={cn(
          "relative w-full !min-w-0 max-w-full cursor-pointer rounded-md px-2 py-2 transition-[background-color,box-shadow,color] sm:py-2.5",
      ...
      >
        <div className="min-w-0 flex-1 text-left">
          <div className="flex min-w-0 items-center gap-2">
            <div className="min-w-0 truncate text-xs font-medium leading-snug max-sm:text-[15px] max-sm:leading-5">
      ...
      <div className="absolute right-2 top-1/2 z-10 flex -translate-y-1/2 items-center">
        {favoriteButton}
      </div>
    </div>
  );
