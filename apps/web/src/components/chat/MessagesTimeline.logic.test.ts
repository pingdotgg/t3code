  ...
  it.each(["completed", "interrupted", "running"] as const)(
    ...
    (state) => {
      ...
      const messages = [
        ...
        message("thought-first", "reasoning", 2),
        message("steer-1", "user", 10),
        message("steer-2", "user", 11),
        message("thought-middle", "reasoning", 11),
        message("middle", "assistant", 12),
        message("steer-3", "user", 20),
        message("thought-last", "reasoning", 20),
      ...
      if (state === "running") {
        ...
        expect(collapsed.filter((row) => row.kind === "message")).toHaveLength(
          messages.filter((message) => message.role !== "reasoning").length,
        );
      ...
      expect(
        expanded
          .filter((row) => row.kind === "activity-group")
          .flatMap((row) =>
            row.entries
              .filter((entry) => entry.kind === "message")
              .map((entry) => entry.message.id),
          ),
      ).toEqual(["thought-middle"]);
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps subagent spawn rows outside turn folds even after they settle", () => {
    ...
    expect(derive(direct, new Set(), new Set(["turn-fold:turn-1"]))).toEqual([
      "turn-fold:turn-1",
  ...
  it.each(["assistant", "user", "error", "turn"] as const)(
    ...
    (boundary) => {
      ...
      const rows = deriveMessagesTimelineRows({
        ...
        expandedFoldIds: new Set(["turn-fold:turn-1", "turn-fold:turn-2"]),
        isWorking: false,
        activeTurnStartedAt: null,
        turnDiffSummaries: [],
        supportsConversationRollback: false,
