...
function SyntaxHighlightedTokens(props: {
  ...
  const highlighter = use(getSyntaxHighlighterPromise(props.language, props.palette));
  const tokens = useMemo(() => {
    try {
      return highlighter.codeToTokens(props.line, {
        lang: props.language,
        theme: resolveDiffThemeName(props.theme, props.palette),
      }).tokens[0];
    } catch {
      return undefined;
    }
  }, [highlighter, props.language, props.line, props.palette, props.theme]);
