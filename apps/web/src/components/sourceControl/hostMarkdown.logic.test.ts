...
describe("host body segmentation", () => {
  it("keeps a plain body as a single markdown run", () => {
    expect(splitHostBody("## What changed\n\nSome prose.")).toEqual([
  ...
  it("names a bare link to a video file a video", () => {
    expect(splitHostBody("https://example.com/demo.mp4?raw=1")).toEqual([
      {
        id: "attachment:0",
        kind: "attachment",
  ...
  it("leaves a dropped image as markdown so it still renders as an image", () => {
    ...
    expect(splitHostBody(body)).toEqual([{ id: "markdown:0", kind: "markdown", text: body }]);
  });
  ...
  it("stays linear when the body is full of unclosed video tags", () => {
    ...
    expect(splitHostBody(body)).toHaveLength(1);
