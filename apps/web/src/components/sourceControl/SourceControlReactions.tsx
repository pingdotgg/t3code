...
} from "../pullRequest/pullRequestReactions.logic";
...
export function SourceControlReactionBar({
  ...
  readonly onToggle: (content: PullRequestReactionContent, reacted: boolean) => Promise<boolean>;
  ...
  const toggle = async (content: PullRequestReactionContent, reacted: boolean) => {
    ...
    const saved = await onToggle(content, reacted);
    if (!saved) {
      setPending((current) => {
