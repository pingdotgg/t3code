import type { PullRequestReviewThread } from "@t3tools/contracts";
import { CheckIcon, LinkIcon, MessageSquarePlusIcon } from "lucide-react";
import { createContext, useContext } from "react";

import { useCopyToClipboard } from "~/hooks/useCopyToClipboard";

import { Button } from "../ui/button";
import { toastManager } from "../ui/toast";
import { Tooltip, TooltipPopup, TooltipTrigger } from "../ui/tooltip";
import type { PullRequestCommentReference } from "./pullRequestDetail.logic";

export const PullRequestCommentActionsContext = createContext<{
  add: (comment: PullRequestCommentReference) => void;
  disabled: boolean;
  threads: ReadonlyMap<string, PullRequestReviewThread>;
  resolve?: (thread: PullRequestReviewThread) => void;
} | null>(null);

export function PullRequestCommentActions({
  comment,
  showResolution = false,
}: {
  comment: PullRequestCommentReference;
  showResolution?: boolean;
}) {
  const actions = useContext(PullRequestCommentActionsContext);
  const thread = showResolution ? actions?.threads.get(comment.id) : undefined;
  const { copyToClipboard, isCopied } = useCopyToClipboard({
    target: "comment link",
    onError: () => toastManager.add({ type: "error", title: "Could not copy the comment link" }),
  });
  return (
    <span className="inline-flex shrink-0 items-center gap-0.5">
      {thread && actions?.resolve ? (
        <Button
          size="xs"
          variant="ghost"
          disabled={actions.disabled}
          onClick={() => actions.resolve?.(thread)}
        >
          {thread.isResolved ? "Reopen" : "Resolve"}
        </Button>
      ) : null}
      {comment.url ? (
        <Tooltip>
          <TooltipTrigger
            render={
              <Button
                size="icon-xs"
                variant="ghost"
                aria-label={isCopied ? "Link copied" : "Copy comment link"}
                onClick={() => copyToClipboard(comment.url!)}
              >
                {isCopied ? <CheckIcon className="size-3" /> : <LinkIcon className="size-3" />}
              </Button>
            }
          />
          <TooltipPopup>{isCopied ? "Link copied" : "Copy comment link"}</TooltipPopup>
        </Tooltip>
      ) : null}
      {actions ? (
        <Tooltip>
          <TooltipTrigger
            render={
              <Button
                size="icon-xs"
                variant="ghost"
                aria-label="Add comment to composer"
                disabled={actions.disabled}
                onClick={() => actions.add(comment)}
              >
                <MessageSquarePlusIcon className="size-3" />
              </Button>
            }
          />
          <TooltipPopup>Add to composer</TooltipPopup>
        </Tooltip>
      ) : null}
    </span>
  );
}
