    ...
    <div className="w-72 bg-popover">
      <div className="flex items-center justify-between border-b border-border/70 px-4 py-3">
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-muted-foreground">{t("Choose a color")}</p>
        </div>
        <span
        ...
        <div className="grid grid-cols-[1fr_1.2fr] gap-2">
          <label className="grid min-w-0 gap-1">
            ...
            <span className="flex min-w-0 items-center gap-2 rounded-lg border border-input bg-background px-2 focus-within:border-ring">
              ...
              />
              <input
                aria-label={`${label} ${t("picker hex value")}`}
          ...
          <label className="grid min-w-0 gap-1">
            ...
            <span className="flex min-w-0 items-center rounded-lg border border-input bg-background px-2 focus-within:border-ring">
              <input
                aria-label={`${label} ${t("picker RGB value")}`}
                className="h-8 min-w-0 flex-1 bg-transparent font-mono text-xs text-foreground outline-none"
                onBlur={() => {
                  isEditingTextRef.current = false;
                  setHexDraft(currentColor);
  ...
  return (
    <Popover>
      <Tooltip>
        <TooltipTrigger
          render={
            <PopoverTrigger
              render={
                <button
                  aria-label={`${t("Choose")} ${label} ${t("color")}`}
        ...
        />
        <TooltipPopup side="top">{`${t("Choose")} ${label} ${t("color")}`}</TooltipPopup>
      </Tooltip>
      <PopoverPopup
        align="end"
    ...
    >
      <Tooltip>
        <TooltipTrigger
          render={
            <button
              aria-label={`${t(selected ? "Hide" : "Show")} ${label} ${t("usage")}`}
        ...
        <TooltipPopup side="top">
          {`${t(selected ? "Hide" : "Show")} ${t("where")} ${label} ${t("is used")}`}
        </TooltipPopup>
      </Tooltip>
