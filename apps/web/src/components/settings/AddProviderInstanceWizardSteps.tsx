...
export function AddProviderInstanceWizardSteps({
  ...
  const { t } = useI18n();
  const stepLabelKeys: readonly MessageKey[] = [
    "providers.driver",
    "providers.identity",
    "providers.config",
  ];
  return (
    ...
    >
      {ADD_PROVIDER_WIZARD_STEPS.map((step, index) => {
        const stepLabel = t(stepLabelKeys[index] ?? "providers.step", { number: index + 1 });
        return (
          <li key={step} className="min-w-0">
            <button
              ...
              aria-label={`${stepLabel}, ${t("providers.step", { number: index + 1 })}${index < currentStep && summaries[index] ? `, ${summaries[index]}` : ""}`}
              onClick={() =>
                onNavigation(
