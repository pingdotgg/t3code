...
export function LinearSettingsPanel() {
  ...
  const handleDisconnect = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const result = await clearToken({ environmentId, input: {} });
      if (result._tag !== "Success") {
        toastManager.add({
          type: "error",
          title: "Could not disconnect Linear",
          description: "The stored token could not be removed.",
        });
        return;
      }
      authQuery.refresh();
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="Linear" icon={<LinearIcon className="size-3.5" />}>
        <SettingsRow
          id={searchableSetting("linear-connection").id}
          title="Connection"
          description={
