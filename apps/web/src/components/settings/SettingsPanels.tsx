...
function BackgroundActivityAdvancedDialog({
  ...
  // Scoped settings resolve through SettingsScopeContext, which selects
  // this environment whenever the dialog can open. environmentId stays on
  // the props so environment-scoped call sites pass their target explicitly.
  void environmentId;
  const settings = useScopedSettings();
  const updateSettings = useUpdateScopedSettings();
...
function EnvironmentSettingsSections({
  ...
  const textGenerationProviders = serverConfig.providers.filter(
    (provider) => provider.supportsTextGeneration !== false,
  );
  ...
  return (
    <>
      <SettingsSection id="environment-workspace" title="Workspace">
        {supportsAutoSettlement ? (
          <>
            <SettingsRow
              serverScoped
      ...
      <SettingsSection id="environment-diagnostics" title="Diagnostics">
        <SettingsRow
