...
export function AddOrInstallProviderPanel({
  ...
  const [uninstallConfirmEntry, setUninstallConfirmEntry] = useState<AcpRegistryEntryWithStatus | null>(null);
  ...
  const refreshAcp = useCallback(async () => {
    try {
      const list = await ensureLocalApi().acpRegistry.list();
      setAcpEntries(list);
      setAcpError(null);
    } catch (cause) {
  ...
  const runAcpAction = useCallback(
    async (agentId: string, name: string, action: "install" | "uninstall") => {
      ...
      try {
        await ensureLocalApi().acpRegistry[action]({ agentId });
        toastManager.add(
          stackedThreadToast({
            type: "success",
  ...
  const confirmUninstall = useCallback(() => {
    ...
    void runAcpAction(uninstallConfirmEntry.entry.id, uninstallConfirmEntry.entry.name, "uninstall");
  ...
  return (
    ...
    >
      ...
      {uninstallConfirmEntry && (
        <AlertDialog open={!!uninstallConfirmEntry} onOpenChange={(open) => !open && setUninstallConfirmEntry(null)}>
          <AlertDialogPopup>
            <AlertDialogHeader>
              <AlertDialogTitle>Remove {uninstallConfirmEntry.entry.name}?</AlertDialogTitle>
              <AlertDialogDescription>
                This will remove the agent and delete all associated provider instances. This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
