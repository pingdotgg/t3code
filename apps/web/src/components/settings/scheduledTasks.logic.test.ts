...
describe("buildScheduledTaskUpdateInput", () => {
  ...
  it("emits the normalized schedule for a legacy sub-minute interval", () => {
    const legacy: ScheduledTask = {
      ...task,
      schedule: { type: "interval", everyMs: 30_000 },
    };
    const baseline = taskToDraft(legacy);
    // The draft is untouched — taskToDraft clamped the baseline to "1" — but
    // the stored 30s interval must still be rewritten to the displayed 60s.
    expect(buildPatch(baseline, legacy, baseline)).toMatchObject({
      schedule: { type: "interval", everyMs: 60_000 },
    });
  });

  it("does not overwrite a concurrent fix to a legacy sub-minute interval", () => {
    const legacy: ScheduledTask = {
      ...task,
      schedule: { type: "interval", everyMs: 30_000 },
    };
    ...
    // Another client already normalized the schedule — an untouched save
    // leaves their interval alone instead of forcing the editor's 60s.
    expect(buildPatch(baseline, live, baseline)).toBeNull();
  });
  ...
  it("sends only the workspace controls the user changed as a sparse patch", () => {
    ...
    // base ref sends just that key — the server merges it into the live
    // strategy inside the update transaction, so the concurrent toggle
  ...
  it("sends a sparse patch so fields the dialog cannot express merge server-side", () => {
    // A stored worktree strategy can carry fields the editor has no control
    // for (`branch`). Editing the base ref sends only that key; the server
    // merges it into the live strategy, which carries `branch` through.
