...
function WorktreeMetadata({
  ...
  return (
    <span className="inline-flex min-w-0 flex-1 items-center gap-x-1.5 text-2xs text-muted-foreground">
...
function WorktreeErrorStrip({ error }: { readonly error: string | null }) {
  ...
  return (
    <Alert variant="error" className="mx-3 sm:mx-4">
...
function WorktreeStateCell({ worktree, onPrune, pendingPath }: WorktreeRowProps) {
  if (pendingPath === worktree.path) {
    return (
      <span role="status" className="shrink-0 text-2xs text-muted-foreground">
        Removing...
      </span>
    );
  }
