    ...
    if (prepared && canManage) {
      void requestVoiceSettings(prepared).then(
        ...
        () => {
          if (active)
            setError(
              "Could not load voice settings. This environment may need a newer T3 Code version.",
            );
        },
      );
    }
      ...
      <SettingsRow
        ...
        control={
          <Select
            ...
            onValueChange={(value) => {
              const next = VOICE_ACTIVATION_MODES.find((mode) => mode === value);
              if (next !== undefined) setOverlay({ activation: next });
            }}
          >
            <SelectTrigger size="sm" className="w-full sm:w-44" aria-label="Voice activation">
      ...
      <SettingsRow
        ...
        control={
          <Switch
            ...
            aria-label="Use fast commands"
            onCheckedChange={(checked) => setFastCommands(Boolean(checked))}
          />
        }
      />
      {environment ? (
