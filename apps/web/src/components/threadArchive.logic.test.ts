...
describe("archive lifecycle guards", () => {
  ...
  it("re-checks server-projected settled membership before Archive all mutates", async () => {
    const threadKey = "newly-unsettled-thread";
    const stableThreadKey = "stable-settled-thread";
    const archiveAllSnapshot = [{ threadKey }, { threadKey: stableThreadKey }] as const;
    let liveSettledThreadKeys = new Set([threadKey, stableThreadKey]);
    expect(liveSettledThreadKeys.has(threadKey)).toBe(true);

    // Archive all captured the thread while settled, but it was manually
    // un-settled and reprojected while the flow waited. The live membership
    // check must skip it while allowing the stable sibling to continue.
    liveSettledThreadKeys = new Set([stableThreadKey]);
