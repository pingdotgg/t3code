...
interface MarkdownFileLinkProps {
  ...
  /** Resolves a bare display path (e.g. `ChatView.tsx`) to the indexed
      workspace path before a shared action runs on it. */
  resolveMenuPath?: ((filePath: string) => Promise<string | null>) | undefined;
  className?: string | undefined;
}
    ...
    async (position: { x: number; y: number }) => {
      ...
      const menuTarget: FileContextMenuTarget | undefined =
        fileMenu && menuPath
          ...
          : undefined;
      // The chip already renders "Open in <editor>", so the default-app Open
      // folds into the shared Open with submenu instead of a third open row.
      const sharedItems = menuTarget
        ? (fileMenu?.buildItems(menuTarget, { hasPrimaryOpenItem: onOpen !== undefined }) ?? [])
        : [];
      ...
      try {
        ...
        // "Open with" selections report the child id ("editor:<id>" or the
        // nested "open"), which is not a top-level item.
        const sharedClicked = sharedItems
          .flatMap((item) => [item, ...(item.children ?? [])])
          .find((item) => item.id === clicked);
        if (menuTarget && fileMenu && sharedClicked) {
          const filePath = (await resolveMenuPath?.(menuTarget.filePath)) ?? menuTarget.filePath;
          await fileMenu.activate(sharedClicked.id, { ...menuTarget, filePath });
          return;
        }
...
function areMarkdownFileLinkPropsEqual(
  ...
  return (
    ...
    previous.resolveMenuPath === next.resolveMenuPath &&
    ...
    (fileLinkMeta: MarkdownFileLinkMeta, copyMarkdown: string, mediaSource?: string) => {
      ...
      return (
        <MarkdownFileLink
          ...
          resolveMenuPath={findWorkspaceBasenameMatch}
          onOpenInBrowser={
            threadRef &&
