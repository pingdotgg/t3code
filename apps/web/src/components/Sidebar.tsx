...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const handleRemoveProject = useCallback(
    async (member: SidebarProjectGroupMember) => {
      ...
      let deletionProjectThreads = selectSidebarThreadsForProjectRef(
        useStore.getState(),
        memberProjectRef,
      );
      try {
        await new Promise<void>((resolve, reject) => {
          let settled = false;
          const unsubscribe = projectApi.orchestration.subscribeShell((item) => {
            ...
            unsubscribe();
            useStore.getState().syncServerShellSnapshot(item.snapshot, member.environmentId);
            resolve();
          });
        ...
        deletionProjectThreads = selectSidebarThreadsForProjectRef(
          useStore.getState(),
          memberProjectRef,
        );
      } catch (error) {
      ...
      const blockingThreads = deletionProjectThreads.filter(
        (thread: SidebarThreadSummary) => thread.archivedAt === null,
      );
    ...
    [clearComposerDraftForThread, clearProjectDraftThreadId, getDraftThreadByProjectRef],
  );
