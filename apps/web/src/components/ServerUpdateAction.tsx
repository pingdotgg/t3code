...
import type { EnvironmentId, ServerSelfUpdateMethod } from "@t3tools/contracts";
import {
...
/**
 * One-click sync for a version-skewed server: tells it to install
 * `t3@targetVersion` and restart into it. When the server cannot update
 * itself (older server, dev checkout, desktop-managed) it degrades to
 * copying the manual relaunch command, so the skew warning always offers a
 * way out.
 */
export function ServerUpdateAction({
  ...
  readonly selfUpdateMethod: ServerSelfUpdateMethod | null;
  ...
  }, [environmentId, serverLabel, targetVersion, updateServer]);

  if (selfUpdateMethod === null) {
