...
describe("buildThreadActionMenuItems", () => {
  ...
  it("offers an optional archive action and preserves its blocked state", () => {
    expect(buildThreadActionMenuItems(baseState).some((item) => item.id === "archive")).toBe(false);
    ...
    expect(items.map((item) => item.id)).toEqual(
      expect.arrayContaining(["settle", "archive", "snooze"]),
    );
  });
