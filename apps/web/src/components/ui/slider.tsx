  ...
  return (
    <div className={cn("relative h-12 w-full", className)}>
      ...
      >
        ...
        />
        {valueLabel ? (
          <div className="absolute inset-y-0 right-0 flex items-center pe-2.5 text-ui-xs font-medium text-muted-foreground">
            {valueLabel}
          </div>
        ) : null}
      </div>
      <div
        aria-hidden
        className="pointer-events-none absolute top-1/2 w-2 h-5.5 -translate-y-1/2 rounded-full border border-border transition-[left,transform,border-color] bg-primary"
        style={{
