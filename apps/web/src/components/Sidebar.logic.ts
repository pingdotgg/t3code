...
// writes ONE key to ONE thread on that thread's own server — neighbors,
// possibly living on other servers, are never touched, and every client
// connected to the same servers converges on the same order.
 ...
 * one-time materialization; every drag after that is single-write.
 */
 ...
 * tiebreak), then keyless threads in the standard v2 creation order — so
 * threads on servers that predate reordering keep today's behavior at the
 * bottom of the block instead of breaking the section.
 */
export function sortPinnedThreadsForSidebarV2<
  T extends {
    readonly id: string;
...
>(threads: readonly T[]): T[] {
  ...
  return [...keyed, ...sortThreadsForSidebarV2(keyless)];
}
