...
export function ElectronBrowserHost() {
      ...
      Object.entries(previewByThreadKey).flatMap(([threadKey, previewState]) => {
        ...
        return threadRef
          ? Object.values(previewState.sessions).map((snapshot) => ({
              ...
              touchEmulation: previewState.desktopByTabId[snapshot.tabId]?.touchEmulation ?? false,
            }))
          : [];
      }),
    [previewByThreadKey],
  ...
  return (
    <div className="contents" data-electron-browser-host>
      {sessions.map(
        ({ threadRef, snapshot, runtimeTabId, pictureInPicture, zoomFactor, touchEmulation }) => {
