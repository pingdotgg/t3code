...
export type ThemeWallpaper = Readonly<{
  /** A base64 `data:image/...` URI, embedded so themes stay one portable file. */
  image: string;
  /** How strongly the image reads over the chrome color, 0–1.
   * Defaults to DEFAULT_THEME_WALLPAPER_OPACITY. */
  opacity?: number;
}>;
export type ThemeDefinition = Readonly<{
  ...
  /** Background image ghosted into the window chrome behind the canvas. */
...
/** Keeps any wallpapered theme file inside the existing import budget
 * (MAX_THEME_FILE_BYTES, 256 KB) with room to spare for the palette. */
...
function isThemeWallpaperImage(value: unknown): value is string {
  return (
    typeof value === "string" &&
    value.length <= MAX_THEME_WALLPAPER_IMAGE_LENGTH &&
    THEME_WALLPAPER_IMAGE_PATTERN.test(value)
  );
}

function isThemeWallpaperOpacity(value: unknown): value is number {
  return typeof value === "number" && value >= 0 && value <= 1;
}

function parseThemeWallpaper(value: unknown): ThemeWallpaper {
  if (!isRecord(value)) throw new Error("Theme wallpapers must be objects.");
  if (!isThemeWallpaperImage(value.image)) {
    throw new Error(
      "Wallpaper images must be base64 data: URIs (png, jpeg, webp, avif, or gif) of at most 192 KB.",
    );
  }
  if (value.opacity !== undefined && !isThemeWallpaperOpacity(value.opacity)) {
    throw new Error("Wallpaper opacity must be a number between 0 and 1.");
  }
  return {
    image: value.image,
...
/** Tolerant twin of parseThemeWallpaper for localStorage reads: a malformed
 * wallpaper drops away while the theme's colors survive, matching how
 * parseStoredThemeColors treats unknown roles. */
function parseStoredThemeWallpaper(value: unknown): ThemeWallpaper | undefined {
  if (!isRecord(value) || !isThemeWallpaperImage(value.image)) return undefined;
  return {
    image: value.image,
    ...(isThemeWallpaperOpacity(value.opacity) ? { opacity: value.opacity } : {}),
  };
}
...
function parseStoredTheme(value: unknown): ThemeDefinition | null {
  ...
  const wallpaper = parseStoredThemeWallpaper(value.wallpaper);
...
export function parseThemeFile(value: unknown): ThemeDefinition {
  ...
  return {
    ...
    ...(value.wallpaper !== undefined ? { wallpaper: parseThemeWallpaper(value.wallpaper) } : {}),
    ...(value.sidebarArtwork === true ? { sidebarArtwork: true } : {}),
    ...(value.managed === true ? { managed: true } : {}),
  };
}
...
const THEME_WALLPAPER_IMAGE_VARIABLE = "--theme-wallpaper-image";
const THEME_WALLPAPER_OPACITY_VARIABLE = "--theme-wallpaper-opacity";

/** Sets or clears the wallpaper hook (`data-wallpaper` plus its variables);
 * index.css paints the image into the body chrome when the hook is present.
 * The data URI charset excludes quotes, so the url() below cannot escape. */
function applyThemeWallpaper(root: HTMLElement, wallpaper?: ThemeWallpaper): void {
  if (wallpaper) {
    root.dataset.wallpaper = "";
    root.style.setProperty(THEME_WALLPAPER_IMAGE_VARIABLE, `url("${wallpaper.image}")`);
    root.style.setProperty(
      THEME_WALLPAPER_OPACITY_VARIABLE,
      String(wallpaper.opacity ?? DEFAULT_THEME_WALLPAPER_OPACITY),
    );
  } else {
    delete root.dataset.wallpaper;
    root.style.removeProperty(THEME_WALLPAPER_IMAGE_VARIABLE);
    root.style.removeProperty(THEME_WALLPAPER_OPACITY_VARIABLE);
  }
}
...
export function applyThemePalette(theme: ThemePreference, appearance?: ThemeAppearance): void {
  ...
  if (palette) {
    ...
    applyThemeWallpaper(root, palette.wallpaper);
    return;
  }
