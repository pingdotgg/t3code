...
import { parseStoredThemeSyntax, parseThemeSyntax, type ThemeSyntax } from "./themeSyntax";
...
export type ThemeDefinition = Readonly<{
  ...
  managed?: boolean;
  /** VS Code `tokenColors` per appearance. Missing means Pierre highlighting. */
  syntax?: ThemeSyntax;
}>;
...
export function getThemePreviewRevision(): number {
  return themePreviewRevision;
}
...
export function notifyThemePreview(): void {
  themePreviewRevision += 1;
...
function parseStoredTheme(value: unknown): ThemeDefinition | null {
  ...
  const syntax = value.syntax === undefined ? undefined : parseStoredThemeSyntax(value.syntax);
...
export function parseThemeFile(value: unknown): ThemeDefinition {
  ...
  const syntax = value.syntax === undefined ? undefined : parseThemeSyntax(value.syntax);
...
export function applyThemeColorPreview(colors: ThemeColors, appearance: ThemeAppearance): void {
  ...
  const alreadyPreviewing = themePreviewAppearance !== null || themePreviewSidebarArtwork !== null;
  themePreviewAppearance = appearance;
  ...
  if (alreadyPreviewing) notifyThemePreview();
}
