...
import { parseThemeTokenColors, type ThemeSyntax } from "./themeSyntax";
...
export type ThemeMode = Readonly<{
  colors: ThemeColors;
  syntax?: ThemeSyntax;
}>;
export type ThemeModes = Readonly<Partial<Record<ThemeAppearance, ThemeMode>>>;
export type ThemeDefinition = Readonly<{
  id: string;
  label: string;
  /** Preferred appearance and fallback when the requested mode is unavailable. */
  appearance: ThemeAppearance;
  /** Colors and syntax move together as one appearance mode. */
  modes: ThemeModes;
...
export type ThemeDefinitionInput = Readonly<{
  id?: string;
  name: string;
  appearance: ThemeAppearance;
  modes: ThemeModes;
...
function parseThemeSyntax(value: unknown): ThemeSyntax | null {
  return isRecord(value) ? parseThemeTokenColors(value.tokenColors) : null;
}
...
function themeModesFromV1(
  appearance: ThemeAppearance,
  colors: ThemeColors,
  variants: ThemeVariants | undefined,
  syntax: ThemeSyntaxVariants | null | undefined,
): ThemeModes {
  const modes: Partial<Record<ThemeAppearance, ThemeMode>> = {
    [appearance]: {
      colors,
      ...(syntax?.[appearance] ? { syntax: syntax[appearance] } : {}),
    },
  };
  for (const [variantAppearance, variantColors] of Object.entries(variants ?? {})) {
    modes[variantAppearance as ThemeAppearance] = {
      colors: variantColors,
      ...(syntax?.[variantAppearance as ThemeAppearance]
        ? { syntax: syntax[variantAppearance as ThemeAppearance] }
        : {}),
    };
  }
  return modes;
...
function parseStoredTheme(value: unknown): ThemeDefinition | null {
  ...
  return {
    ...
    modes: themeModesFromV1(value.appearance, colors, variants ?? undefined, syntax),
...
function canonicalizeThemeDefinition(theme: ThemeDefinition): ThemeDefinition {
  if (!theme.modes[theme.appearance]) {
    throw new Error(`Theme mode "${theme.appearance}" is missing.`);
  }
  return {
    ...theme,
    modes: Object.fromEntries(
      Object.entries(theme.modes).map(([appearance, mode]) => [
        appearance,
        {
          colors: decodeThemeColors(mode.colors),
...
function themeModesToV1(theme: ThemeDefinition) {
  const primaryMode = theme.modes[theme.appearance];
  if (!primaryMode) throw new Error(`Theme mode "${theme.appearance}" is missing.`);

  const variants: Partial<Record<ThemeAppearance, ThemeColors>> = {};
  const syntax: Partial<Record<ThemeAppearance, ThemeSyntax>> = {};
  for (const [appearance, mode] of Object.entries(theme.modes)) {
    const themeAppearance = appearance as ThemeAppearance;
    if (themeAppearance !== theme.appearance) variants[themeAppearance] = mode.colors;
    if (mode.syntax) syntax[themeAppearance] = mode.syntax;
  }
  return {
    appearance: theme.appearance,
    colors: primaryMode.colors,
    ...(Object.keys(variants).length > 0 ? { variants } : {}),
    ...(Object.keys(syntax).length > 0 ? { syntax } : {}),
  };
}

function toStoredThemeV1(theme: ThemeDefinition) {
  return {
    id: theme.id,
    label: theme.label,
    ...themeModesToV1(theme),
...
export const T3_CHAT_THEME: ThemeDefinition = {
  ...
  appearance: "light",
  modes: {
    light: { colors: decodeThemeColors(T3_CHAT_LIGHT_COLORS) },
    dark: { colors: decodeThemeColors(T3_CHAT_DARK_COLORS) },
  },
  sidebarArtwork: true,
};
...
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  return T3_CHAT_THEME.modes[appearance]!.colors;
}
...
export function getThemeColorsForMode(
  ...
  return getThemeMode(theme, appearance)?.colors ?? null;
}

export function getThemeModes(theme: ThemeDefinition): ReadonlyArray<ThemeAppearance> {
  return (["light", "dark"] as const).filter((appearance) => getThemeMode(theme, appearance));
}
...
export function installCustomTheme(theme: ThemeDefinition): ThemeDefinition {
  ...
  saveCustomThemes([...library.storedThemes, toStoredThemeV1(canonicalTheme)], themes);
...
export function updateCustomTheme(theme: ThemeDefinition): ThemeDefinition {
  ...
  for (const storedTheme of library.storedThemes) {
    if (!storedThemeHasId(storedTheme, theme.id)) {
      ...
      nextStoredThemes.push(toStoredThemeV1(canonicalTheme));
      replaced = true;
    }
  }
...
export function replaceCustomThemeCollection(
  ...
  for (const storedTheme of library.storedThemes) {
    if (!storedThemeHasCollectionId(storedTheme, collectionId)) {
      ...
      nextStoredThemes.push(...replacement.map(toStoredThemeV1));
      insertedReplacement = true;
    }
  }
  if (!insertedReplacement) nextStoredThemes.push(...replacement.map(toStoredThemeV1));
...
export function createThemeDefinition(input: ThemeDefinitionInput): ThemeDefinition {
  const provisionalTheme: ThemeDefinition = {
    id: input.id ?? themeIdFromName(input.name),
    label: input.name,
  ...
  return parseThemeFile({
    ...
    id: input.id,
    name: input.name,
    ...themeModesToV1(provisionalTheme),
    ...(input.collection ? { collection: input.collection } : {}),
    ...(input.managed ? { managed: true } : {}),
  });
}
...
export function applyThemePalette(theme: ThemePreference, appearance?: ThemeAppearance): void {
  ...
  if (palette) {
    ...
    const colors =
      getThemeColorsForMode(palette, mode) ?? palette.modes[palette.appearance]!.colors;
    for (const [role, value] of Object.entries(colors) as Array<[ThemeColorRole, string]>) {
