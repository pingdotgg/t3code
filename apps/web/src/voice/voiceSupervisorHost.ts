...
import { MAX_VOICE_TRANSCRIPT_CHARS } from "./voiceSupervisorStore";
...
export interface VoiceSupervisorHostStartInput {
  ...
  readonly getClientSecret: (
    signal: AbortSignal,
  ) => ReturnType<Parameters<RealtimeSessionController["connect"]>[0]["getClientSecret"]>;
...
interface ActiveVoiceSession {
  readonly generation: number;
  readonly transport: RealtimeSessionController;
  ...
  toolEpoch: number;
  transcriptTimer: VoiceSupervisorTimer | null;
  handshakeTimer: VoiceSupervisorTimer | null;
...
export function createVoiceSupervisorHostController(
  ...
  const createTransport = dependencies.createTransport ?? createRealtimeSessionController;
  const now = dependencies.now ?? Date.now;
  const schedule = dependencies.schedule ?? ((callback, delayMs) => setTimeout(callback, delayMs));
  const cancelScheduled = dependencies.cancelScheduled ?? ((handle) => clearTimeout(handle));
  ...
  const clearTranscriptTimer = (candidate: ActiveVoiceSession) => {
    if (candidate.transcriptTimer !== null) cancelScheduled(candidate.transcriptTimer);
    candidate.transcriptTimer = null;
    candidate.transcriptDeltas.clear();
  };

  const clearHandshakeTimer = (candidate: ActiveVoiceSession) => {
    if (candidate.handshakeTimer !== null) cancelScheduled(candidate.handshakeTimer);
  ...
  const start = (input: VoiceSupervisorHostStartInput): number => {
    ...
    let transport: RealtimeSessionController;
    try {
    ...
    active = candidate;
    dependencies.state.beginSession(generation, now());
    let attempt: ReturnType<RealtimeSessionController["connect"]>;
    try {
