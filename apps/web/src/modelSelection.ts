...
function resolveTextGenerationProvider(
  ...
  const requestedSnapshot = providers.find((provider) => provider.provider === requested);
  if (requestedSnapshot?.enabled !== false) {
    return requested;
  }

  const fallback = TEXT_GENERATION_PROVIDERS.find((provider) => {
...
export function resolveAppModelSelectionState(
  ...
  const provider = resolveTextGenerationProvider(settings, providers);
