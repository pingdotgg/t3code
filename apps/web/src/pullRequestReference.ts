...
const GITCAFE_CLI_PR_CHECKOUT_PATTERN = /^cafe\s+pr\s+checkout\s+(.+)$/i;
...
export function parsePullRequestReference(input: string): string | null {
  ...
  const cafeCliCheckoutMatch = GITCAFE_CLI_PR_CHECKOUT_PATTERN.exec(trimmed);
