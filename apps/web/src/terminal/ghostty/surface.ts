...
import { createTerminalClipboardSession } from "@t3tools/client-runtime/terminal-clipboard";
...
export class GhosttyTerminalSurface {
  ...
  private selectionDrag: {
    ...
    // A word/line gesture keeps its original range anchored through viewport scrolling.
    base: {
      mode: "word" | "line";
  ...
  private readonly clipboard = createTerminalClipboardSession({
    isEligible: () => !this.disposed && this.visible && this.focused && document.hasFocus(),
    onCopy: (text, canWrite) => this.options.onClipboardWrite?.(text, canWrite),
  });
  ...
  private constructor(
    ...
    this.canvas = canvas;
    this.input = input;
    // The textarea can receive focus while fonts and WASM are still loading.
    this.focused = document.activeElement === input;
  ...
  /** Feed live attachment events before renderer retention and batching. */
  observeClipboard(event: TerminalAttachStreamEvent): void {
    if (this.disposed) return;
    this.clipboard.update(event);
  }
  ...
  /** Sends committed user input; terminal-generated replies bypass selection cleanup. */
  ...
  private readonly onKeyDown = (event: KeyboardEvent) => {
          ...
          void Promise.resolve().then(() => {
            ...
            void clipboard.writeText(selection).then(
              () => {
                ...
                if (this.disposed || this.copyShortcutToken !== token) return;
                if (this.clearSelectionAfterCopy) this.clearSelection();
              },
              () => {
    ...
    if (MODIFIER_KEYS.has(event.key)) this.options.onData(data);
    else this.sendUserInput(data);
  };
  ...
  private get activePointerId(): number | null {
    return (
      this.selectionDrag?.pointerId ?? this.mouseReportingPointerId ?? this.linkActivationPointerId
    );
  }
  ...
  private readonly onPointerDown = (event: PointerEvent) => {
    ...
    const mode = clickCount >= 3 ? "line" : clickCount === 2 ? "word" : null;
    const range =
      mode === "line"
    ...
    this.selectionDrag = {
      ...
      base: mode !== null && range !== null ? { mode, ...range.screen } : null,
    };
    if (range) {
  ...
  private readonly onPointerMove = (event: PointerEvent) => {
    ...
    if (
      this.mouseReportingPointerId === event.pointerId ||
      // A native drag owns its pointer until release, even if Shift is let go.
    ...
    if (!this.selectionAnchorScreen || drag?.pointerId !== event.pointerId) {
    ...
    const cell = this.cellAt(event.clientX, event.clientY);
    if (cell.x === drag.lastCell.x && cell.y === drag.lastCell.y) return;
    this.extendSelectionTo(cell);
  };

  private extendSelectionTo(cell: { x: number; y: number }): void {
    const drag = this.selectionDrag;
    const anchorScreen = this.selectionAnchorScreen;
    if (!drag || anchorScreen === null) return;
    drag.moved = true;
    drag.lastCell = cell;
  ...
  private setSelectionAutoscroll(delta: number): void {
    ...
    this.selectionScrollTimer = window.setInterval(() => {
      ...
      const pointer = this.selectionDrag?.pointer;
      if (pointer) this.extendSelectionTo(this.cellAt(pointer.x, pointer.y));
    }, 80);
  }
  ...
  private endSelectionDrag(): void {
    ...
    if (drag && this.canvas.hasPointerCapture(drag.pointerId)) {
  ...
  private readonly onPointerUp = (event: PointerEvent) => {
    ...
    if (event.type !== "pointercancel" && this.selectionAnchorScreen) {
      ...
      const cell = this.cellAt(event.clientX, event.clientY);
      if (cell.x !== drag.lastCell.x || cell.y !== drag.lastCell.y) {
        this.extendSelectionTo(cell);
      }
    }
    this.endSelectionDrag();
    if (!drag.moved && drag.base === null) this.clearSelection();
    this.options.onSelectionChange();
  };
