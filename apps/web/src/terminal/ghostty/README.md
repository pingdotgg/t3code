...
- `vendor/` holds only the artifacts, reproducibly generated by
  ...
  in its build info and the ABI test verifies it against mobile's `VERSION`. The build
  compiles in only the feature areas this adapter calls (`-Dvt-features`), so a new
  `ghostty_*` export used here must belong to an enabled feature or `runtime.call` throws.
