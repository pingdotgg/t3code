...
- `ghostty-write-pty.wasm` is a 119-byte callback trampoline for terminal-generated PTY replies.
...
- `vendor/` holds only the artifacts, reproducibly generated by
  ...
  in its build info and the ABI test verifies it against the canonical `VERSION`.
