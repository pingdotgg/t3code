...
describe("terminalLinkAtPositionWithRange", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    ...
    expect(terminalLinkAtPositionWithRange([row], 0, 2)?.text ?? null).toBe("https://t3.codes");
    expect(terminalLinkAtPositionWithRange([row], 0, cells.length - 1)?.text ?? null).toBe(
      "https://t3.codes",
    );
    expect(terminalLinkAtPositionWithRange([row], 0, 0)?.text ?? null).toBeNull();
  ...
  it("uses shared path matching and reconstructs soft-wrapped links", () => {
    ...
    expect(terminalLinkAtPositionWithRange(rows, 0, 8)?.text ?? null).toBe(
      "https://example.com/reference",
    );
    expect(terminalLinkAtPositionWithRange(rows, 1, 4)?.text ?? null).toBe(
      "https://example.com/reference",
    );
    expect(terminalLinkAtPositionWithRange(rows, 2, 2)?.text ?? null).toBe("~/project/file");
    expect(terminalLinkAtPositionWithRange(rows, 3, 4)?.text ?? null).toBe("C:\\repo\\file.ts");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    expect(terminalLinkAtPositionWithRange(headCut, 0, 4)?.text ?? null).toBeNull();
    // The bottom row soft-wraps on below the viewport.
    const tailCut = [row("https://t3.codes", false, true)];
    expect(terminalLinkAtPositionWithRange(tailCut, 0, 8)?.text ?? null).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://t3.codes", false), row("", false)];
    expect(terminalLinkAtPositionWithRange(complete, 0, 8)?.text ?? null).toBe("https://t3.codes");
    ...
    expect(terminalLinkAtPositionWithRange([wideFull], 0, 8)?.text ?? null).toBeNull();
    ...
    expect(terminalLinkAtPositionWithRange([unwrittenTail], 0, 8)?.text ?? null).toBe(
      "https://t3.codes",
    );
  });
});
