...
describe("uiStateStore pure functions", () => {
  ...
  it("reorders threads from their scoped sidebar keys", () => {
    const state = makeUiState();
    const currentOrder = ["env-a:thread-1", "env-b:thread-1", "env-a:thread-2"];

    const next = reorderThreads(state, currentOrder, ["env-a:thread-2"], ["env-a:thread-1"]);

    expect(next.threadOrder).toEqual(["env-a:thread-2", "env-a:thread-1", "env-b:thread-1"]);
  });
