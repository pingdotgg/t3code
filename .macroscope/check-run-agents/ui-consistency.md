...
- Keep global CSS when it is genuinely reusable or behaviorally complex: generated markdown or imperative DOM, custom elements and shadow roots, masks, pseudo-elements, animations, glass composition, runtime theme variables, safe-area calculations, scrollbar-lane preservation, Electron drag regions, and browser/vendor integration.
...
- Prefer source-level logic over clever selectors when behavior depends on consumer-provided class strings. For default menu sizing, for example, callers with `w-*`, `min-w-*`, or `max-w-*` must retain control while callers without an explicit width keep the default minimum width. Account for variant prefixes, arbitrary values, functional/non-string class inputs, and Tailwind merge order.
