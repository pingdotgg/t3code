...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".moreCode"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("MORECODE_T3CODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("MORECODE_T3CODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid MORECODE_T3CODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `MORECODE_T3CODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric MORECODE_T3CODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed MORECODE_T3CODE_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      MORECODE_T3CODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.MORECODE_T3CODE_PORT = String(serverPort);
      ...
      delete output.MORECODE_T3CODE_MODE;
      delete output.MORECODE_T3CODE_NO_BROWSER;
      delete output.MORECODE_T3CODE_HOST;
    }
    ...
    if (!isDesktopMode && noBrowser !== undefined) {
      output.MORECODE_T3CODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.MORECODE_T3CODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.MORECODE_T3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd
        ? "1"
        : "0";
    } else {
      delete output.MORECODE_T3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.MORECODE_T3CODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.MORECODE_T3CODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.MORECODE_T3CODE_MODE = "web";
      delete output.MORECODE_T3CODE_DESKTOP_WS_URL;
    }
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message:
              "Failed to read MORECODE_T3CODE_PORT_OFFSET/MORECODE_T3CODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.MORECODE_T3CODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.MORECODE_T3CODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Base directory for all more Code data (equivalent to MORECODE_T3CODE_HOME).",
    ),
    Flag.withFallbackConfig(optionalStringConfig("MORECODE_T3CODE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to MORECODE_T3CODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("MORECODE_T3CODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to MORECODE_T3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(
      optionalBooleanConfig("MORECODE_T3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
    ),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription(
      "WebSocket event logging toggle (equivalent to MORECODE_T3CODE_LOG_WS_EVENTS).",
    ),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("MORECODE_T3CODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to MORECODE_T3CODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("MORECODE_T3CODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to MORECODE_T3CODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("MORECODE_T3CODE_PORT")),
  ),
