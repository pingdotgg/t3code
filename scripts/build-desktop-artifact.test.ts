...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    // This fork brands the stable desktop build as "T3 (Logflash)" (see apps/desktop/package.json
    // productName); nightly builds keep the upstream "T3 Code (Nightly)" name from the function.
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 (Logflash)");
