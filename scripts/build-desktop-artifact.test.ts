...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      // The t3code scheme is internal to Electron's renderer, so neither
      // platform registers it with the OS.
      assert.notProperty(mac.mac as Record<string, unknown>, "protocols");
      assert.notProperty(linux.linux as Record<string, unknown>, "protocols");
