...
- `apps/web/src/components/settings/ArchiveSettings.tsx` requests the shared themed confirmation dialog's destructive variant for single and project delete actions, while project unarchive keeps the default variant. `apps/web/src/components/settings/ArchiveSettings.test.tsx` covers all three confirmation paths through their rendered action handlers.
