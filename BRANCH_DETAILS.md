...
- When a bounded WebSocket resume falls back to a fresh snapshot, an `OrchestrationGetSnapshotError` with reason `thread_not_found` applies the same cache removal and deleted-state transition for a warm cached thread.
