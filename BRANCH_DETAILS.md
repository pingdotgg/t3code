...
- Desktop preview guests create their CDP debugger session lazily, with initialization included in the automation operation deadline. `apps/desktop/src/preview/Manager.ts` restores a persisted non-system color-scheme override after webview registration or detached DevTools closes through a separately bounded recovery path; tabs following the system scheme stay detached until the next automation operation.
