...
- Electron screenshot capture can still be unavailable when both the CDP view capture and bounded Electron `capturePage` fallback fail. The intended degraded result remains a usable semantic snapshot with `screenshot: null`, not raster evidence.
