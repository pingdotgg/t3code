...
async function resolve({ github, context, core }) {
  ...
  const serverTestJobs = sourceJobs.filter((job) => /^Test Server \d+$/.test(job.name));
  if (serverTestJobs.length > 0 && serverTestJobs.every((job) => job.conclusion === "skipped")) {
    core.info("Skipping thread transfer report because the server tests were not needed.");
    core.setOutput("publish", "false");
    return;
  }
