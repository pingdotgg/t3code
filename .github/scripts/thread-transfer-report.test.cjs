  ...
  const github = {
    paginate: async (method, input) => {
      ...
      if (method === listJobsForWorkflowRun) {
        return [1, 2, 3].map((shard) => ({
          name: `Test Server ${shard}`,
          conclusion: "success",
        }));
      }
...
test("does not publish when every server test shard was intentionally skipped", async () => {
  const outputs = {};
  const listJobsForWorkflowRun = () => {};
  const listWorkflowRunArtifacts = () => {};
  await resolve({
    github: {
      paginate: async (method) => {
        ...
        if (method === listWorkflowRunArtifacts) {
          throw new Error("must not inspect artifacts for skipped server tests");
        }
        throw new Error("unexpected pagination call");
      },
      rest: {
